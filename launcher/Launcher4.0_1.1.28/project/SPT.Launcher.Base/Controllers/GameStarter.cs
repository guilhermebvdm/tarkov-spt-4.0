...
namespace SPT.Launcher
{
    public class GameStarter
    {
        ...
        public async Task<GameStarterResult> LaunchGame(ServerInfo server, AccountInfo account, string gamePath)
        {
            ...
            try
            {
                LogManager.Instance.Info("[LaunchGame] Starting patch process...");
                await _frontend.CompletePatchTask(patchRunner.PatchFiles());
                LogManager.Instance.Info("[LaunchGame] Patch process completed successfully.");
            }
            catch (TaskCanceledException)
            {
                LogManager.Instance.Warning("[LaunchGame] Patch task canceled.");
                return GameStarterResult.FromError(-4);
            }
            catch (Exception ex)
            {
                LogManager.Instance.Error($"[LaunchGame] Fatal error during patching: {ex.Message}");
