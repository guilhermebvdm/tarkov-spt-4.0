...
namespace SPT.Launcher.Models.Launcher
{
    public class ProfileInfo : NotifyPropertyChangedBase
    {
        ...
        {
            if (string.IsNullOrEmpty(expectedVersion) || string.IsNullOrEmpty(currentVersion)) return false;
        ...
        public ProfileInfo(ServerProfileInfo serverProfileInfo)
        {
            ...
            SideImage = !string.IsNullOrEmpty(Side) 
                ? Path.Combine(ImageRequest.ImageCacheFolder, $"side_{Side.ToLower()}.png")
                : Path.Combine(ImageRequest.ImageCacheFolder, "side_unknown.png");
