...
namespace SPT.Launcher.Helpers
{
    public static class GameStateDetector
    {
        ...
        {
            string root = string.IsNullOrWhiteSpace(gamePath) ? AppDomain.CurrentDomain.BaseDirectory : gamePath;
            return Path.Combine(root, "SPT", "user", "launcher", StateFileName);
        }
        ...
        public static BaseGameState LoadState(string gamePath)
        {
            lock (_fileLock)
            {
                try
                {
                    ...
                    string root = string.IsNullOrWhiteSpace(gamePath) ? AppDomain.CurrentDomain.BaseDirectory : gamePath;
                    string legacyPath = Path.Combine(root, "user", "launcher", StateFileName);

                    // Migração automática se existir na pasta legada fora da SPT
                    if (!File.Exists(filePath) && File.Exists(legacyPath))
                    {
                        try
                        {
                            ...
                            LogManager.Instance.Info($"[GameStateDetector] Migrado {StateFileName} de {legacyPath} para {filePath}");
                        }
                        catch { }
                    }
                ...
                if (exeExists && dataExists)
                {
                    ...
                    SaveStateInternal(gamePath, legacyState);
                    return legacyState;
                }
        ...
        private static void SaveStateInternal(string gamePath, BaseGameState state)
        {
            try
            {
        ...
        public static void SaveState(string gamePath, BaseGameState state)
        {
            lock (_fileLock)
            {
                SaveStateInternal(gamePath, state);
            }
        }
        ...
        public static void MarkAsDownloading(string gamePath, string torrentHash, double progress, long downloaded, long total)
        {
            lock (_fileLock)
            {
                var state = LoadStateInternal(gamePath);
        ...
        private static BaseGameState LoadStateInternal(string gamePath)
        {
            try
            {
                string filePath = GetStateFilePath(gamePath);
