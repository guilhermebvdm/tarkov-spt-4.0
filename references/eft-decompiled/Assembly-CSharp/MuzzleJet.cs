...
public class MuzzleJet : MuzzleEffect
{
	...
	private static readonly int int_0 = Shader.PropertyToID("_RandomVals");

	private static readonly int int_1 = Shader.PropertyToID("_RotMx");

	private static readonly int int_2 = Shader.PropertyToID("_CellShift");

	public static void RandomizeMaterial(Material material, Vector2 cellSize)
	{
		material.SetVector(int_0, new Vector4(1f + UnityEngine.Random.value, 1f + UnityEngine.Random.value, 1f + UnityEngine.Random.value, 1f + UnityEngine.Random.value) * 878788.8f);
		Vector2 normalized = new Vector2(0.5f - UnityEngine.Random.value, 0.5f - UnityEngine.Random.value).normalized;
		material.SetVector(int_1, new Vector4(normalized.x, normalized.y, 0f - normalized.y, normalized.x));
		material.SetVector(int_2, new Vector4((float)UnityEngine.Random.Range(-255, 255) * cellSize.x, (float)UnityEngine.Random.Range(-255, 255) * cellSize.y, 0f, 0f));
	}

	public void method_0(ref int quadNum, Particle particle, Vector3 axis, Vector3 position, Vector2 cellSize, Vector2 posNorm, byte commonRnd, float addLengthRnd, Vector3[] vertices, Vector3[] normals, Vector2[] uv0, Vector2[] uv1, Color32[] colors, Vector4[] tangents)
	{
		int num = quadNum;
		int num2 = quadNum + 1;
		int num3 = quadNum + 2;
	...
	public void method_1(ref int quadNum, Transform patent, Vector2 cellSize, Vector3[] vertices, Vector3[] normals, Vector2[] uv0, Vector2[] uv1, Color32[] colors, Vector4[] tangents)
	{
		Vector3 axis = ((patent == base.transform) ? Vector3.down : (TransformHelperClass.GetRotationRelativeToParent(patent, base.transform) * Vector3.down));
		Vector3 position = ((patent == base.transform) ? Vector3.zero : TransformHelperClass.GetPositionRelativeToParent(patent, base.transform));
		...
		{
			method_0(ref quadNum, particle2, axis, position, cellSize, posNorm, commonRnd, addLengthRnd, vertices, normals, uv0, uv1, colors, tangents);
		}
	}
	...
	public static Transform UpdateOrCreateMesh(MuzzleJet[] muzzleJets, Transform patent, Material material, Vector2 cellSize)
	{
		...
		{
			muzzleJets[k].method_1(ref quadNum, patent, cellSize, vertices, normals, array, array2, array3, tangents);
		}
		Transform transform = patent.Find("MuzzleJetCombinedMesh");
		if (transform == null)
		{
			GameObject obj = new GameObject("MuzzleJetCombinedMesh", typeof(MeshFilter), typeof(MeshRenderer))
			{
			...
			Mesh mesh = new Mesh
			{
				...
				colors32 = array3,
				tangents = tangents,
				name = "MuzzleJet UpdateOrCreateMesh"
			};
			mesh.MarkDynamic();
