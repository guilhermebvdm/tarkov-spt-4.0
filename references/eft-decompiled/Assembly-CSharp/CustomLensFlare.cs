...
public class CustomLensFlare
{
	...
	public class Element
	{
		...
		public void Calc(Vector2 texScale, float screenAspect)
		{
			...
			HalfSize = new Vector2(1f, screenAspect) * Size * 0.5f;
		}
		...
		public void Draw(Vector2 pos, float distance, float alpha)
		{
			...
			if (Rotation == 0f)
			{
				GL.TexCoord(TexCoords[0]);
				GL.Vertex3(pos.x - HalfSize.x, pos.y - HalfSize.y, 0f);
				GL.TexCoord(TexCoords[1]);
				GL.Vertex3(pos.x - HalfSize.x, pos.y + HalfSize.y, 0f);
				GL.TexCoord(TexCoords[2]);
				GL.Vertex3(pos.x + HalfSize.x, pos.y + HalfSize.y, 0f);
				GL.TexCoord(TexCoords[3]);
				GL.Vertex3(pos.x + HalfSize.x, pos.y - HalfSize.y, 0f);
			}
			else
			{
				...
				float num3 = (num - num2) * HalfSize.x;
				float num4 = (num + num2) * HalfSize.x;
				float num5 = num3 * ScreenAspect;
				float num6 = num4 * ScreenAspect;
				GL.TexCoord(TexCoords[0]);
	...
	public void Draw(Vector2 positionOnScreen)
	{
		if (!(FlareMaterial == null) && Elements.Length != 0)
		{
			Vector2 vector = Center - positionOnScreen;
			...
			{
				Vector2 pos = Center + vector * element.Position;
