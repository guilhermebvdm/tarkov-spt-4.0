...
public class EditorScreenshoter : MonoBehaviour
{
	...
	public static Texture2D MakeScreenshot(Camera camera, int width, int height)
	{
		...
		texture2D.name = "EditorScreenshoter";
		...
		GClass972.TransformTexture(temporary, new Color(0f, 0f, 0f, 1f), new Color(1f, 1f, 1f, 0f), temporary2);
	...
	public void MakeScreenshotAndSave()
	{
		...
		if (!Directory.Exists(text + "/EFT_Screenshots/"))
		{
			Directory.CreateDirectory(text + "/EFT_Screenshots/");
		}
		_screenshot = MakeScreenshot(_camera, (int)_textureDimensions.x, (int)_textureDimensions.y);
		string text2 = text + "/EFT_Screenshots/eft_screenshot" + EFTDateTimeClass.Now.ToString("[HH-mm-ss]dd-MM-yyyy") + ".png";
		File.WriteAllBytes(text2, _screenshot.EncodeToPNG());
		Debug.Log("Screenshot saved at " + text2);
	}
}
