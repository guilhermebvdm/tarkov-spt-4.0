...
[Serializable]
public class AnimationEvent : ICloneable
{
	...
	public bool IsConditionsSucceed(IAnimator animator)
	{
		...
		while (true)
		{
			if (num < EventConditions.Count)
			{
				if (!EventConditions[num].IsSucceed(animator))
				{
					break;
				}
				num++;
	...
	{
		return $"{FunctionName}({Parameter}) : {Time} | {FunctionNameHash}";
	}
