using System;
using System.Collections.Generic;
using System.Runtime.CompilerServices;
using EFT;
using UnityEngine;

public class GClass362 : GClass356
{
	[Serializable]
	[CompilerGenerated]
	public class Class236
	{
		public static readonly Class236 class236_0 = new Class236();

		public static Func<bool> func_0;

		public bool method_0()
		{
			return true;
		}
	}

	[NonSerialized]
	public const float Float_0 = 30f;

	[NonSerialized]
	public const int Int_0 = 1;

	[NonSerialized]
	public const int Int_1 = 2;

	[NonSerialized]
	public const int Int_2 = 3;

	[NonSerialized]
	public const int Int_3 = 4;

	[NonSerialized]
	public const int Int_4 = 5;

	[NonSerialized]
	public const int Int_5 = 7;

	[NonSerialized]
	public const int Int_6 = 11;

	[NonSerialized]
	public const int Int_7 = 12;

	[NonSerialized]
	public float Float_1;

	[NonSerialized]
	public bool Bool_0;

	[NonSerialized]
	public GClass135 Gclass135_0;

	[NonSerialized]
	public GClass151 Gclass151_0;

	[NonSerialized]
	public GClass152 Gclass152_0;

	[NonSerialized]
	public Class103 Class103_0;

	[NonSerialized]
	public float Float_2;

	public GClass362(BotOwner owner)
		: base(owner)
	{
		Gclass151_0 = new GClass151(owner, 150);
		method_0(1, Gclass151_0, activeOnStart: false);
		GClass48 layer = new GClass48(owner, 130);
		method_0(5, layer, activeOnStart: true);
		GClass118 layer2 = new GClass118(owner, 128);
		method_0(12, layer2, activeOnStart: true);
		Gclass152_0 = new GClass152(owner, 120);
		method_0(4, Gclass152_0, activeOnStart: true);
		GClass166 layer3 = new GClass166(owner, 100);
		method_0(11, layer3, activeOnStart: true);
		GClass101 layer4 = new GClass101(owner, 47, () => true);
		method_0(3, layer4, activeOnStart: true);
		Class103_0 = new Class103(owner, 12);
		method_0(7, Class103_0, activeOnStart: true);
		Gclass135_0 = new GClass135(owner, 11, tryFinGreenFirst: true);
		method_0(2, Gclass135_0, activeOnStart: true);
		Owner.Boss.OnFollowerStatusChange += method_7;
	}

	public override GClass671 EventsPriority()
	{
		return new GClass671(-1, 125, 55, -1);
	}

	public override void ManualUpdate()
	{
		method_9();
		if (Float_1 < Time.time)
		{
			Float_1 = Time.time + 2f;
			method_6();
		}
		base.ManualUpdate();
	}

	public override void SetCorePosition(Vector3 pos)
	{
		Gclass135_0.SetCorePosition(pos);
	}

	public override string ShortName()
	{
		return "SectantPriest";
	}

	public void method_6()
	{
		Bool_0 = Owner.AIData.PlaceInfo != null && Owner.AIData.PlaceInfo.IsInside;
		Gclass152_0.SetInside(Bool_0);
		Gclass151_0.SetInside(Bool_0);
	}

	public void method_7(BotOwner arg1, FollowerStatusChange arg2)
	{
		if (Owner.Boss.Followers.Count < 1)
		{
			method_8();
		}
	}

	public void method_8()
	{
		if (Owner.WeaponManager.Grenades.HaveGrenade)
		{
			method_1(1);
		}
		else
		{
			UnityEngine.Debug.LogError("sectant priest can't change logic to suicide because have no grenade");
		}
	}

	public void method_9()
	{
		if (!Bool_0 || !(Float_2 < Time.time))
		{
			return;
		}
		Float_2 = Time.time + 5f;
		if (Owner.Memory.GoalEnemy != null)
		{
			return;
		}
		foreach (KeyValuePair<IPlayer, EnemyInfo> enemyInfo in Owner.EnemiesController.EnemyInfos)
		{
			if (enemyInfo.Value.Distance < 30f)
			{
				enemyInfo.Value.SetVisible(value: true);
			}
		}
	}

	public override void Dispose()
	{
		Owner.Boss.OnFollowerStatusChange -= method_7;
		base.Dispose();
	}
}
