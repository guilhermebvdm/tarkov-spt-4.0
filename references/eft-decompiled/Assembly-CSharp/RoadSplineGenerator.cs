...
public class RoadSplineGenerator : MonoBehaviour
{
	...
	public class Road
	{
			...
			set
			{
				if (_hold != value)
				{
					...
					if (value)
					{
						...
						smethod_3(ref points, ref vals, Parent.Error, Parent.MaxOptimization);
						Optimized = points;
						OptimizedVals = vals;
					}
				}
		...
		public void GeneratePoints(float error)
		{
			...
			{
				Optimized = smethod_1(Parent.GetPoints(this), error, Parent.MaxOptimization);
			}
		}
		...
		{
			GStruct61[] points = RoadSplineGenerator.GeneratePoints(Optimized, true);
			int num = vIndex;
			method_0(points, WidthVals, ref vIndex, ref tIndex, triangles, positions, tangents, uv0, colors);
			...
			if (lodGeneration)
			{
				LastVertId += VerticesDelta;
				VerticesDelta = vIndex;
			}
			else
			{
				LastVertId = num;
		...
		public void method_0(GStruct61[] points, float[] vals, ref int vIndex, ref int tIndex, int[] triangles, Vector3[] positions, Vector4[] tangents, Vector2[] uv0, Color32[] colors)
		{
			...
			for (int i = 0; i < points.Length; i++)
			{
				...
				if (UseCurve)
				{
					...
					vector = gStruct.Right * num7;
					vector2 = gStruct.Position + gStruct.Right * num7 * 0.5f;
					num8 = (1f + num6) * 0.5f * texXDensity;
					num9 = 0f - num6;
				}
				else
				{
					vector = gStruct.Right * Width;
					vector2 = gStruct.Position + gStruct.Right * num;
					num8 = 1f * texXDensity;
					num9 = -1f;
				}
				Vector4 vector3 = gStruct.Right;
				vector3.w = 1f;
				zero.x = num8;
	...
	public void Generate()
	{
		...
		if (Mesh == null)
		{
			MeshFilter meshFilter = meshFilter_0;
			Mesh obj = new Mesh
			{
				vertices = array,
				normals = array2,
				...
				uv = array3,
				colors32 = array4,
				name = "RoadSplineGenerator Generate"
			};
			Mesh mesh = obj;
			Mesh = obj;
	...
	public static void smethod_0(Vector2[] uv, int widthVerts, int count, out int[] starts, out int[] lengths)
	{
		float num = uv[^1].y / (float)count;
	...
	public void GenerateLods(int roadI, Color32[] oldColors)
	{
		...
		if (oldColors == null)
		{
			oldColors = (road.Hold ? method_0(roadI) : null);
		}
		else if (!road.Hold)
		{
			oldColors = null;
		...
		smethod_0(array5, num, road.LodsCount, out var starts, out var lengths);
		for (int j = 0; j < road.LodsCount; j++)
		{
			...
			Mesh sharedMesh = new Mesh
			{
				...
				uv = array11,
				colors32 = array12,
				name = "RoadSplineGenerator GenerateLods"
			};
			string n = "road[" + roadI + "]  lod[" + j + "]";
			if (base.transform.Find(n) == null)
			{
				...
				if (method_3(component2, component.sharedMaterial))
				{
					string materialName = component.gameObject.name + component.sharedMaterial.name + roadI;
					component2.sharedMaterial = GClass934.Get(materialName, component);
				...
				GClass934.Add(component2);
			}
		}
	}

	public Color32[] method_0(int roadI)
	{
		Color32[] array = new Color32[Roads[roadI].GetVerticesCount()];
		int i = 0;
		method_1(roadI, array, ref i);
		return array;
	}

	public void method_1(int roadI, Color32[] colors, ref int i)
	{
		Road road = Roads[roadI];
		int num = road.WidthParts + 1;
		for (int j = 0; j < road.LodsCount; j++)
	...
	public static Vector3[] smethod_1(Vector3[] points, float error, int maxOptimization)
	{
	...
	public static void smethod_2(LinkedList<Vector3> points, float error)
	{
	...
	public static void smethod_3(ref Vector3[] points, ref float[] vals, float error, int maxOptimization)
	{
		...
		while (linkedList.Count < num && num2++ < maxOptimization)
		{
			...
			smethod_4(points, linkedList, error);
			error *= 0.5f;
		}
	...
	public static void smethod_4(Vector3[] points, LinkedList<int> indices, float error)
	{
	...
	{
		return smethod_5(t, p0, n0, n1, p1);
	}

	public static Vector3 smethod_5(float t, Vector3 p0, Vector3 m0, Vector3 m1, Vector3 p1)
	{
		float num = t * t;
		float num2 = num * t;
		float num3 = 3f * num;
	...
	public static GStruct61[] GeneratePoints(Vector3[] positions, bool calcDistances = true)
	{
		int num = positions.Length;
		int num2 = num - 1;
		GStruct61[] array = new GStruct61[num];
	...
	public void method_2()
	{
		...
		foreach (GameObject item2 in linkedList)
		{
			...
			{
				GClass934.Remove(component.sharedMaterial.name, component);
			}
			UnityEngine.Object.DestroyImmediate(item2);
		}
	}

	public bool method_3(MeshRenderer lodMeshRenderer, Material lodMeshMaterial)
	{
		MeshCollider meshCollider = lodMeshRenderer.gameObject.AddComponent<MeshCollider>();
		foreach (Renderer lodRoad in GClass934.LodRoads)
		{
			if (!(lodRoad == null))
			{
