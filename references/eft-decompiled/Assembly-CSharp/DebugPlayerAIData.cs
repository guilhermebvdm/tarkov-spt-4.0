...
public class DebugPlayerAIData : MonoBehaviour
{
	...
	public void UpdateData(ActorDataStruct actorData)
	{
		GStruct12 aIData = actorData.AIData;
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.AppendLine(aIData.Nickname ?? "");
		stringBuilder.AppendLine("Power:" + aIData.PowerOfEquipment.ToString("0.0"));
		stringBuilder.AppendLine($"Place:{aIData.PlaceInfoId}");
		stringBuilder.AppendLine("Shot:" + aIData.shotNameWeapon);
		stringBuilder.AppendLine("Hash:" + aIData.hashName);
		stringBuilder.AppendLine("CurOp:" + aIData.currentOpName);
		IsInTree.SetBool(aIData.IsInTree);
		IsAggressor.SetBool(aIData.IsAggressor);
		if (!aIData.IsAI && actorData.PlayerOwner.iPlayer.Side == EPlayerSide.Savage)
		{
