...
public class GClass162 : GClass157
{
	...
	public class Class227
	{
		...
		{
			if ((gclass162_0.BotOwner_0.Position - point.Position).sqrMagnitude < d)
			{
				p = point;
				v = true;
			}
	...
	[NonSerialized]
	public const float Float_5 = 0.35f;
	...
	[NonSerialized]
	public const float Float_7 = 0.1f;

	[NonSerialized]
	public const float Float_8 = 5f;

	[NonSerialized]
	public const float Float_9 = 10f;

	[NonSerialized]
	public const float Float_10 = 7f;

	[NonSerialized]
	public bool Bool_5;

	[NonSerialized]
	public ETagillaBattleLogic EtagillaBattleLogic_0 = ETagillaBattleLogic.Ambush;
	...
	[NonSerialized]
	public float Float_14 = 999f;

	[NonSerialized]
	public bool Bool_7;

	[NonSerialized]
	public float Float_15 = -1f;
	...
	public GClass162(BotOwner bot, int priority)
		: base(bot, priority)
	{
	...
	public override bool ShallUseNow()
	{
		...
		if (haveEnemy)
		{
			EtagillaBattleLogic_0 = ETagillaBattleLogic.Ambush;
		}
		else if (BotOwner_0.WeaponManager.IsMelee)
		{
		...
		if (haveEnemy)
		{
			num = ((!BotOwner_0.BotFollower.HaveBoss) ? 1 : 0);
			if (num != 0 && BotOwner_0.CallForHelp.WantCallForSavages())
			{
	...
	{
		EtagillaBattleLogic_0 = ETagillaBattleLogic.Default;
		...
		bool anywayAttack;
		int num = method_13(out anywayAttack, resetCache: true);
		if (method_15(num))
		{
		...
		{
			EtagillaBattleLogic_0 = ETagillaBattleLogic.MeleeAssault;
		...
		if (EtagillaBattleLogic_0 == ETagillaBattleLogic.Ambush && !goalEnemy.IsVisible)
		{
			if (BotOwner_0.Memory.IsInCover)
			{
				return method_8(3f, "Amb");
			}
			if (BotOwner_0.Brain.LastDecision.HasValue && (BotOwner_0.Brain.LastDecision == BotLogicDecision.runToCover || BotOwner_0.Brain.LastDecision == BotLogicDecision.goToCoverPoint))
			{
				return new global::AICoreActionResultStruct<BotLogicDecision, GClass26>(BotOwner_0.Brain.LastDecision.Value, "RepeatPrev");
			}
		}
	...
	{
		if (Gclass459_0.WantMeleeAssault() && method_13(out var _, resetCache: true) < 3)
		{
			EtagillaBattleLogic_0 = ETagillaBattleLogic.MeleeAssault;
	...
	public override CustomNavigationPoint FindPoint(CoverSearchData data, Func<CoverSearchData, CustomNavigationPoint> p, bool checkCurrent)
	{
		...
		if (EtagillaBattleLogic_0 == ETagillaBattleLogic.Default)
		{
			if (!Bool_6)
			{
		...
		else
		{
			...
			data = new CoverSearchData((BotOwner_0.Position + vector) / 2f, BotOwner_0.CoverSearchInfo, CoverShootType.shoot, data.MaxDistSqr, 0f, CoverSearchType.distToToCenter, data.Shoot2Point, null, vector, ECheckSHootHide.shootAndHide, new CoverSearchDefenceDataClass(0f));
			data.MinSDistToCarePos = 0f;
			if (BotOwner_0.Brain.LastDecision == BotLogicDecision.attackMoving)
			{
				switch (EtagillaBattleLogic_0)
				{
				case ETagillaBattleLogic.Ambush:
					data.searchLabel = "Retreat";
					break;
				case ETagillaBattleLogic.Default:
					data.searchLabel = "GoToNextCover";
					data.CenterPos = (vector + BotOwner_0.Position) * 0.5f;
					break;
				case ETagillaBattleLogic.MeleeAssault:
	...
	public override AICoreActionEndStruct EndRunToEnemy()
	{
		...
		{
			if (BotOwner_0.Brain.LastDecision.Value == BotLogicDecision.runToEnemy && method_32())
			{
	...
	{
		if (method_13(out var _, resetCache: true) >= 3)
		{
	...
	{
		BotLogicDecision runDecision = ((Float_20 < Time.time) ? BotLogicDecision.runToEnemyZigZag : BotLogicDecision.runToEnemy);
