...
public abstract class GClass810
{
	...
	public static void DebugPoint(Vector3 position, float scale = 1f, float duration = 0f, bool depthTest = true, Color color = default(Color))
	{
		DebugPoint(position, color, scale, duration, depthTest);
	}
	...
	public static void DebugCube(Vector3 pos, Quaternion rot, Vector3 scale, Color c, float duration = 0f)
	{
		...
		Debug.DrawLine(vector, vector2, c, duration);
		Debug.DrawLine(vector2, vector3, c, duration);
		Debug.DrawLine(vector3, vector4, c, duration);
		Debug.DrawLine(vector4, vector, c, duration);
		Debug.DrawLine(vector5, vector6, c, duration);
		Debug.DrawLine(vector6, vector7, c, duration);
		Debug.DrawLine(vector7, vector8, c, duration);
		Debug.DrawLine(vector8, vector5, c, duration);
		Debug.DrawLine(vector, vector5, c, duration);
		Debug.DrawLine(vector2, vector6, c, duration);
		Debug.DrawLine(vector3, vector7, c, duration);
		Debug.DrawLine(vector4, vector8, c, duration);
	}
	...
	public static void DebugWireSphere(Vector3 position, float radius = 1f, Color? color = null, float duration = 0f, bool depthTest = true)
	{
		if (!color.HasValue)
		{
			color = Color.white;
		}
		DebugWireSphere(position, color.Value, radius, duration, depthTest);
	}
	...
	public static void smethod_0(Plane plane, float r, Color color, bool debug)
	{
		Color color2 = Gizmos.color;
		if (!debug)
		{
		...
		float a = Vector3.Dot(plane.normal, Vector3.forward);
		Vector3 vector = ((Mathf.Approximately(a, 1f) || Mathf.Approximately(a, -1f)) ? Vector3.Cross(plane.normal, Vector3.up) : Vector3.Cross(plane.normal, Vector3.forward));
		vector.Normalize();
		Vector3 vector2 = plane.normal * (0f - plane.distance);
		Vector3 vector3 = vector2 + vector * r;
		float angle = 90f;
		for (int i = 0; i < 4; i++)
		{
			Vector3 vector4 = Quaternion.AngleAxis(angle, plane.normal) * vector3;
			if (debug)
			{
		...
		if (debug)
		{
			Debug.DrawLine(vector2, vector2 + plane.normal * r, color);
		}
		else
		{
			Gizmos.DrawLine(vector2, vector2 + plane.normal * r);
		}
		if (!debug)
		{
			Gizmos.color = color2;
	...
	public static void DrawPlane(Plane plane, float r, Color color)
	{
		smethod_0(plane, r, color, debug: false);
	}

	public static void DebugPlane(Plane plane, float r, Color color)
	{
		smethod_0(plane, r, color, debug: true);
	}

	public static void DebugPlane(Vector3 position, Vector3 normal, Color color)
	{
		...
		Debug.DrawLine(vector3, vector5, color);
