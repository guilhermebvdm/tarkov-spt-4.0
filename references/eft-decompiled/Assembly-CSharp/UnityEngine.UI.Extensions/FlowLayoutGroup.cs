...
public class FlowLayoutGroup : LayoutGroup
{
	...
	private readonly IList<RectTransform> ilist_0 = new List<RectTransform>();
	...
	{
		float_0 = SetLayout(base.rectTransform.rect.width, 1, layoutInput: true);
	}
	...
	public float SetLayout(float width, int axis, bool layoutInput)
	{
		...
		for (int i = 0; i < base.rectChildren.Count; i++)
		{
			...
			if (num3 + preferredSize > num)
			{
				...
				if (!layoutInput)
				{
					float yOffset = method_0(height, num2, num4);
					LayoutRow(ilist_0, num3, num4, num, base.padding.left, yOffset, axis);
				}
				ilist_0.Clear();
				num2 += num4;
				num2 += SpacingY;
		...
		if (!layoutInput)
		{
			float yOffset2 = method_0(height, num2, num4);
			num3 -= SpacingX;
			LayoutRow(ilist_0, num3, num4, num - ((ilist_0.Count > 1) ? SpacingX : 0f), base.padding.left, yOffset2, axis);
		}
		ilist_0.Clear();
		num2 += num4;
	...
	public float method_0(float groupHeight, float yOffset, float currentRowHeight)
	{
		if (IsLowerAlign)
		{
		...
		if (IsMiddleAlign)
		{
			return groupHeight * 0.5f - float_0 * 0.5f + yOffset;
		}
		return yOffset;
	}
	...
	public void LayoutRow(IList<RectTransform> contents, float rowWidth, float rowHeight, float maxWidth, float xOffset, float yOffset, int axis)
	{
		...
		{
			num2 = (maxWidth - rowWidth) / (float)ilist_0.Count;
		}
		else if (ExpandHorizontalSpacing)
		{
			num3 = (maxWidth - rowWidth) / (float)(ilist_0.Count - 1);
			if (ilist_0.Count > 1)
			{
				if (IsCenterAlign)
				{
					num -= num3 * 0.5f * (float)(ilist_0.Count - 1);
				}
				else if (IsRightAlign)
				{
					num -= num3 * (float)(ilist_0.Count - 1);
				}
			}
		}
		for (int i = 0; i < ilist_0.Count; i++)
		{
			int index = (IsLowerAlign ? (ilist_0.Count - 1 - i) : i);
			RectTransform rect = ilist_0[index];
