...
{
	public class GClass984
	{
		public Mesh Mesh;

		public Matrix4x4 Matrix;

		public GClass984(Mesh mesh, Matrix4x4 matrix)
		{
			Mesh = mesh;
			Matrix = matrix;
		}
	...
	public class Class667
	{
		...
		public Predicate<GClass984> predicate_0;

		public bool method_0(GClass984 x)
		{
			return x.Mesh == waterForSSRv2_0.WaterPlanes[i].sharedMesh;
		}
	}
	...
	public static event Action<WaterForSSRv2> OnAdd
	{
		...
		add
		{
			Action<WaterForSSRv2> action = action_0;
	...
	public static event Action<WaterForSSRv2> OnRemove
	{
		...
		add
		{
			Action<WaterForSSRv2> action = action_1;
			Action<WaterForSSRv2> action2;
			do
			{
				action2 = action;
	...
	public void Update()
	{
		if (!(material_0 == null))
		{
			...
			material_0.SetFloat(int_19, Mathf.Lerp(AdditionalCubemapReflectionMinWetting, AdditionalCubemapReflectionMaxWetting, t));
		}
	}
	...
	public void method_0()
	{
		...
		int_1 = Shader.PropertyToID("_WaterForSSR_SavedG2");
		int_2 = Shader.PropertyToID("_BumpMap");
		int_3 = Shader.PropertyToID("_BumpMapDetails");
		int_4 = Shader.PropertyToID("_FoamTex");
		int_5 = Shader.PropertyToID("_DepthVals");
		int_6 = Shader.PropertyToID("_BorderFadeDistVals");
		int_7 = Shader.PropertyToID("_Bumpiness");
		int_8 = Shader.PropertyToID("_DepthColorDeep");
		int_9 = Shader.PropertyToID("_DepthColorShallow");
		int_10 = Shader.PropertyToID("_FoamVals");
		int_11 = Shader.PropertyToID("_FoamColor");
		int_12 = Shader.PropertyToID("_ReflectionColor");
		int_13 = Shader.PropertyToID("_DiffuseColor");
		int_14 = Shader.PropertyToID("_ValsNormA");
		int_15 = Shader.PropertyToID("_ValsNormB");
		int_16 = Shader.PropertyToID("_ValsNormD");
		int_17 = Shader.PropertyToID("_ValsFoamA");
		int_18 = Shader.PropertyToID("_ValsFoamB");
		int_19 = Shader.PropertyToID("_AdditionalCubemapReflection");
		int_20 = Shader.PropertyToID("_RippleScale");
		int_21 = Shader.PropertyToID("_RippleBumpness");
		int_22 = Shader.PropertyToID("_FresnelVals");
		int_23 = Shader.PropertyToID("_RippleTex");
	}
	...
	public void method_1(Camera currentCamera)
	{
		if (renderTargetIdentifier_0 == null)
		{
			renderTargetIdentifier_0 = new RenderTargetIdentifier[5]
			{
				...
				Class679.Class679_0.method_1(currentCamera, this)
			};
		}
		else
		{
			...
			renderTargetIdentifier_0[4] = Class679.Class679_0.method_1(currentCamera, this);
		}
	}
	...
	public void RenderBuffer(CommandBuffer buffer, Camera currentCamera)
	{
		...
		buffer.Blit(BuiltinRenderTextureType.GBuffer2, int_1);
		buffer.SetGlobalTexture(int_1, int_1);
	...
	public void method_2()
	{
		...
		if (list_0 == null)
		{
			list_0 = new List<GClass984>();
			for (int i = 0; i < WaterPlanes.Length; i++)
			{
				if (WaterPlanes[i] != null)
				{
					GClass984 item = new GClass984(WaterPlanes[i].sharedMesh, WaterPlanes[i].gameObject.transform.localToWorldMatrix);
					list_0.Add(item);
				}
			}
			return;
		...
		for (j = 0; j < WaterPlanes.Length; j++)
		{
			...
			{
				list_0.Find((GClass984 x) => x.Mesh == WaterPlanes[j].sharedMesh).Matrix = WaterPlanes[j].gameObject.transform.localToWorldMatrix;
			}
		}
	}
	...
	public void method_3()
	{
		...
		material_0.SetTexture(int_2, Normals);
		material_0.SetTexture(int_3, NormalsDetails);
		material_0.SetTexture(int_4, Foam);
		material_0.SetVector(int_5, new Vector4(DepthFade, DepthRefractions, DepthColorFade, BorderFade));
		material_0.SetVector(int_6, new Vector4(BorderFadeDistStart, 1f / BorderFadeDistRange));
		material_0.SetFloat(int_7, Bumpiness);
		material_0.SetColor(int_8, DepthColorDeep);
		material_0.SetColor(int_9, DepthColorShallow);
		material_0.SetVector(int_10, new Vector4(FoamSize, 1f / FoamSize, FoamIntensity));
		material_0.SetColor(int_11, FoamColor);
		material_0.SetColor(int_12, ReflectionColor);
		material_0.SetColor(int_13, DiffuseColor);
		material_0.SetVector(int_14, NormalsA.GetVals());
		material_0.SetVector(int_15, NormalsB.GetVals());
		material_0.SetVector(int_16, NormalsDetails0.GetVals());
		material_0.SetVector(int_17, FoamA.GetVals());
		material_0.SetVector(int_18, FoamB.GetVals());
		method_4();
		material_0.SetFloat(int_20, RippleScale);
		material_0.SetFloat(int_21, RippleBumpness);
		material_0.SetVector(int_22, new Vector4(FresnelIntensity, FresnelPower));
		material_0.SetTexture(int_23, RippleTexture);
	}
	...
	public void method_4()
	{
		...
		{
			float_1 = float_0;
			material_0.SetFloat(int_19, Mathf.Lerp(AdditionalCubemapReflectionMinWetting, AdditionalCubemapReflectionMaxWetting, float_0));
		}
	}
}
