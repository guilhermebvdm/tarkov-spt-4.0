...
public class GClass499 : LookData
{
	public GClass499(BotOwner owner)
		: base(owner)
	{
	}

	public override bool IsLookPointExistAndValid(CustomNavigationPoint point)
	{
		return false;
	}

	public override void SetLookPointByHearing(CustomNavigationPoint closestPoint = null)
	{
		...
		bool flag = true;
		NextCheckTimeSetLookHearing = Time.time + BotOwner_0.Settings.FileSettings.Look.WAIT_NEW_SENSOR * (Random.value + 0.2f);
		Vector3? vector = null;
		float num = -1f;
		...
		if (!vector.HasValue)
		{
			...
			{
				int lastSideSign = LastSideSign;
				if (Time.time > NextCheckTimeSetLookAnotherSide && closestPoint != null)
				{
					float num2 = BotOwner_0.Settings.FileSettings.Look.WAIT_NEW__LOOK_SENSOR * (Random.value + 0.5f);
					NextCheckTimeSetLookAnotherSide = Time.time + num2;
					if (lastSideSign > 0)
					{
						lastSideSign = -1;
					}
		...
		if (vector.HasValue && vector.Value.sqrMagnitude > 0.001f)
		{
			...
			if (flag && customNavigationPoint != null && flag4 && (BotOwner_0.Transform.position - customNavigationPoint.Position).sqrMagnitude < LocalBotSettingsProviderClass.Core.CLOSE_TO_WALL_ROTATE_BY_WALL_SQRT && (customNavigationPoint.CoverLevel == CoverLevel.Stay || customNavigationPoint.StrategyType == PointWithNeighborType.both || customNavigationPoint.StrategyType == PointWithNeighborType.ambush))
			{
				...
				{
					float num3 = Vector3.Angle(customNavigationPoint.LeftBorderLight, vector2);
					float num4 = Vector3.Angle(customNavigationPoint.RightBorderLight, vector2);
					bool flag5 = num > 0f && num < LocalBotSettingsProviderClass.Core.LOOK_ANYSIDE_BY_WALL_SEC_OF_ENEMY;
					vector = ((num3 < num4 && (customNavigationPoint.CanLookLeft || flag5)) ? new Vector3?(RotateWallBySide(customNavigationPoint, 1)) : ((!(customNavigationPoint.CanLookRight || flag5)) ? new Vector3?(GClass855.RotateOnAngUp(-customNavigationPoint.ToWallVector, GClass856.Random(-10f, 10f))) : new Vector3?(RotateWallBySide(customNavigationPoint, -1))));
				}
			}
