...
public class RoadMarksGenerator : MonoBehaviour
{
	...
	public void method_0()
	{
		...
		Vector3[] array = smethod_0(Parent.Roads[Road], Shift, MarkLength, MarkDist, Start, End);
		...
		meshFilter_0.sharedMesh = method_2(array, meshFilter_0.sharedMesh);
	}

	public void method_1(Vector3[] marksPos, int parts, float cullingRate)
	{
		Vector3[][] array = method_3(marksPos, parts);
		...
		for (int i = 0; i < parts; i++)
		{
			...
			transform.GetComponent<MeshFilter>().sharedMesh = method_2(array[i], mesh);
	...
	public Mesh method_2(Vector3[] marksPos, Mesh mesh = null)
	{
		...
		method_4(marksPos, array2, array, normals, tangents, uv, UvRect, Width, colors, array3, bool_0);
		if (mesh == null)
		{
			mesh = new Mesh
			{
				...
				triangles = array2,
				uv = uv,
				name = "RoadMarksGenerator GetMesh"
			};
		}
		else
		{
	...
	public Vector3[][] method_3(Vector3[] marksPos, int parts)
	{
	...
	public void method_4(Vector3[] marksPos, int[] tris, Vector3[] verts, Vector3[] normals, Vector4[] tangents, Vector2[] uv, Rect uvRect, float width, Color[] colors, Color[] oldColors, bool copyColorsInReverseOrder)
	{
		width *= 0.5f;
	...
	public static Vector3[] smethod_0(RoadSplineGenerator.Road road, float shift, float markLength, float markDist, float start, float end)
	{
