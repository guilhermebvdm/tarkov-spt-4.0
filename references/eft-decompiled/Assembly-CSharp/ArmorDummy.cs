...
public class ArmorDummy : MonoBehaviour
{
	...
	public void CooliderOnOnHitAction(DamageInfoStruct damageInfo)
	{
		float damage = damageInfo.Damage;
		float penetrationPower = damageInfo.PenetrationPower;
		float num = -1f;
		float num2 = Durability / (float)MaxDurability * 100f;
		float num3 = Singleton<BackendConfigSettingsClass>.Instance.Armor.GetArmorClass(ArmorClass).Resistance;
		...
		Health -= damage;
		PopupText.text = $"<color=#913>–{(int)damage}</color> <color=#778>–{(int)num}</color> {(int)num5}%";
		if (Health <= 0f)
		{
			Renderer[] body = Body;
