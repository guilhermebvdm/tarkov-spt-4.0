...
public class AmplifyMotionCamera : MonoBehaviour
{
	...
	{
		AmplifyMotionEffectBase.smethod_0(this);
	}
	...
	public void OnDisable()
	{
		m_initialized = false;
		method_2();
		AmplifyMotionEffectBase.smethod_1(this);
	}
	...
	public void FixedUpdateTransform(AmplifyMotionEffectBase inst, CommandBuffer updateCB)
	{
		...
		{
			if (m_affectedObjects[i].Boolean_0)
			{
				m_affectedObjects[i].method_5(inst, m_camera, updateCB, m_starting);
			}
		}
	}
	...
	public void RenderReprojectionVectors(RenderTexture destination, float scale)
	{
		...
		m_renderCB.Blit(new RenderTargetIdentifier(tex), destination, Instance.Material_0);
	}
	...
	public void RenderVectors(float scale, float fixedScale, Quality quality)
	{
		...
		for (int i = 0; i < m_affectedObjects.Length; i++)
		{
			...
			{
				m_affectedObjects[i].method_6(m_camera, m_renderCB, m_affectedObjects[i].Boolean_0 ? fixedScale : scale, quality);
			}
		}
	}
}
