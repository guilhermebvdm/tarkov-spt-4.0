...
public class IKSolverTrigonometric : IKSolver
{
	...
	public class TrigonometricBone : Bone
	{
		...
		public void Initiate(Vector3 childPosition, Vector3 bendNormal)
		{
			...
			TargetToLocalSpace = GClass1463.RotationToLocalSpace(transform.rotation, rotation);
	...
	[NonSerialized]
	public Vector3 WeightIKPosition;

	[NonSerialized]
	public bool DirectHierarchy = true;
	...
	public void SetBendGoalPosition(Vector3 goalPosition, Vector3 ikPos)
	{
		if (base.initiated)
		{
			Vector3 vector = Vector3.Cross(goalPosition - bone1.transform.position, ikPos - bone1.transform.position);
			if (vector != Vector3.zero)
			{
				bendNormal = vector;
			}
	...
	public static void Solve(Transform bone1, Transform bone2, Transform bone3, Vector3 targetPosition, Vector3 bendNormal, float weight)
	{
		...
		if (magnitude != 0f)
		{
			...
			Vector3 toDirection = smethod_0(vector, magnitude, bendDirection, sqrMagnitude, sqrMagnitude2);
			bone1.rotation = Quaternion.FromToRotation(bone2.position - bone1.position, toDirection) * bone1.rotation;
	...
	public static void SolveVirtual(Bone[] bones, int first, int second, int third, Vector3 targetPosition, Vector3 bendNormal, float weight)
	{
		...
		if (sqrMagnitude != 0f)
		{
			...
			Vector3 bendDirection = Vector3.Cross(vector, bendNormal);
			Vector3 toDirection = smethod_0(vector, directionMag, bendDirection, sqrMagnitude2, sqrMagnitude3);
			Quaternion rotation = Quaternion.FromToRotation(bones[second].solverPosition - bones[first].solverPosition, toDirection);
			IKSolver.SolverRotateBonesAroundPoint(bones, first, bones[first].solverPosition, rotation);
			Quaternion rotation2 = Quaternion.FromToRotation(bones[third].solverPosition - bones[second].solverPosition, targetPosition - bones[second].solverPosition);
			IKSolver.SolverRotateBonesAroundPoint(bones, second, bones[second].solverPosition, rotation2);
		}
	}
	...
	public static void SolveVirtualPositions(Bone bone1, Bone bone2, Bone bone3, Vector3 targetPosition, Vector3 bendNormal, float weight)
	{
		...
		Vector3 vector = targetPosition - bone1.solverPosition;
		float sqrMagnitude = vector.sqrMagnitude;
		if (sqrMagnitude != 0f)
		{
			float directionMag = Mathf.Sqrt(sqrMagnitude);
			float sqrMagnitude2 = (bone2.solverPosition - bone1.solverPosition).sqrMagnitude;
			float sqrMagnitude3 = (bone3.solverPosition - bone2.solverPosition).sqrMagnitude;
			Vector3 bendDirection = Vector3.Cross(vector, bendNormal);
			Vector3 vector2 = smethod_0(vector, directionMag, bendDirection, sqrMagnitude2, sqrMagnitude3);
			bone2.solverPosition = bone1.solverPosition + vector2.normalized * Mathf.Sqrt(sqrMagnitude2);
			bone3.solverPosition = ((sqrMagnitude < sqrMagnitude2 + sqrMagnitude3) ? targetPosition : (bone2.solverPosition + (targetPosition - bone2.solverPosition).normalized * Mathf.Sqrt(sqrMagnitude3)));
		}
	}

	public static Vector3 smethod_0(Vector3 direction, float directionMag, Vector3 bendDirection, float sqrMag1, float sqrMag2)
	{
	...
	public virtual void OnInitiateVirtual()
	{
	}
	...
	public virtual void OnPostSolveVirtual()
	{
	}

	public Vector3 GetBendDirection(Vector3 IKPosition, Vector3 bendNormal)
	{
