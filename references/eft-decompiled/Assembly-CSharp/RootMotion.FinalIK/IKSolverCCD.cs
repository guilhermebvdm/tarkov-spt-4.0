...
public class IKSolverCCD : IKSolverHeuristic
{
	...
	public override void OnUpdate()
	{
		...
		for (int i = 0; i < maxIterations && (!(vector == Vector3.zero) || i < 1 || !(tolerance > 0f) || !(base.positionOffset < tolerance * tolerance)); i++)
		{
			...
			method_1(IKPosition + ((i == 0) ? vector : Vector3.zero));
		}
		LastLocalDirection = localDirection;
	}

	public void method_1(Vector3 targetPosition)
	{
		if (XY)
		{
	...
	public static void SolveVirtual(Bone[] bones, Vector3 targetPosition, float weight)
	{
		for (int num = bones.Length - 2; num > -1; num--)
		{
			float num2 = bones[num].weight * weight;
			if (num2 > 0f)
			{
				...
				if (num2 < 1f)
				{
					quaternion = Quaternion.Lerp(Quaternion.identity, quaternion, num2);
				}
				IKSolver.SolverRotateBonesAroundPoint(bones, num, bones[num].solverPosition, quaternion);
			}
		}
	}
}
