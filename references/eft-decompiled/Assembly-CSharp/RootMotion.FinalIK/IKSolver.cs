...
public abstract class IKSolver
{
	...
	public delegate void GDelegate47();

	public delegate void GDelegate48(int i);
	...
	public GDelegate47 OnPreInitiate;

	public GDelegate47 OnPostInitiate;

	public GDelegate47 OnPreUpdate;

	public GDelegate47 OnPostUpdate;
	...
	{
		GClass1465.Log(message, root, logInEditMode: true);
	}
	...
	public static bool HierarchyIsValid(Bone[] bones)
	{
		...
		while (true)
		{
			if (num < bones.Length)
			{
				if (!GClass1461.IsAncestor(bones[num].transform, bones[num - 1].transform))
				{
					break;
				}
				num++;
	...
	public static float PreSolveBones(ref Bone[] bones)
	{
		float num = 0f;
		for (int i = 0; i < bones.Length; i++)
		{
	...
	{
		Quaternion rotation2 = GClass1463.FromToRotation(bones[index].solverRotation, rotation);
		SolverRotateBonesAroundPoint(bones, index, bones[index].solverPosition, rotation2);
	}
