...
public class IKSolverFullBody : IKSolver
{
	...
	public GDelegate47 OnPreRead;

	public GDelegate47 OnPreSolve;

	public GDelegate48 OnPreIteration;

	public GDelegate48 OnPostIteration;

	public GDelegate47 OnPreBend;

	public GDelegate47 OnPostSolve;

	public GDelegate47 OnStoreDefaultLocalState;

	public GDelegate47 OnFixTransforms;
	...
	public IKEffector GetEffector(Transform t)
	{
		...
		while (true)
		{
			if (num < effectors.Length)
			{
				if (effectors[num].bone == t)
				{
					break;
				}
				num++;
	...
	public override bool IsValid(ref string message)
	{
		...
		while (true)
		{
			if (num < chain.Length)
			{
				if (!chain[num].IsValid(ref message))
				{
					break;
				}
				num++;
			...
			while (true)
			{
				...
				{
					if (!array[num2].IsValid(this, ref message))
					{
						break;
					}
					num2++;
				...
				while (true)
				{
					...
					{
						if (!array2[num2].IsValid(this, ref message))
						{
							break;
						}
						num2++;
					...
					while (true)
					{
						...
						{
							if (!array3[num2].IsValid(this, ref message))
							{
								break;
							}
							num2++;
	...
	{
		for (int i = (Quick ? 2 : 0); i < chain.Length; i++)
		{
	...
	public virtual void WritePose()
	{
		...
		for (int j = (Quick ? 2 : 0); j < limbMappings.Length; j++)
		{
