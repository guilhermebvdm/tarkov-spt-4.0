#!/bin/bash
# Pre-commit gate: warn when a staged */memory/sessions.md has a bullet under the
# top "Pendências" block WITHOUT a [P-N.M] ID. The IDs are the cross-ref + GC
# primitive (memory-curation §7); a bare bullet can never enter the date-keyed GC.
# WARN-ONLY (exit 0) so legacy migration can be staged incrementally.
#
# Findings D1-01/D5-04 from the harness value review.
# memory/repo-sessions.md (repo-wide) was discontinued 2026-07-06 — mod-level only now.
...
STAGED=$(git diff --cached --name-only --diff-filter=ACM \
  | grep -E '/memory/sessions\.md$' || true)
[ -z "$STAGED" ] && exit 0
