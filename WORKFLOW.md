...
| [/optimize-mod-performance](.claude/commands/optimize-mod-performance.md) | mod com código | achados anotados + item de backlog `perf-*` | **auxiliar** (fora do ciclo linear) — pipeline de engenharia de performance em 4 fases com portões: investigação (via `/audit-mod-code --perf`) → plano técnico (achados aceitos viram **um** item de backlog com spec de **não-regressão** + metas medíveis) → implementação (ciclo normal `/code-mod`→`/code-review`→`/compile-mod`, refs `// ref: AUD-NN-MM`) → validação **medida** antes/depois (AP-06) |
