...
4. **Implementar seguindo o checklist da spec técnica**, em ordem:
   ...
   - Comentar referências ao Assembly inline com `// ref: Assembly-CSharp/<arquivo>:<linha>` quando fizer sentido para manutenção. Para tipo ofuscado hardcoded (`typeof(GClassNNNN)`/`AccessTools`), anexar o conceito da tabela de deofuscação (`GClass680 = ABotProfileCreator`) — semeia a migração 4.1 no ponto exato do rename. **Nunca inline o FQN 4.1 como se fosse pinado** (drift/AP-09): o comentário aponta o conceito; a fonte viva é `docs/files-from-4.1/consolidated-mappings.txt`. **Membro ofuscado por string** (`AccessTools.Field/Method("method_5"/"_player"/"float_3")`) **não** está no mapa (só cobre tipos) nem é pego pelo compilador — é a superfície de quebra silenciosa no 4.1. Manter o fallback defensivo por lista de candidatos (`ResolveBackingFieldByCandidates`) e não removê-lo alegando nome estável.
