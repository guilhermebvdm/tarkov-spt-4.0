...
| 2026-06-13 | **Glow do menu RESOLVIDO + gradiente do ícone refeito (via instrumentação).** Glow: o `TopGlowPve` usa SPRITE azul/ciano (`Top_glow_PVE`); Unity UI faz `sprite × color`, então tint laranja × azul = **verde** (confirmado por diag em runtime). Fix `FixTopGlow`: troca o sprite do PvE pelo neutro do `TopGlowRegular` (`glow_top_regular`) + tint da classe → laranja. Ícone: o `BaseMeshEffect` (`ClassIconGradient`) **não renderiza** em Image criada em runtime (menu) — substituído por **gradiente embutido na textura** (`ClassIconCache.GetTinted`: silhueta branca × degradê vertical no load; clareamento 0.35, mais forte que os nomes pois o ícone é pequeno). Diagnóstico temporário removido. Client 0/0, 44.5 KB. |
| 2026-06-12 | **2 fixes do menu (via grafo do MO + leitura).** (1) **Glow não acompanhava a cor da classe** (AccentColor laranja, glow verde): corrida de timing — o MO recolore em `MenuOverhaulPatch.UpdateLayoutElements()` após `await LoadPatchContent`, e nosso `SetAccent` disparava o `SettingChanged` com `FindEnvironmentObjects()==null` (no-op). Fix: `MenuOverhaulBridge.ReapplyLayout()` chama `UpdateLayoutElements()` (public static, via reflection cacheada) após o `SetAccent` na coroutine (ambiente já pronto = nick existe) → recolore determinístico. (2) **Ícone do menu sem o tom da classe**: `ApplyIconGradient` setava `icon.color=white` e confiava só no `BaseMeshEffect`; se o ModifyMesh não renderizava (Image criada em runtime, filha de TMP), ficava branco. Fix: fallback `icon.color=baseColor` (tom certo garantido) + `SetAllDirty()`. Client compilado isolado (0/0, 43 KB) p/ não acoplar ao server da sessão do editor. A validar in-game. |
