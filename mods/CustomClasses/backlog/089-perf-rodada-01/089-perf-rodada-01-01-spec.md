...
- [ ] **`AUD-01-08` (corrigido pelo PA-03-04):** arrastando o picker de cor de uma classe por ~5 s, `tintedCache` **para de crescer** e estabiliza em **≤ 4 entradas por ícone distinto em uso** (o cap `MaxVariantsPerIcon` da spec técnica). Numa aba CLASS aberta o valor típico é **2** — o brasão com gradiente e a marca d'água chapada são duas variantes legítimas do mesmo ícone, vivas ao mesmo tempo — podendo tocar 4 durante o arrasto antes da eviction. **O que reprova é crescimento monotônico:** qualquer número que continue subindo enquanto se arrasta. ⚠️ A redação anterior pedia "≤1 por classe visível" e teria **reprovado um resultado correto** — o critério é "para de crescer", não um valor absoluto.
