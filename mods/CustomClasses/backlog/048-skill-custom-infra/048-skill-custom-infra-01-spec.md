...
- Entrega **uma skill de exemplo** funcional ponta a ponta (prova do mecanismo) + a skill aparece no **viewer do editor** (convenção do 047: skills custom entram no catálogo canônico do editor, não no dump removido).
...
- [ ] **Fika/multiplayer:** `N/A` — XP e efeito são **por-jogador/local** (mesmo escopo do mecanismo de multiplicador do item 005); a skill custom de um player não afeta as skills/efeito de outro. *(Confirmar no smoke do 052 com 2+ players.)*
