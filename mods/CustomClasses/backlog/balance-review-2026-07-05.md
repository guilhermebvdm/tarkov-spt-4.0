...
| B12 | 🟡 | Médico | Implementar os perks (050 — ~~perna transpiler~~: Rapid Care/Swift Surgeon) → **ENTREGUE no 072 (2026-07-13), e SEM transpiler**. A "perna" era um **falso-negativo**: o decompile do repo tem 102 namespaces vazios (`EFT.HealthSystem` inclusive), então o `UseTimeFor` — que é **público e existe** — parecia não existir. Rapid Care ×0.7 · Swift Surgeon ×0.5 (efeito e animação casados por um escopo armado no `method_5`) · **Mobile Surgery** também entregue (era dado como "não localizável"; é a flag `EPhysicalCondition.HealingLegs`) | item 050 | — | ✅ | ✅ 2026-07-13 |
