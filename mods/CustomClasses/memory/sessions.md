...
**Sessão 15 (2026-07-13, autônoma `/g-autodev`):** os **perks 072** que o 050 dera como impossíveis foram ENTREGUES — Calm Sights (Caçador, sway ×0.7), Rapid Care + Swift Surgeon + Mobile Surgery (Médico) — **nenhum precisou de transpiler**; a premissa "impossível" era falso-negativo do decompile do repo com 102 namespaces VAZIOS. **Sessão 16 (2026-07-15):** o usuário validou movimento in-game e achou **2 bugs do Tank**, ambos corrigidos: velocidade DECAINDO a cada passo (patches gravavam em campos com feedback → getters stateless, v0.2.2) e stamina drenando ao ANDAR com pouco peso (Pack Mule não aplicado no `Physical.Init` pré-MainPlayer → recálculo no `OnGameStarted`, v0.2.4). Rooted/Execution ficaram inertes pelo lever (item 074). **Versão v0.2.4. 26 commits locais em `main` — NADA pushado.**
...
- 🔴 [P-16.1] (aberta 2026-07-15) **Validar in-game os 2 fixes de movimento (v0.2.4) + perks 072**: velocidade constante ao andar; stamina não drena ao andar com ~48–55 kg (diag "Walk overweight limit" ≈58.5, não ~45); Mobile Surgery = cirurgia + apertar W (risco de lock de Animator). Em **cliente Fika**, não só solo. ⚠️ launcher reverte a DLL no sync (`feedback_server_launcher_sync_builds`) — conferir build v0.2.4 no jogo.
- 🔴 [P-10.1] (aberta 2026-06-23, avança 2026-07-15) **Validação in-game 050.0–050.4** (~21 efeitos) + 047 — gates de runtime que o compile não pega. **Progresso Sessão 16:** movimento validado (2 bugs achados+corrigidos). Regra `feedback_spt_validation`. (Supersede P-8.1; P-16.1 é o subset dos fixes novos.)
- 🟡 [P-16.2] (aberta 2026-07-15) **Rooted (−15% ADS) e Execution (+10% melee) inertes** pelo lever de velocidade — item **074**. Rooted precisa de `AimMovementSpeed`/`StateSpeedLimit` de mira; Execution precisa levantar o cap do `SpeedLimiter`/`GClass2175` (obfuscado, frágil).
...
### 2026-07-15 22:22 (GMT-3) — Sessão 16: 2 bugs de movimento do Tank (in-game) + code-review dos perks 072

> A entrega dos **perks 072** (Calm Sights / Rapid Care / Swift Surgeon / Mobile Surgery) está na entrada **2026-07-13** acima (gravada sem número na sessão autônoma `/g-autodev` — é a "Sessão 15" de facto). Esta sessão é o pós-validação in-game dela + 2 bugs novos.

**Tema central:** o usuário testou in-game e reportou 2 bugs do Tank ligados a movimento; ambos diagnosticados por decompile e corrigidos, cada um com code-review adversarial limpo. Versões v0.2.2 → v0.2.4.

**Decisões-chave:**
- **Bug de VELOCIDADE decaindo (v0.2.2, `05fafd11`)**: os patches de −vel gravavam ×0.9 em CAMPOS que o EFT relê+regrava por frame (`CharacterMovementSpeed` via `SetCharacterMovementSpeed(RelativeSpeed×MaxSpeed)`=`SetCMS(CMS)`, MovementContext.cs:4181; `SprintSpeed_1` via `SprintAcceleration`, :2550) → ×0.9 COMPUNHA geometricamente até quase parar. Fix: reduzir só nos getters SEM ESTADO (`MaxSpeed`=>Evaluate :910, `SprintingSpeed`=>Evaluate :912). Removidos `SetCharacterMovementSpeedPatch`/`SprintAccelerationPatch`; ativados `MaxSpeedPatch`/`SprintingSpeedPatch` (ClassMovementPatches.cs). Reverte o "fix" de 2026-06-24 que diagnosticou ao contrário (achou os getters "só teto").
- **Bug de STAMINA do Tank drenando ao andar (v0.2.4, `f35b2768`)**: TIMING. `BasePhysicalClass.UpdateWeightLimits()` roda em `Physical.Init()` (Player.cs:28642) ANTES de `RegisterPlayer` setar `MainPlayer` (:28697) → gate do `PackMulePatch` falha (gw≠null, MainPlayer==null) → piso +30% não entra → `WalkOverweightLimits` cacheado vanilla → Tank drena ao andar em ~45 kg em vez de ~58.5. Fix: no Postfix de `GameWorld.OnGameStarted` (`RaidPerksNotificationPatch`, após o Prefetch resolver a classe e MainPlayer já setado), forçar `MainPlayer.Physical.OnWeightLimitsUpdated()`. + linha de diag "Walk overweight limit (kg)" em `PerkDiagnostics` p/ validar (esperado ~58.5).
- **Rooted/Execution ficaram INERTES** pelo lever novo (não é regressão — já estavam quebrados no decaimento): Rooted (−15% ADS) é limitado pelo teto de mira (`ClampSpeed` ~0.33–0.50 < MaxSpeed reduzido ~0.61); Execution (+10% melee) é comido pelo cap absoluto do `SpeedLimiter` (`GClass2175`, Run 4.6 m/s, não lê MaxSpeed). Documentado em `ClassMovementPatches.cs` + item **074**. Insight: o getter MaxSpeed é lever só pra BAIXO e só acima do teto de mira.

**Lições / hipóteses descartadas:**
- **Anti-pattern "campo com feedback"**: multiplicar valor em campo que o EFT relê+regrava por frame COMPÕE geometricamente. Usar getter stateless. → memória global `reference_spt_init_before_mainplayer` cobre o irmão (timing); candidato a `spt-antipatterns.md`.
- **Anti-pattern "patch gateado em MainPlayer não aplica no Init"**: valor cacheado no `Physical.Init` nasce vanilla (MainPlayer ainda null) → reforçar em `OnGameStarted`. → memória global `reference_spt_init_before_mainplayer`.
- **Descompilar a DLL por HÁBITO foi erro** (o usuário apontou): repo+grafos cobrem a maioria (`Player`/`GameWorld`/`MovementContext`/`BasePhysicalClass`/`SkillManager` COMPLETOS); a DLL real só p/ os 3 namespaces vazios (health/animation/inventory). → `reference_eft_decompile_incomplete` atualizada (DLL é exceção estreita, não default).
- **Diagnóstico anterior invertido**: o "fix" de 2026-06-24 concluiu que os getters `MaxSpeed`/`SprintSpeed` eram "só teto, não movem o boneco" e migrou pros drivers — exatamente os campos com feedback. A velocidade real É `RelativeSpeed×MaxSpeed`, então o getter reduz de verdade (e de forma estável).

**Atividade cronológica:**
1. Diagnóstico do bug de velocidade (decompile de `MovementContext`) → fix nos getters stateless → v0.2.2 → code-review adversarial (confirmou fix, achou Rooted/Execution inertes = F1/F2).
2. Backlog **073** (rename perk/drawback→buff/debuff, só texto visível, 2 línguas) e **074** (Rooted/Execution levers errados) criados.
3. Diagnóstico do bug de stamina (sub-agente, decompile de `BasePhysicalClass`/`Player`) → causa = timing pré-MainPlayer → fix em `OnGameStarted` → v0.2.4 → code-review adversarial LIMPO (0 bloqueadores, 8 hipóteses derrubadas).
4. Handoff `.handoffs/handoff-2026-07-15-customclasses-perks-bugfixes.md` (`ca53f941`). 26 commits locais, NADA pushado.

**Pendências abertas nesta sessão:**
- 🔴 [P-16.1] (aberta 2026-07-15) **Validar in-game os 2 fixes de movimento** (velocidade constante; stamina não drena ao andar ~48–55 kg; diag "Walk overweight limit" ≈58.5) + os perks 072 (esp. Mobile Surgery: cirurgia + W — risco de lock de Animator não inspecionável). Testar em **cliente Fika**, não só solo. ⚠️ launcher reverte a DLL no sync — conferir build v0.2.4 no jogo.
- 🟡 [P-16.2] (aberta 2026-07-15) **Rooted e Execution inertes** — item 074. Levers certos: `AimMovementSpeed`/`StateSpeedLimit` (Rooted); cap do `SpeedLimiter`/`GClass2175` (Execution, frágil).

**Cross-refs:**
- ✅ Resolve **[P-10.2]** (deferrals do 050): Combat Medic (Rapid Care/Swift Surgeon/Mobile Surgery) e Iron Lungs sway (→ Calm Sights do Caçador) ENTREGUES no 072 (entrada 2026-07-13); Quick Hands já era (061). A premissa "precisa transpiler / não localizável" era falso-negativo do decompile incompleto.
- Avança **[P-10.1]/[P-13.1]** (validação in-game): usuário validou movimento e achou 2 bugs, ambos corrigidos.
- 3 candidatos a promoção p/ `spt-antipatterns.md` (ver relatório) — vivem hoje na memória global do assistente.
...
**Teste in-vivo (P-14.3 ✅ RESOLVIDA):** parei/reiniciei o `SPT.Server.exe` (⚠️ ele EXIGE console interativo — falha com `Unable to get console mode` se stdout for redirecionado; usar `Start-Process ... -WindowStyle Minimized` e ler o log file `user/logs/spt/spt<data>.log`, não capturar stdout). Boot com o fix de ordem de load: `Could not add to cache` **dezenas→0**, `1x1` **→0**, 7 classes OK. Restaram 2 resíduos: (a) 4× `não foi encontrado no cache` do `ItemBaseClassService` (SPT CORE, não CustomClasses, base-class de itens de mod) — separado; (b) 2× `contêiner sem grades` no Tanque.
...
**Board:** B1–B18 todos ✅ (status → 🟢). Sobraram 2 debuffs grátis fora do escopo do board (`Shadowconnections` no Fuzileiro e no Tanque) — candidatos a um "B20".

#### B14 / item 065 — COOP: perks de som passam a valer para peers Fika (2026-07-11)

**O problema (o mais impactante do mod em coop):** os **BOTS vivem no processo do HOST** — é o host quem calcula o que a IA ouve, inclusive do barulho de um peer. Os patches de som gateavam no player **LOCAL** (`IsYourPlayer` / `ProfileId == MainPlayer`), então **todo perk sonoro de quem joga como CLIENTE era PLACEBO contra a IA**: Ghost Step (Furtivo −30%), Stalker (Caçador −20%), Loud Operator (Fuzileiro/Tanque +30%) e Silent Looter (Saqueador). Só funcionavam para quem hospedava.

**Fix (sem protocolo novo — a infra já existia):** o host resolve a classe de **QUEM EMITIU** o som e aplica o multiplicador **dela**.
- **`ClassIdentities.ClassNameEnOf(Player)`** (novo): local → `SkillMultipliers.ClassNameEn`; peer → mapa nickname→classe da **rota 057** (`TryResolve`). Vocabulário confere: os dois lados usam o campo `classNameEn` (a chave EN estável — o server documenta isso explicitamente).
- **`SkillMultipliers.IsClass(a, b)`** (novo): compara duas classes sem assumir que uma é a local; `IsLocalClass` delega p/ ele.
- Helpers `QuietStep`/`LoudOperator`/`SilentLooter` ganharam **`MultFor(classe)`**; `Mult()` virou atalho para a classe local.
- **`AiSoundPatch`** (`BotEventHandler.PlaySound`) e **`SainSoundPatch`** (SAIN, resolve o player via `GameWorld.GetAlivePlayerByProfileID` — ref: GameWorld.cs:1238) passaram a usar a classe do emissor.
- **`SoundRadiusPatch`** (`method_67`) segue LOCAL de propósito: é o rolloff que VOCÊ ouve; sincronizá-lo para peers humanos exigiria protocolo real → **fora de escopo** (o board já dizia isso).

**⚠️ Armadilha PEGA na implementação (bug real, não teórico):** **bots TAMBÉM emitem passo** por esse pipeline — `MovementContext.cs:1629` chama `PlaySound(_player, …, AISoundType.step)` e bots têm MovementContext. O gate `person is not Player` NÃO os barra. E o EFT **gera nome de bot a partir de uma lista de nicknames REAIS** → um bot cujo nickname colidisse com o de um jogador do mapa herdaria a classe dele e teria o próprio som alterado. **Gate `player.IsAI`** (ref: Player.cs:25135) adicionado no `ClassNameEnOf` — ponto central, protege todos os call-sites.

**⚠️ Hitch evitado:** `ClassIdentities.EnsureLoaded()` faz **GET SÍNCRONO**. Como agora ele pode ser alcançado de dentro do `PlaySound` (a cada passo!), adicionei `ClassIdentities.Prefetch()` no `GameWorld.OnGameStarted` — **mesmo padrão e mesma razão do warm de `SkillMultipliers` que já existia ali** (CR-051-01: HTTP síncrono no 1º ADS). O prefetch do deploy não é garantido (solo / painel de grupo não renderizado).

**⚠️ Limite conhecido:** o VALOR do multiplicador sai do **F12 do HOST** (ele é a autoridade da percepção da IA, que é dele). Não há sync de config entre peers — aceitável e previsto no board.
