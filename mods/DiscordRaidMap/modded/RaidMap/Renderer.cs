...
{
    internal readonly struct RenderedImage
    {
        public RenderedImage(byte[] data, string fileName, string contentType)
        {
            Data = data;
            FileName = fileName;
    ...
    internal sealed class Renderer : IDisposable
    {
        ...
        private readonly MapImageFormat _imageFormat;
        ...
        // Reused across renders (renders are serialized by the broadcaster's lock).
        ...
        public Renderer(string mapAssetPath, string markerAssetPath, string fontAssetPath, string mapTextFont, int mapTextFontSize, int markerDisplaySize, int maxImageDimension, MapImageFormat imageFormat, int jpegQuality)
        {
            ...
            _maxImageDimension = Math.Max(1, maxImageDimension);
            _imageFormat = imageFormat;
            _jpegQuality = Mathf.Clamp(jpegQuality, 1, 100);
        }

        private string FileName => _imageFormat == MapImageFormat.Png ? "raid-map.png" : "raid-map.jpg";
        private string ContentType => _imageFormat == MapImageFormat.Png ? "image/png" : "image/jpeg";

        public RenderedImage Render(RaidSnapshot snapshot)
        {
            var background = LoadBackground(snapshot.Map);
            var width = background.Width;
            var height = background.Height;
            var pixels = GetCanvas(background);
            ...
            return new RenderedImage(EncodeImage(pixels, width, height), FileName, ContentType);
        }
        ...
        private Color32[] GetCanvas(CpuImage source)
        {
            var length = source.Pixels.Length;
            if (_canvas == null || _canvas.Length != length)
            {
                _canvas = new Color32[length];
            }

            Array.Copy(source.Pixels, _canvas, length);
            return _canvas;
        }
        ...
        private CpuImage LoadMarker(RaidMarkerType markerType)
        {
            ...
            // Markers are already small — load at native size, do not clamp to the map's max dimension.
        ...
        {
            var graphics = GetMeasureGraphics();
            var font = GetFont();
        ...
        private void DrawTrueTypeText(Color32[] pixels, int width, int height, string text, int x, int y, Color32 color)
        {
            ...
            var font = GetFont();
            var measureGraphics = GetMeasureGraphics();
        ...
        private System.Drawing.Graphics GetMeasureGraphics()
        {
            ...
            _measureBitmap = new Bitmap(1, 1, PixelFormat.Format32bppArgb);
            _measureGraphics = System.Drawing.Graphics.FromImage(_measureBitmap);
            return _measureGraphics;
        }
        ...
        private System.Drawing.Font GetFont()
        {
            ...
            _cachedFont = new System.Drawing.Font(_fontCollection.Families[0], _mapTextFontSize, FontStyle.Regular, GraphicsUnit.Pixel);
            return _cachedFont;
        }
        ...
        private static CpuImage LoadPng(string path, int maxDimension = 0)
        {
            ...
            if (maxDimension > 0 && Math.Max(targetWidth, targetHeight) > maxDimension)
            {
                var scale = maxDimension / (float)Math.Max(targetWidth, targetHeight);
                targetWidth = Math.Max(1, Mathf.RoundToInt(targetWidth * scale));
                targetHeight = Math.Max(1, Mathf.RoundToInt(targetHeight * scale));
            }
        ...
        private byte[] EncodeImage(Color32[] pixels, int width, int height)
        {
            ...
            try
            {
                ...
                if (_encodeBuffer == null || _encodeBuffer.Length != needed)
                {
                    _encodeBuffer = new byte[needed];
                }

                var bytes = _encodeBuffer;
                for (var y = 0; y < height; y++)
                {
                    var row = y * stride;
                    for (var x = 0; x < width; x++)
                ...
                Marshal.Copy(bytes, 0, data.Scan0, needed);
            }
            finally
            {
                bitmap.UnlockBits(data);
            ...
            if (_imageFormat == MapImageFormat.Png)
            {
