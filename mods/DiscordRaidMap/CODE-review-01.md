...
> 🔴 Bloqueadores: 0 · 🟠 Fortes: 1 · 🟡 Médios: 2 · 🟢 Menores: 5 · Total: 8
...
|---|---|---|---|---|
| CR-01-01 | B | 🟠 | Scan de airdrop não filtra sync objects pooled/não-inicializados → marcador fantasma | Pendente |
| CR-01-02 | B | 🟡 | Marcadores de morte re-derivados por intervalo somem se o corpo despawna (evento removido os tornava permanentes) | Pendente |
| CR-01-03 | B | 🟡 | JPEG sem canal alpha: mapa com transparência achata (provável fundo preto) | Pendente |
| CR-01-04 | F | 🟢 | `GetJpegEncoder()` enumera encoders a cada render — cachear | Pendente |
| CR-01-05 | F | 🟢 | `Bitmap`/`MemoryStream` de encode alocados por render (agora pequenos) | Pendente |
| CR-01-06 | D | 🟢 | `DiscordWebhookClient` não dá `Dispose` no `HttpClient` (pré-existente) | Pendente |
| CR-01-07 | B | 🟢 | `_headlessReferencePlayer` cacheado e nunca revalidado (pré-existente) | Pendente |
| CR-01-08 | E | 🟢 | Tooltips dos novos configs só em inglês (convenção do repo é bilíngue) | Pendente |
