...
public sealed partial class FikaServer : MonoBehaviour, INetEventListener, INatPunchListener, GInterface279, IFikaNetworkManager
{
    ...
    private readonly System.Collections.Concurrent.ConcurrentDictionary<NetPeer, FikaPlayer> _peerToPlayer = new();
    ...
    public void SendGenericPacket(EGenericSubPacketType type, IPoolSubPacket subpacket, bool broadcast = false, NetPeer peerToIgnore = null)
    {
        ...
        // ref: FikaServer.cs SendNetReusable (abaixo) ignora o parâmetro `broadcast`
        // e envia sempre via SendToAll(peerToIgnore) — ou seja, este método é sempre
        // "todos os peers, menos no máximo um", nunca um envio 1:1 real.
    ...
    public void SendGenericPacketToPeer(EGenericSubPacketType type, IPoolSubPacket subpacket, NetPeer peer)
    {
        ...
        // Sempre 1:1 por definição (SendNetReusableToPeer manda só pro `peer`
        // recebido) — o peer só chega a pedir a operação porque já tinha aquele
        // inventário (próprio ou de terceiro) carregado localmente, então devolver
        // rápido não tem risco de corrida de descoberta (backlog 013, PA-01-02).
    ...
    public void SendNetReusable<T>(ref T packet, DeliveryMethod deliveryMethod, byte channelNumber, bool broadcast = false, NetPeer peerToIgnore = null) where T : INetReusable
    {
        _dataWriter.Reset();
        _dataWriter.PutEnum(EPacketType.Serializable);
    ...
    {
        SendNetReusable(ref packet, deliveryMethod, 0, broadcast, peerToIgnore);
    }

    public void SendNetReusableToPeer<T>(ref T packet, DeliveryMethod deliveryMethod, byte channelNumber, NetPeer peer) where T : INetReusable
    {
        _dataWriter.Reset();
        _dataWriter.PutEnum(EPacketType.Serializable);
    ...
    {
        SendNetReusableToPeer(ref packet, deliveryMethod, 0, peer);
    }
    ...
    public void SendStatesToPeer(NetPeer peer, NetDataWriter writer)
    {
        peer.Send(writer.AsReadOnlySpan(), DeliveryMethod.Unreliable);
    }

    public void SendStatesToPeer(NetPeer peer, ReadOnlySpan<byte> data)
    {
    ...
    public void GetConnectedPeers(List<NetPeer> peers)
    {
        if (_netServer != null)
        {
            _netServer.GetConnectedPeers(peers);
        }
        else
        {
            peers.Clear();
    ...
    public bool TryGetPlayerByPeer(NetPeer peer, out FikaPlayer player)
    {
        if (peer != null && _peerToPlayer.TryGetValue(peer, out player) && player != null)
        {
            return true;
        }

        // Fallback caso o dicionário ainda não tenha sido populado no primeiro pacote
        if (peer != null && peer.Tag is string nickname && _coopHandler != null && _coopHandler.HumanPlayers != null)
        {
            var humanPlayers = _coopHandler.HumanPlayers;
            for (var i = 0; i < humanPlayers.Count; i++)
            {
                var hp = humanPlayers[i];
                if (hp?.Profile != null &&
                    (hp.Profile.Info?.MainProfileNickname == nickname || hp.Profile.Nickname == nickname))
                {
                    player = hp;
                    _peerToPlayer[peer] = hp;
                    return true;
                }
            }
        }
    ...
    public void RegisterPeerPlayer(NetPeer peer, FikaPlayer player)
    {
        if (peer != null && player != null)
        {
            _peerToPlayer[peer] = player;
        }
    }

    public void UnregisterPeerPlayer(NetPeer peer)
    {
        if (peer != null)
        {
            _peerToPlayer.TryRemove(peer, out _);
        }
    }
    ...
    public void OnConnectionRequest(ConnectionRequest request)
    {
        if (_coopHandler != null && _coopHandler.LocalGameInstance != null && Singleton<IFikaGame>.Instance.GameController.RaidStarted)
        {
            ...
            if (key == "fika.reconnect" || key == "fika.core")
            {
                request.Accept();
                return;
            }
