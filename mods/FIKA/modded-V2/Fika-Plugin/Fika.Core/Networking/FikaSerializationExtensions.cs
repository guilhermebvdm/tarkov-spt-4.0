...
public static class FikaSerializationExtensions
{
    ...
    // DIAGNOSTIC (temporary — investigação do bug de itens de TacticalVest acumulando
    // entre corpos distintos ao invadir raid em andamento). _bufferWriter/_bufferReader
    // são estáticos e compartilhados por TODAS as chamadas de Put/GetProfile e
    // Put/GetItemDescriptor. Se duas chamadas alguma vez se sobrepuserem (reentrância na
    // mesma thread OU corrida entre threads diferentes), uma chamada pode fazer
    // Reset()/SetSource() enquanto a outra ainda está no meio de escrever/ler — cada uma
    // corrompendo o que a outra estava serializando. Isso detecta e loga alto na hora,
    // em vez de deixar corromper silenciosamente.
    private static int _writerGuardThreadId;
    private static int _readerGuardThreadId;

    private static void EnterWriterGuard(string caller, object context)
    {
        var threadId = Environment.CurrentManagedThreadId;
        var previous = System.Threading.Interlocked.CompareExchange(ref _writerGuardThreadId, threadId, 0);
        if (previous != 0)
        {
            FikaGlobals.LogError($"[DIAG-BUFFER-RACE] {caller} entrou em _bufferWriter (thread {threadId}, contexto={context}) enquanto já estava em uso pela thread {previous}! _bufferWriter será corrompido.");
        }
    }

    private static void ExitWriterGuard()
    {
        System.Threading.Interlocked.Exchange(ref _writerGuardThreadId, 0);
    }

    private static void EnterReaderGuard(string caller, object context)
    {
        var threadId = Environment.CurrentManagedThreadId;
        var previous = System.Threading.Interlocked.CompareExchange(ref _readerGuardThreadId, threadId, 0);
        if (previous != 0)
        {
            FikaGlobals.LogError($"[DIAG-BUFFER-RACE] {caller} entrou em _bufferReader (thread {threadId}, contexto={context}) enquanto já estava em uso pela thread {previous}! _bufferReader será corrompido.");
        }
    }

    private static void ExitReaderGuard()
    {
        System.Threading.Interlocked.Exchange(ref _readerGuardThreadId, 0);
    }
    ...
    {
        EnterWriterGuard(nameof(PutItemDescriptor), null);
        try
        {
    ...
    {
        EnterReaderGuard(nameof(GetItemDescriptor), null);
        try
        {
    ...
    {
        EnterWriterGuard(nameof(PutProfile), profile?.ProfileId);
        try
        {
    ...
    {
        EnterReaderGuard(nameof(GetProfile), null);
        try
        {
            _bufferReader.SetSource(reader.DecompressAndGetByteArray());
