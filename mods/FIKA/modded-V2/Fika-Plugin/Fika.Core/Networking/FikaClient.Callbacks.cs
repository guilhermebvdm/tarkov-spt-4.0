...
public sealed partial class FikaClient
{
    ...
    private void OnClearSnapshotterPacketReceived(ClearSnapshotterPacket packet)
    {
        if (_coopHandler.Players.TryGetValue(packet.NetId, out var player) && player is ObservedPlayer observedPlayer)
        {
            ...
            observedPlayer.ForceTeleport(packet.Position, packet.Rotation);
            return;
        }
    ...
    {
        if (Singleton<GameWorld>.Instantiated && Singleton<GameWorld>.Instance.MainPlayer != null && Singleton<GameWorld>.Instance.MainPlayer.ProfileId == packet.ProfileId)
        {
            if (!packet.Success)
            {
