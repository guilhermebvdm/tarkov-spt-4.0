...
public sealed class FikaConfig(ConfigFile config)
{
    ...
    public ConfigEntry<bool> EnableAoICulling { get; set; }
    public ConfigEntry<float> AoINearDistance { get; set; }
    public ConfigEntry<float> AoIMidDistance { get; set; }
    public ConfigEntry<bool> EnableNetworkThreading { get; set; }
    ...
    public void SetupConfig()
    {
        ...
        EnableAoICulling = SetupSetting(networkDefaultHeader, "Enable AoI Culling", true,
            new ConfigDescription("Filtra o envio de pacotes de bots baseado na proximidade aos jogadores humanos.", tags: new ConfigurationManagerAttributes
            {
                Category = networkHeader,
                DispName = "Enable AoI Culling",
                Order = -1
            }),
            "Enable AoI Culling", ref failed, headers);

        AoINearDistance = SetupSetting(networkDefaultHeader, "AoI Near Distance", 250.0f,
            new ConfigDescription("Distancia maxima (em metros) para a Zona Tatica (taxa maxima de envio ou em combate).",
            new AcceptableValueRange<float>(50.0f, 500.0f), new ConfigurationManagerAttributes
            {
                Category = networkHeader,
                DispName = "AoI Near Distance",
                Order = -2
            }),
            "AoI Near Distance", ref failed, headers);

        AoIMidDistance = SetupSetting(networkDefaultHeader, "AoI Mid Distance", 500.0f,
            new ConfigDescription("Distancia maxima (em metros) para a Zona Periferica (envio intercalado a cada 4 ticks).",
            new AcceptableValueRange<float>(200.0f, 1000.0f), new ConfigurationManagerAttributes
            {
                Category = networkHeader,
                DispName = "AoI Mid Distance",
                Order = -3
            }),
            "AoI Mid Distance", ref failed, headers);

        EnableNetworkThreading = SetupSetting(networkDefaultHeader, "Enable Network Threading", true,
            new ConfigDescription("Processa a filtragem AoI e o despacho UDP em background thread, liberando a Main Thread grafica da Unity.", tags: new ConfigurationManagerAttributes
            {
                Category = networkHeader,
                DispName = "Enable Network Threading",
                Order = -4
            }),
            "Enable Network Threading", ref failed, headers);
