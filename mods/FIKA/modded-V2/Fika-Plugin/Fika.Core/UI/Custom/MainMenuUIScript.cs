...
public class MainMenuUIScript : MonoBehaviour
{
    ...
    private void CreateMainMenuUI()
    {
        ...
        _joinSessionModal = JoinSessionModal.Create(_mainMenuUI.transform);
    ...
    private void SetupPlayers(ref FikaPlayerPresence[] responses)
    {
        foreach (var presence in responses)
        {
            ...
            if (presence.Activity is EFikaPlayerPresence.IN_RAID && presence.RaidInformation.HasValue)
            {
                ...
                var tooltip2 = buttonGo.GetOrAddComponent<HoverTooltipArea>();
                tooltip2.enabled = true;
                tooltip2.SetMessageText(information.Started ? "Raid em andamento — Invadir partida" : LocaleUtils.UI_JOIN_RAID.Localized());

                mainMenuUIPlayer.JoinButton.onClick.RemoveAllListeners();
                mainMenuUIPlayer.JoinButton.onClick.AddListener(() =>
                {
                    if (JoinInProgress)
                    {
                        return;
                    ...
                    var isInvade = information.Started;
                    var title = isInvade ? "INVADIR RAID" : "ENTRAR NA SESSÃO";
                    var btnLabel = isInvade ? "INVADIR" : "ENTRAR";

                    _joinSessionModal.Show(title, btnLabel, true, async (enteredPassword) =>
                    {
                        JoinInProgress = true;
                        ...
                        if (FikaBackendUtils.JoinMatch(FikaBackendUtils.Profile.Id, information.MatchId, out var result, out var errorMessage, enteredPassword))
                        {
