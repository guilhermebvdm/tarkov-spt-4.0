    ...
    private void HandleResult(IResult result)
    {
        ...
        try
        {
            if (Operation != null)
            {
                try
                {
                    // Se o servidor rejeitou a operação (ou falhou por timeout sem confirmação),
                    // garantimos que o status da operação no cliente seja Failed antes do Dispose(),
                    // acionando o RollBack() nativo do EFT para reverter a posição do item localmente.
                    if (serverStatus == EOperationStatus.Failed && Operation.Status != EOperationStatus.Failed)
                    {
                        Operation.Status = EOperationStatus.Failed;
                    }
