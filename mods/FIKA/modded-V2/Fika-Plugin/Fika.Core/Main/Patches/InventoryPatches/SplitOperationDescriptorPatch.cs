    ...
    public static bool Prefix(SplitDescriptorClass __instance, IPlayer player, ref GStruct152<BaseInventoryOperationClass> __result)
    {
        if (player.InventoryController is Interface18)
        {
            ...
            if (!fromAddress.Value.Equals(itemResult.Value.Parent))
            {
                ...
                // permitimos que o Split ocorra a partir da localização real do item no host.
                FikaGlobals.LogWarning($"[SplitOperationDescriptorPatch] Desvio de localização detectado para item {itemResult.Value.Id} (esperado: {fromAddress.Value}, real no host: {itemResult.Value.Parent}). Dono do container esperado: {InventoryPatchDiagnostics.DescribeGhostOwner(fromAddress.Value)}. Auto-reconciliando origem.");
            }
