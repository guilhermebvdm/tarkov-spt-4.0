...
public class FikaPlayer : LocalPlayer
{
    ...
    public void HandleCallbackFromServer(OperationCallbackPacket operationCallbackPacket)
    {
        ...
        {
            if (operationCallbackPacket.Status == EOperationStatus.Succeeded)
            {
                FikaGlobals.LogInfo($"[Fika-Watchdog] Pacote tardio da operação {operationCallbackPacket.CallbackId} recebido com Succeeded — estado reconciliado com sucesso com o servidor.");
            }
            else
            {
                FikaGlobals.LogWarning($"[Fika-Watchdog] Pacote tardio da operação {operationCallbackPacket.CallbackId} recebido do servidor com status {operationCallbackPacket.Status}: {operationCallbackPacket.Error}");
            }
        }
        else
        {
