...
public static class FikaBackendUtils
{
    ...
    public static bool JoinMatch(string profileId, string serverId, out CreateMatch result, out string errorMessage, string password = null)
    {
        ...
        MatchJoinRequest body = new(serverId, profileId, password);
        try
        {
        ...
        {
            FikaGlobals.LogError($"JoinMatch request exception: {ex}");
            errorMessage = "Falha de comunicação com o servidor ao tentar entrar na partida.";
            return false;
        }
        ...
        if (string.IsNullOrEmpty(result.RaidCode))
        {
            errorMessage = "Servidor não retornou dados válidos da partida.";
            return false;
        }
    ...
    public static async Task CreateMatch(string profileId, string hostUsername, RaidSettings raidSettings, string password = null)
    {
        ...
        CreateMatch body = new(raidCode, profileId, serverGuid, hostUsername, IsSpectator, timestamp, raidSettings, FikaPlugin.Crc32,
            raidSettings.Side, raidSettings.SelectedDateTime, CustomRaidSettings, password);
