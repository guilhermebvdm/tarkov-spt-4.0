...
public class BaseInventoryController : Player.PlayerOwnerInventoryController
{
    ...
    private sealed class CustomAmmoUnloader : Interface19
    {
        ...
        private readonly IItemOwner _initialOwner;
        ...
        public CustomAmmoUnloader(BaseInventoryController baseInventoryController, MagazineItemClass magazine, int unloadPerTick, float unloadOneAmmoSpeed, bool isElite)
        {
            ...
            _isElite = isElite;
            _initialOwner = magazine.Parent?.GetOwner() ?? baseInventoryController;
        ...
        {
            var currentOwner = _magazine.Parent?.GetOwner();
            var owner = _initialOwner ?? currentOwner ?? _inventoryController;
            ...
            {
                currentOwner.RaiseUnloadMagazineEvent(eventArgs);
            }
            if (owner != _inventoryController && currentOwner != _inventoryController)
            {
                _inventoryController.RaiseUnloadMagazineEvent(eventArgs);
            }

            _isEventActive = (status == CommandStatus.Begin);
        }
