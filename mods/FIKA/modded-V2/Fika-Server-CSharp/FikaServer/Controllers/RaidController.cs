    ...
    WebhookService webhookService)
{
    ...
    public FikaRaidJoinResponse HandleRaidJoin(FikaRaidJoinRequestData request)
    {
        ...
        if (match == null)
        {
            ...
            {
                Error = "A partida selecionada não foi encontrada ou já foi encerrada."
            };
        }

        if (match.HasPassword && match.Password != request.Password)
        {
            return new FikaRaidJoinResponse
            {
                Error = "Senha incorreta para a partida selecionada."
            };
        }
