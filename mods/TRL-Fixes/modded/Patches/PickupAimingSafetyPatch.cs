    ...
    public class PickupAimingSafetyPatch
    {
        ...
        public void Enable()
        {
            try
            {
                ...
                if (setter == null)
                {
                    Debug.LogError("TRL-Fixes: setter FirearmController.IsAiming nao encontrado — PickupAimingSafetyPatch NAO aplicado.");
                    return;
                }
                ...
                Debug.Log("TRL-Fixes: Hook no FirearmController.IsAiming (pickup/equip safety) aplicado com sucesso!");
            }
            catch (Exception ex)
            {
                Debug.LogError($"TRL-Fixes: Erro ao aplicar patch PickupAimingSafetyPatch: {ex}");
            }
        }
