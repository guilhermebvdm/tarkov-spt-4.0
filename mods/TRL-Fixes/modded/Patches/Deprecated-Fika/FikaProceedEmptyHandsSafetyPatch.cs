    ...
    public class FikaProceedEmptyHandsSafetyPatch
    {
        ...
        public static bool Prefix(object __instance, object packet, object peer)
        {
            try
            {
                ...
                // Envia ProceedResponsePacket via SendDataToPeer(ref response, DeliveryMethod.ReliableOrdered, peer)
                if (_sendDataToPeerMethod != null && _deliveryMethodEnum != null)
                {
                    object deliveryMethodVal = Enum.ToObject(_deliveryMethodEnum, ReliableOrderedDeliveryMethod);
                    object[] invokeArgs = new object[] { response, deliveryMethodVal, peer };
