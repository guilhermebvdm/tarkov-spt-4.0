...
namespace tarkin.ladders.bep
{
    public class ProceduralLadderBody : IDisposable
    {
        ...
        void InitHands()
        {
            ...
            if (player.HandPosers != null && player.HandPosers.Length >= 2 && player.HandPosers[0] != null && player.HandPosers[1] != null)
            {
        ...
        public void Update(float rollAngle = 0f)
        {
            ...
            if (player.CameraContainer != null)
            {
        ...
        void UpdateGrip()
        {
            ...
            if (proceduralGripLeft != null && proceduralArmLeft != null)
            {
                proceduralGripLeft.Update(shouldCurl: !proceduralArmLeft.InArc);
                leftPoser.SetGrip(proceduralGripLeft);
            }

            if (proceduralGripRight != null && proceduralArmRight != null)
            {
        ...
        public void Dispose()
        {
            if (proceduralArmLeft?.limb?.solver != null)
            {
                proceduralArmLeft.limb.solver.IKPositionWeight = 0f;
                proceduralArmLeft.limb.solver.IKRotationWeight = 0f;
            }
            if (proceduralArmRight?.limb?.solver != null)
            {
                proceduralArmRight.limb.solver.IKPositionWeight = 0f;
                proceduralArmRight.limb.solver.IKRotationWeight = 0f;
            }

            if (player.HandPosers != null)
            {
                if (player.HandPosers.Length > 0 && player.HandPosers[0] != null)
                    player.HandPosers[0].weight = 0f;
                if (player.HandPosers.Length > 1 && player.HandPosers[1] != null)
                    player.HandPosers[1].weight = 0f;
            }
