...
public class CoverAnalyzer(BotComponent bot, CoverFinderComponent coverFinder) : BotBase(bot)
{
    ...
    {
        // ref: AUD-09-05 - Triagem rápida geométrica preliminar fail-fast antes de amostrar NavMesh
        Vector3 colliderPos = collider.transform.position;
        Vector3 dirTargetToCollider = (colliderPos - targetPosition).normalized;
        ...
        if ((colliderPos - targetPosition).sqrMagnitude < (CoverFinderComponent.CoverMinEnemyDistSqr * 0.5f))
        {
        ...
        if (!FindNavMesh(collider.bounds.size, out reason, colliderPos, targetPosition, out navMeshHit))
        {
            return false;
        }
        ...
        // ref: AUD-09-05 - Validar posição de companheiros de esquadrão antes de disparar 5 raycasts no colisor
