...
public static class EnemyGainSightClass
{
    ...
    {
        // ref: AUD-09-06 - Cache intra-frame: se o valor ja foi calculado neste frame para este inimigo, retorna em O(1)
        if (enemy?.Vision != null)
        {
            int currentFrame = Time.frameCount;
            if (enemy.Vision.LastGainSightFrame == currentFrame)
            {
                return enemy.Vision.CachedGainSightModifier;
            }

            float modifier = CalcModifier(enemy) * CalcRepeatSeenCoef(enemy.KnownPlaces);
            enemy.Vision.CachedGainSightModifier = modifier;
            enemy.Vision.LastGainSightFrame = currentFrame;
            return modifier;
        }
