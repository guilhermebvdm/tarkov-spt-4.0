...
public class VisionRaycastJob : BotManagerBase
{
        ...
        while (BotController != null && !_disposed)
        {
            ...
            if (bots != null && bots.Count > 0)
            {
                ...
                if (enemyCount > 0)
                {
                    ...
                    // ref: AUD-09-01 - Buffers nativos persistentes reutilizados via GetSubArray (Zero-Alloc)
                    EnsureCapacity(totalRaycasts);

                    var commandsSlice = _commandsBuffer.GetSubArray(0, totalRaycasts);
                    var hitsSlice = _hitsBuffer.GetSubArray(0, totalRaycasts);

                    CreateCommands(commandsSlice, enemyCount, partCount);
                    _handle = RaycastCommand.ScheduleBatch(commandsSlice, hitsSlice, 32);

                    yield return null;

                    _handle.Complete();
                    AnalyzeHits(hitsSlice, commandsSlice, enemyCount, partCount);
                }
            }
    ...
    public void Dispose()
    {
        ...
        if (_hitsBuffer.IsCreated)
        {
            _hitsBuffer.Dispose();
        }
        if (_commandsBuffer.IsCreated)
        {
            _commandsBuffer.Dispose();
        }
    ...
    {
        if (_bufferCapacity >= required && _hitsBuffer.IsCreated && _commandsBuffer.IsCreated)
        {
            return;
        }
