...
Na versão **v4.5.0**, a gestão de esquadrões recebeu limpeza completa de ciclo de vida (`BotSquads.Dispose()`), eliminação de reflexão no spawn de diálogos de bots, proteção contra nulos em gestos de mãos e otimização de delegados na ponte de compatibilidade com o FIKA.
...
- **Teardown Seguro (v4.5.0):** Ao fim da raid, o `BotSquads.Dispose()` descarta ativamente todos os esquadrões registrados e desinscreve manipuladores de eventos de morte, evitando leaks de memória entre sessões.
...
### Otimizações na v4.5.0:
- **Template de Frases Estático:** Substituiu a reflexão custosa (`Activator.CreateInstance`) no spawn de cada bot por clonagem rápida de um dicionário estático de templates (`_defaultPhraseTemplate`).
- **Null-Safety em Gestos:** A execução de gestos táticos em [`GroupTalk.cs`](../../modded/SAIN/Classes/Bot/Talk/GroupTalk.cs) valida defensivamente `HandsController != null`.
- **Desinscrição Segura:** [`EnemyTalkClass`](../../modded/SAIN/Classes/Bot/Talk/EnemyTalkClass.cs) desinscreve seus eventos de áudio com validação estrita do singleton `BotManagerComponent.Instance`.
...
1. **Project Fika ([`FikaInterop`](../../modded/SAIN/Plugin/ModDetection.cs)):**
   - No modo cooperativo dedicado, a verificação compilada em delegados `Func<bool>` assegura zero overhead de reflexão durante o tick de rede, garantindo que apenas a instância host compute a IA do SAIN.
2. **Questing Bots:**
