...
Na versão **v4.5.0**, foram implementadas correções matemáticas na curva de recuperação de recuo (`Mathf.Abs`), suavização inercial contínua de mira sem snaps bruscos, verificação de fogo amigo com *Zero-Alloc GC* (`SphereCastNonAlloc`) e condicionamento de logs de diagnóstico de mira.
...
2. **Inércia e Suavização Humana ([`PositionSmoother`](../../modded/SAIN/Classes/Bot/WeaponFunction/AimClass.cs)):**
   A mira se desloca com velocidade angular limitada. Na v4.5.0, a chamada `.Snap()` foi restrita estritamente a `AimStatus.NoTarget`, preservando a interpolação fluida e inercial durante toda a transição de visada contra alvos ativos.
3. **Prevenção de Fogo Amigo ([`SAINFriendlyFireClass`](../../modded/SAIN/Classes/Bot/Decision/SAINFriendlyFireClass.cs)):**
   Antes de acionar o gatilho, um raio esférico de segurança é traçado. Na v4.5.0, a verificação utiliza `Physics.SphereCastNonAlloc` com um buffer estático de 16 acertos, eliminando coletas de lixo (*GC spikes*) no loop de disparo.
...
- **Correção Matemática de Decaimento (v4.5.0):** A rotina de decaimento do vetor de recuo utiliza `Mathf.Abs` para evitar inversões bruscas de sinal durante a recuperação do cano da arma.
...
- **Lançador de Granadas ([`BotGrenadeManager`](../../modded/SAIN/Classes/Bot/WeaponFunction/Grenades/BotGrenadeManager.cs)):**
  ...
  - Na v4.5.0, o registro de granadas via [`GrenadePatches.cs`](../../modded/SAIN/Patches/Shoot/GrenadePatches.cs) foi padronizado via `SAINEnableClass.GetSAIN` com resolução defensiva de ID de perfil.
