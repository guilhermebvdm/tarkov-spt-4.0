...
> **⚠️ Relação com o `SAINAILimit` original (`AUD-01-04`):** este LOD **não substitui** o `SAINAILimit`/`AILimitSettings` já existente antes do fork multithread — os dois coexistem e controlam coisas diferentes. O `SAINAILimit` (`SAIN/Classes/Bot/SAINAILimit.cs`) degrada o **alcance sensorial** (visão/audição) de um bot contra *outros bots* quando ambos estão longe de qualquer jogador humano, usando os limiares de `AILimitSettings.AILimitRanges` (150/250/400m). O LOD desta seção controla a **taxa de atualização** (tick rate) da árvore de decisão do bot conforme a distância até o humano mais próximo — um bot em Tier 1/2 continua rodando `SAINAILimit` normalmente, só que numa cadência de tick mais baixa (o `SAINAILimit` está registrado no grupo `_tickWhenActiveClasses`, que é exatamente o grupo throttlado por este LOD). Os limiares de distância do LOD (`LODCloseDistance`/`LODMidDistance`/`LODCloseDistanceMargin`/`LODMidIntervalSeconds`/`LODFarIntervalSeconds`) vivem em `AILimitSettings.cs`, ao lado de `AILimitRanges`, e são editáveis no F6 — mas são dois mecanismos independentes, não uma única configuração. `BotComponent.GetMinDistanceToHumanPlayer()` reusa `SAINAILimit.ClosestPlayerDistanceSqr` (que, apesar do nome, já é distância linear em metros) em vez de calcular a distância de novo, para não duplicar a mesma busca.
