...
namespace Band_Aid
{
    public static class BandAidNetworkHandler
    {
        ...
        private static void CacheTypes()
        {
            ...
            // GInterfaces comuns — para LEITURA
            _heavyBleedType = typeof(GInterface314);
            _lightBleedType = typeof(GInterface313);
            _fractureType = typeof(GInterface316);
            // Tipos concretos — para REMOÇÃO
        ...
        private static void OnBandAidHealPacketReceived(BandAidHealPacket packet)
        {
            ...
            if (Singleton<FikaServer>.Instantiated)
            {
                ...
                if (packet.DoctorProfileId != myProfileId)
                {
                    ...
                    Singleton<FikaServer>.Instance.SendDataToAll(ref retransmit, DeliveryMethod.ReliableOrdered);
        ...
        private static void OnHealCheckReceived(BandAidHealCheckPacket packet)
        {
            ...
            if (Singleton<FikaServer>.Instantiated)
            {
                ...
                {
                    var relay = packet;
                    Singleton<FikaServer>.Instance.SendDataToAll(ref relay, DeliveryMethod.ReliableOrdered);
                }
                if (mainPlayer == null) return;
            }
            ...
            if (Singleton<FikaServer>.Instantiated)
                Singleton<FikaServer>.Instance.SendDataToAll(ref response, DeliveryMethod.ReliableOrdered);
            else if (Singleton<FikaClient>.Instantiated)
