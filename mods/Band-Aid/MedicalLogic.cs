...
namespace Band_Aid
{
    public static class MedicalLogic
    {
        ...
        // Cache dos tipos — GInterfaces para LEITURA, nested types para REMOÇÃO
        // GInterfaces: funcionam em qualquer HC (ActiveHC + ObservedHC)
        private static Type _heavyBleedType;   // GInterface314 — para HasEffect
        private static Type _lightBleedType;   // GInterface313 — para HasEffect
        private static Type _fractureType;     // GInterface316 — para HasEffect
        // Nested types concretos: necessários para method_15 (RemoveEffect) no ActiveHC
        ...
        private static void CacheTypes()
        {
            ...
            // GInterfaces comuns — para LEITURA (HasEffect, UI)
            _heavyBleedType = typeof(GInterface314);   // HeavyBleeding
            _lightBleedType = typeof(GInterface313);   // LightBleeding
            _fractureType = typeof(GInterface316);     // Fracture

            // Tipos concretos do ActiveHC — para REMOÇÃO (method_15/ForceResidue)
            ...
            Logger.LogInfo($"Tipos cacheados - GInterface: HB={_heavyBleedType != null}, LB={_lightBleedType != null}, FR={_fractureType != null} | Concretos: HB={_heavyBleedConcreteType != null}, LB={_lightBleedConcreteType != null}, FR={_fractureConcreteType != null}");
            _typesCached = true;
        }
        ...
        public static void ApplyTreatment(Player doctor, Player patient, Item item, ItemStats stats)
        {
            ...
            // === PACIENTE REMOTO (ObservedCoopPlayer) ===
            // Jogadores remotos usam ObservedHealthController (não ActiveHealthController).
            // Não é possível aplicar cura local — enviamos via rede para o paciente curar em si mesmo.
            if (doctor.ProfileId != patient.ProfileId && !(hc is ActiveHealthController))
            {
                ...
                // Estimar custo de consumo do item do médico
                float consumeCost;
            ...
            if (stats.StopsHeavyBleed || stats.StopsAllBleeds)
            {
                ...
                {
                    if (RemoveEffect(hc, target, _heavyBleedConcreteType, "HeavyBleeding"))
                    {
                        hpToConsume += cost;
                        actionTaken = true;
            ...
            if (stats.StopsLightBleed || stats.StopsAllBleeds)
            {
                if (!actionTaken || stats.StopsAllBleeds)
                {
                    ...
                    if (CanAfford(item, cost, hpToConsume))
                    {
                        if (RemoveEffect(hc, target, _lightBleedConcreteType, "LightBleeding"))
                        {
                            hpToConsume += cost;
                            actionTaken = true;
            ...
            if (stats.FixesFracture)
            {
                ...
                if (CanAfford(item, cost, hpToConsume))
                {
                    if (RemoveEffect(hc, target, _fractureConcreteType, "Fracture"))
                    {
                        hpToConsume += cost;
                        actionTaken = true;
