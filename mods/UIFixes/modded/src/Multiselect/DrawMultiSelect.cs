...
public class DrawMultiSelect : MonoBehaviour
{
    ...
    private readonly List<GridItemView> _cachedGridItemViews = [];
    private readonly List<RaycastResult> _sharedRaycastResults = [];
    private PointerEventData _sharedEventData;
    ...
    public void Start()
    {
        ...
        _sharedEventData = new(EventSystem.current);
    }

    public void OnDisable()
    {
        _drawing = false;
        _cachedGridItemViews.Clear();
        MultiSelect.Clear();
    }
    ...
    public void Update()
    {
        ...
        if (Settings.SelectionBoxKey.Value.IsDownIgnoreOthers())
        {
            ...
            {
                _cachedGridItemViews.AddRange(transform.root.GetComponentsInChildren<GridItemView>());
            }
            if (Singleton<PreloaderUI>.Instantiated && Singleton<PreloaderUI>.Instance != null)
            {
                _cachedGridItemViews.AddRange(Singleton<PreloaderUI>.Instance.GetComponentsInChildren<GridItemView>());
            }
        ...
        if (_drawing)
        {
            ...
            foreach (GridItemView gridItemView in _cachedGridItemViews)
            {
    ...
    private bool IsOnTop(Rect itemRect, Transform itemTransform, GraphicRaycaster raycaster)
    {
        ...
        _sharedEventData.position = new Vector2(itemRect.xMin + widthMargin, itemRect.yMin + heightMargin);
        raycaster.Raycast(_sharedEventData, _sharedRaycastResults);
        if (_sharedRaycastResults.Count > 0 && !_sharedRaycastResults[0].gameObject.transform.IsDescendantOf(itemTransform))
        {
            return false;
        }
        ...
        // Upper left
        _sharedRaycastResults.Clear();
        _sharedEventData.position = new Vector2(itemRect.xMin + widthMargin, itemRect.yMax - heightMargin);
        raycaster.Raycast(_sharedEventData, _sharedRaycastResults);
        if (_sharedRaycastResults.Count > 0 && !_sharedRaycastResults[0].gameObject.transform.IsDescendantOf(itemTransform))
        {
            return false;
        ...
        // Upper right
        _sharedRaycastResults.Clear();
        _sharedEventData.position = new Vector2(itemRect.xMax - widthMargin, itemRect.yMax - heightMargin);
        raycaster.Raycast(_sharedEventData, _sharedRaycastResults);
        if (_sharedRaycastResults.Count > 0 && !_sharedRaycastResults[0].gameObject.transform.IsDescendantOf(itemTransform))
        {
            return false;
        ...
        // Lower right
        _sharedRaycastResults.Clear();
        _sharedEventData.position = new Vector2(itemRect.xMax - widthMargin, itemRect.yMin + heightMargin);
        raycaster.Raycast(_sharedEventData, _sharedRaycastResults);
        return _sharedRaycastResults.Count == 0 || _sharedRaycastResults[0].gameObject.transform.IsDescendantOf(itemTransform);
    }
}
