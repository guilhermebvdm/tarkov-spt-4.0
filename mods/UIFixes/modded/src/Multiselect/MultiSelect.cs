...
public class MultiSelect
{
    ...
    private static readonly Dictionary<MultiSelectItemContext, (WindowContext Context, Action Handler)> WindowCloseHandlers = [];
    ...
    public static void Select(GridItemView itemView, bool secondary = false)
    {
        ...
        if (itemView.IsSelectable() && !SelectedItems.Any(x => x.Key.Item == itemView.Item) && !SecondaryItems.Any(x => x.Key.Item == itemView.Item))
        {
            ...
            {
                Action closeHandler = () => Deselect(itemContext);
                WindowCloseHandlers[itemContext] = (windowContext, closeHandler);
                windowContext.OnClose += closeHandler;
            }
    ...
    public static void Deselect(MultiSelectItemContext itemContext, bool secondary = false)
    {
        ...
        if (WindowCloseHandlers.TryGetValue(itemContext, out var closeData))
        {
            if (closeData.Context != null)
            {
                closeData.Context.OnClose -= closeData.Handler;
            }
            WindowCloseHandlers.Remove(itemContext);
        }
