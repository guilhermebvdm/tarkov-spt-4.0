...
public static class QueueInputPatches
{
    ...
    public class ReloadPatch : ModulePatch
    {
        ...
        private static FirearmHandsInputTranslator GetInputTranslator()
        {
            ...
            // Support for FIKA (CoopGame inherits from BaseLocalGame<EftGamePlayerOwner>)
            var playerOwnerProperty = game.GetType().GetProperty("PlayerOwner", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            if (playerOwnerProperty != null && playerOwnerProperty.GetValue(game) is PlayerOwner playerOwner)
            {
                return playerOwner.HandsInputTranslator as FirearmHandsInputTranslator;
            }

            var mainPlayerOwner = Singleton<GameWorld>.Instance?.MainPlayer?.GetComponentInChildren<PlayerOwner>();
            return mainPlayerOwner?.HandsInputTranslator as FirearmHandsInputTranslator;
        }
    }
