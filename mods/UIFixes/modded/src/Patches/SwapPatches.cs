...
public static class SwapPatches
{
    ...
    public static void Enable()
    {
        ...
        new ActiveWeaponMagDropPatch().Enable();
        new FikaActiveWeaponMagSwapPatch().Enable();
    ...
    public class WeaponApplyPatch : ModulePatch
    {
        ...
        public static void Postfix(Weapon __instance, TraderControllerClass itemController, Item item, bool simulate, ref ItemOperation __result)
        {
            ...
            if (__result.Succeeded || item is not MagazineItemClass magazine || __result.Error is not SlotNotEmptyError)
            {
                return;
            }

            Slot magazineSlot = __instance.GetMagazineSlot();
            if (magazineSlot?.ContainedItem == null)
            {
                return;
            }

            ItemAddress itemAddress = item.CurrentAddress;
            if (itemAddress == null)
            {
                return;
            }
            ...
            ItemAddress currentMagAddress = magazineSlot.ContainedItem.CurrentAddress;
            if (currentMagAddress == null)
            {
                return;
            }

            __result = InteractionsHandlerClass.Swap(item, currentMagAddress, magazineSlot.ContainedItem, itemAddress, itemController, simulate);
        }
    }

    // Log and track dropping magazines onto weapons in inventory (or its mod_magazine slot in Inspect)
    // Lets the canonical InteractionsHandlerClass.Swap pipeline run, providing in-place 1-to-1 swap in the same slot.
    public class ActiveWeaponMagDropPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
        ...
        [PatchPrefix]
        public static bool Prefix(SlotView __instance, ItemContextClass itemContext)
        {
            if (itemContext?.Item is not MagazineItemClass magazine)
            {
                return true;
            }

            Weapon targetWeapon = null;
            if (__instance.Slot?.ContainedItem is Weapon weaponInSlot)
            {
                // Dropping directly onto the weapon in equipment slots (FirstPrimaryWeapon, SecondPrimaryWeapon, Holster)
                targetWeapon = weaponInSlot;
            }
            else if (__instance.Slot?.ParentItem is Weapon weaponParent && __instance.Slot.ContainedItem is MagazineItemClass)
            {
                // Dropping onto the magazine slot within the weapon's Inspect window
                targetWeapon = weaponParent;
            }

            if (targetWeapon != null)
            {
                Player mainPlayer = Singleton<GameWorld>.Instance?.MainPlayer;
                Player.FirearmController firearmController = mainPlayer?.HandsController as Player.FirearmController;
                bool isHeldWeapon = firearmController != null && firearmController.Item != null && firearmController.Item.Id == targetWeapon.Id;

                Plugin.Instance.Logger.LogWarning($"[UIFixes-Diag] === DROP DETECTADO === Mag: {magazine.Id}, TargetWeapon: {targetWeapon.Name.Localized()} (ID: {targetWeapon.Id}), Slot: {__instance.Slot?.ID}, HeldWeapon: {firearmController?.Item?.Name?.Localized()} (ID: {firearmController?.Item?.Id}), isHeldWeapon: {isHeldWeapon} => Despachando Swap de inventário...");
            }
    ...
    // Prevents Headless and Host from rejecting in-place magazine swap transactions on active weapons.
    // When a magazine is swapped on a weapon held in hands, TraderControllerClass.CheckItemAction normally returns
    // GClass1561 ("Cannot apply item because item Default Inventory is currently being modified") due to inOutHandsProcess.
    // This patch clears the artificial GClass1561 error for magazine swaps, allowing FIKA Headless to accept the transaction,
    // confirm with OperationCallbackPacket(Succeeded), and prevent trigger/inventory freeze.
    public class FikaActiveWeaponMagSwapPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(TraderControllerClass), nameof(TraderControllerClass.CheckItemAction));
        }

        [PatchPostfix]
        public static void Postfix(TraderControllerClass __instance, Item item, ItemAddress location, ref GStruct155 __result)
        {
            if (!__result.Failed)
            {
                return;
            ...
            // GClass1561 is "Cannot apply {Item_0} because {Item_1} is currently being modified"
            string errorName = __result.Error?.GetType().Name;
            if (errorName != "GClass1561" && !(__result.Error?.ToString()?.Contains("currently being modified") ?? false))
            {
                return;
            }

            // If the item is a magazine or the target/origin is a weapon slot, allow the in-place magazine swap
            bool isMagazineAction = item is MagazineItemClass || 
                                    location?.Container?.ParentItem is Weapon || 
                                    item?.Parent?.Container?.ParentItem is Weapon;

            if (isMagazineAction)
            {
                Plugin.Instance.Logger.LogWarning($"[UIFixes-Diag] CheckItemAction: Suprimindo GClass1561 para item {item?.Id} ({item?.GetType().Name}) em location {location?.Container?.ID} - permitindo swap no mesmo slot!");
                __result = default;
            }
        }
    }
