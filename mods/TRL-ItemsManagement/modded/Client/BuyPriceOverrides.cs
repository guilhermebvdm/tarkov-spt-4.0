...
/// <summary>
///     Lazy-loaded cache of <c>config/buy-overrides.json</c> (traderId → tpl → {count, currency}), fetched
///     once from the paired server mod's route (<c>/trlitemsmanagement/buy-overrides</c>,
///     <c>modded/Server/Web/TraderBuyOverridesRouter.cs</c> — the SAME file the server-side Harmony
///     backstop <c>modded/Server/Pricing/SellItemPatch.cs</c> reads), so the displayed sell-to-trader
///     price and the money actually credited always agree. Loading pattern mirrors CustomClasses'
...
internal static class BuyPriceOverrides
{
    ...
    private static void EnsureLoaded()
    {
        ...
        try
        {
            var json = RequestHandler.GetJson("/trlitemsmanagement/buy-overrides");
            var result = new Dictionary<string, Dictionary<string, Entry>>(StringComparer.OrdinalIgnoreCase);

            if (!string.IsNullOrWhiteSpace(json))
            {
                // Parsed leaf-by-leaf via JObject/JToken (never a single strongly-typed
            ...
            Plugin.Log?.LogInfo($"[TRLItemsManagement] {CountEntries(result)} buy-price override(s) loaded.");
        }
        catch (Exception ex)
        {
            _byTraderThenTpl = new Dictionary<string, Dictionary<string, Entry>>();
            Plugin.Log?.LogError($"[TRLItemsManagement] failed to fetch buy-overrides: {ex.Message}");
        }
    }
