...
- **Alinhamento do Controller de Servidor com o Padrão Canônico do SPT 4.0:** Removidos atributos `Consumes` de [SftChannelController.cs](file:///d:/Projetos/GITHUB%20TARKOV/tarkov-spt-4.0/mods/TRL-SpeakFromTarkov/Server/TRL-SpeakFromTarkov.Server/Controllers/SftChannelController.cs#L40) alinhando a assinatura ao padrão canônico de todos os controllers de produção (`PlayerIpsManager.cs`, `HwidManager.cs`), erradicando os erros de `UnsupportedMediaType` no cliente BepInEx.
