...
namespace TRL_SpeakFromTarkov.Audio
{
    ...
    public class AudioFilter : IDisposable
    {
        ...
        // AUD-03-05: config reconfigurada TODO FRAME pela main thread (MicrophoneCapturer.Update(),
        // a partir do F12) e lida pela THREAD DE CAPTURA em Apply(). Propriedades soltas permitiam
        // ler um conjunto parcialmente atualizado (campo novo + campo antigo misturados). Trocado
        // por um snapshot imutável trocado por referência — leitura/escrita de referência já é
        // atômica em .NET, e Interlocked.Exchange garante a barreira de memória; a thread de
        // captura sempre vê um conjunto coerente, nunca uma mistura.
        public sealed class NoiseGateConfig
        {
            ...
            public readonly float LpfAlpha; // já pré-calculado a partir do LPFCutoffHz recebido
        ...
        private NoiseGateConfig _config = new NoiseGateConfig(
            0.008f, 0.005f, 0.15f, true, 0.85f, 0.15f, false, true, false, 4000f, 48000);

        public void UpdateConfig(NoiseGateConfig newConfig) => System.Threading.Interlocked.Exchange(ref _config, newConfig);

        private float _lpfPrevOut = 0f; // estado do filtro (não é config — persiste entre chamadas, só na thread de captura)
        ...
        {
            var config = _config; // snapshot único e coerente, lido 1x no topo do método (thread de captura)

            if (_rnAvailable && config.UseRNNoise)
            {
                ApplyHPF(buffer);
                if (config.LpfAlpha > 0) ApplyLPF(buffer, config.LpfAlpha);
                ApplyRNNoise(buffer); // não lê nenhum campo de NoiseGateConfig
            }
            else
            {
            ...
            {
                ApplyAGC(buffer); // não lê NoiseGateConfig
            }

            // O limiter é a última barreira de proteção de áudio
            if (config.EnableLimiter) ApplyLimiter(buffer); // não lê NoiseGateConfig
        }
        ...
        {
            ApplyHPF(buf);
            ApplyNoiseGate(buf, config); // ApplyNoiseGate é quem de fato lê OpenThreshold/HoldTime
        }
        ...
        private void ApplyLPF(float[] buf, float lpfAlpha)
        {
            for (int i = 0; i < buf.Length; i++)
            {
                float x = buf[i];
                float y = _lpfPrevOut + lpfAlpha * (x - _lpfPrevOut);
                _lpfPrevOut = y;
                buf[i] = y;
            }
        }
        ...
        private void ApplyNoiseGate(float[] buf, NoiseGateConfig config)
        {
            float rms = GetRMS(buf);

            if (rms >= config.OpenThreshold) { _gateOpen = true; _gateHoldTimer = config.HoldTime; }
            else if (_gateOpen)
            {
