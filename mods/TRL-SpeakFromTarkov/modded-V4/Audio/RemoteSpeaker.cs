...
namespace TRL_SpeakFromTarkov.Audio
{
    public class RemoteSpeaker : MonoBehaviour
    {
        ...
        // AUD-03-09: referência ao AudioClip runtime criado em Initialize(), destruído em OnDestroy.
        private AudioClip? _streamClip;
        // PERF-INSTR AUD-015: throttle da instrumentação temporária de calibração da curva de distância.
        private int _lastCurveLogTick = 0;
        ...
        // AUD-03-12: packetQueue removido — nunca era usado (EnqueuePacket decodifica direto no
        // streamBuffer, ver comentário corrigido mais abaixo).
        ...
        // AUD-03-02: posição/direção do listener resolvidas 1x por frame em Update() (main thread).
        // OnAudioFilterRead (audio thread) só LÊ estes campos — nunca chama Camera.main/Singleton<GameWorld>.
...
#pragma warning restore CS0618
            ...
            // AUD-015: bypassReverbZones bypassa o AudioReverbZone CLÁSSICO do Unity — sistema
            // diferente do AudioMixer/AudioMixerSnapshot usado pelo roteamento abaixo (ver
            // BetterAudio.cs). Não remover esta linha: não conflita com o reverb nativo.
            audioSource.bypassReverbZones = true;

            // AUD-015: roteia pro grupo de mixer nativo do jogo — reverb ambiental por sala/
            // corredor já aplicado via AudioMixerSnapshot (ref: BetterAudio.cs:24,543,1003).
            // ATENÇÃO: a linha "ignoreListenerVolume/Pause" acima foi escrita de propósito pra
            // evitar que o jogo "multe"/silencie o VoIP em certos estados — se o roteamento pelo
            // mixer nativo reintroduzir esse problema (grupo pai mutado/ducked em algum estado do
            // jogo), desative EnableNativeReverb no F12 como mitigação imediata.
            bool enableNativeReverb = VoIPPlugin.EnableNativeReverb == null || VoIPPlugin.EnableNativeReverb.Value;
            if (enableNativeReverb && Singleton<BetterAudio>.Instantiated)
            {
                audioSource.outputAudioMixerGroup = Singleton<BetterAudio>.Instance.ObservedPlayerSpeechMixer;
            }
            ...
            _streamClip = clip; // AUD-03-09: guardado pra ser destruído em OnDestroy
        ...
        public void EnqueuePacket(byte[] opusData, float voiceLevel = 0f)
        {
            ...
            // AUD-03-12: decodificação síncrona no callback do NetPacketProcessor. Roda na MAIN
            // THREAD — PollEvents() só é chamado do Update() de FikaClient/FikaServer, não existe
            // "thread de rede" separada aqui (confirmado em
            // references/fika-plugin/Fika.Core/Networking/{FikaClient,FikaServer}.cs).
...
#pragma warning restore CS0618
        ...
        void Update()
        {
            try
            {
                // AUD-03-02: único lugar (main thread) que resolve Camera.main/Singleton<GameWorld>
                // pra oclusão E pra OnAudioFilterRead (audio thread) — antes cada um lia de novo.
                if (Camera.main != null)
                {
                ...
                if (Time.time - _lastOcclusionCheckTime >= 0.20f)
                {
                    ...
                    if (occlusionEnabled && currentSpatialBlend > 0f && !isEmergency2DMode)
                    {
                        if (_occlusionLayerMask == -1)
                        {
                            try
                            {
                                // AUD-03-03: DoorLayer é índice de layer cru (LayerMaskClass.cs:82,125), não
                                // bitmask — precisa do shift "1 <<". InteractiveMask já vem deslocado
                                // (LayerMaskClass.cs:58,132), diferente de InteractiveLayer (índice cru).
                        ...
                        Vector3 listenerPos = _cachedListenerPos; // era Camera.main.transform.position lido de novo aqui
        ...
        void OnAudioFilterRead(float[] data, int channels)
        {
            ...
            // AUD-03-02: OnAudioFilterRead roda na audio thread do Unity — Camera.main/Singleton<GameWorld>
            // não são seguros de chamar aqui. _cachedListenerPos já foi resolvido no Update() (main thread).
            ...
            if (currentSpatialBlend > 0f)
            {
                if (listenerPos != Vector3.zero)
                {
                    ...
                    // PERF-INSTR AUD-015: instrumentação TEMPORÁRIA pra calibrar a curva de
                    // distância com dados reais de raid — remover depois da calibração. Usa
                    // Environment.TickCount (não Time.*) porque OnAudioFilterRead roda na audio
                    // thread do Unity (mesmo motivo de SftNetwork.LogErrorThrottled).
                    if (VoIPPlugin.EnableDebugLogs != null && VoIPPlugin.EnableDebugLogs.Value
                        && Environment.TickCount - _lastCurveLogTick > 2000)
                    {
                        _lastCurveLogTick = Environment.TickCount;
                        VoIPPlugin.Log.LogInfo($"[SFT-CURVE] dist={dist:F1}m attn={distanceAttenuation:F2} airDamp={airDampingAlpha:F2}");
                    }
                }
            }
            ...
            {
                // AUD-03-02: idem acima — leitura pura de campo, zero chamada de API Unity nesta thread.
        ...
        {
            decoder = null!;
            if (_streamClip != null) // AUD-03-09: sem isso, o AudioClip runtime nunca é liberado
            {
                Destroy(_streamClip);
                _streamClip = null;
            }
