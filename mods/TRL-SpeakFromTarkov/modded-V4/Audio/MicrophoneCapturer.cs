...
namespace TRL_SpeakFromTarkov.Audio
{
    ...
    public class MicrophoneCapturer : MonoBehaviour
    {
        ...
        public bool StartCapture(string device)
        {
            ...
            audioFilter.UpdateConfig(BuildNoiseGateConfig());
        ...
        void Update()
        {
            ...
            if (audioFilter != null)
            {
                // AUD-03-05: monta o snapshot inteiro de uma vez e troca por referência
                // (Interlocked.Exchange dentro de UpdateConfig) em vez de escrever campo a campo —
                // a thread de captura nunca vê uma mistura de valores novos/antigos.
                audioFilter.UpdateConfig(BuildNoiseGateConfig());
            }
        }

        /// <summary>Monta o snapshot de config a partir dos ConfigEntry atuais do F12 (main thread).</summary>
        private AudioFilter.NoiseGateConfig BuildNoiseGateConfig()
        {
            return new AudioFilter.NoiseGateConfig(
