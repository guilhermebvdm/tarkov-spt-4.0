...
namespace TRL_SpeakFromTarkov.Network
{
    ...
    public class SftNetwork : MonoBehaviour
    {
        ...
        private void DrainSendQueue()
        {
            ...
            while (sendQueue.TryDequeue(out var pending))
            {
                try
                {
                    ...
                    // AUD-014: só o Canal 0 (raid 3D) participa do culling por distância — Canais
                    // 1 (menu) / 2 (espectador) continuam broadcast:true, sem noção de distância.
                    if (pending.Channel == 0 && Fika.Core.Main.Utils.FikaBackendUtils.IsServer)
                    {
                        // Host falando: FikaServer.SendData ignora o parâmetro broadcast e sempre
                        // faz SendToAll (ver docs/investigacao-canal-litenetlib-voip.md) — o host
                        // decide aqui mesmo, localmente, pra quem retransmitir.
                    ...
                    {
                        // Convidado: broadcast:false — vai só pro host, que decide o relay em
                        // OnReceiveVoipDataV2 (o FIKA nunca relaya automaticamente pacotes com
                        // broadcast:false pros outros convidados).
        ...
        /// pacote ter chegado só no host por broadcast:false). Só tem efeito real no host — a
        /// checagem IsServer é defensiva, nunca deveria ser chamada num convidado.
        /// </summary>
        private static void RelayVoiceToNearbyPeers(SftAudioPacketV2 packet, Player senderPlayer, Fika.Core.Networking.LiteNetLib.NetPeer? excludePeer)
        {
            ...
            float maxCullDistance = maxHearing * 1.10f; // mesma margem de HandleVoipPacket (ver HandleVoipPacket abaixo)
            float sqrMaxCull = maxCullDistance * maxCullDistance;

            // RegisteredPlayers (não AllAlivePlayersList): inclui jogadores mortos/espectadores,
            // que têm "escuta dupla" (Canal 0 3D dos vivos + Canal 2 2D) — usar só a lista de
            // vivos quebraria essa feature já existente.
            ...
            for (int i = 0; i < allPlayers.Count; i++)
            {
                ...
                {
                    // Falha isolada por peer não deve abortar o relay pros demais.
        ...
        private static void OnReceiveVoipDataV2(SftAudioPacketV2 packet)
        {
            ...
            // audição local. Independente do resultado do relay, o host continua ouvindo
            // normalmente via DispatchVoipPacket abaixo.
        ...
        public static void BroadcastChannelAnnouncement(byte channelId, string channelName, string hostProfileId, string hostNickname, byte action, string targetProfileId = "")
        {
            try
            {
                ...
                Task.Run(() =>
                {
                    ...
                    {
                        // AUD-03-14: era catch{} silencioso — falha de rede aqui ficava invisível.
