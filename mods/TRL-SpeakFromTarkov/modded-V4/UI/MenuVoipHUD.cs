        ...
        private void FetchServerChannels()
        {
            Task.Run(() =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    SftNetwork.LogErrorThrottled("Erro ao buscar canais do servidor SPT (sft/channels/list)", ex);
                }
            });
        }
        ...
        private void OnGUI()
        {
            ...
            if (ConnectedChannelId.HasValue)
            {
                ...
                if (activeChannels.TryGetValue(ConnectedChannelId.Value, out var curChannel))
                {
                    ...
                    // AUD-03-06: HashSet<string> só é mutado por confirmação (frame separado, ver
                    // DrawConfirmationModal) — nunca durante este próprio foreach. .ToList() era
                    // uma alocação desnecessária a cada OnGUI (2-4x/frame enquanto o menu HUD é visível).
                    foreach (var memberId in curChannel.Members)
                    {
            ...
            // AUD-03-06: ConcurrentDictionary.Values já é seguro pra enumeração direta (snapshot
            // momento-a-momento garantido pelo próprio tipo) — .ToList() era alocação desnecessária.
            ...
            else
            {
                ...
                float contentHeight = activeChannels.Count * 26f;
