...
namespace TRL_SpeakFromTarkov.UI
{
    ...
    public class PlayerVolumeMixerHUD : MonoBehaviour
    {
        ...
        // AUD-03-07: debounce da persistência em disco — SetPlayerVolume só marca "sujo"; a
        // escrita real (File.WriteAllText) acontece no máximo 1x/segundo via Update(), ou
        // imediatamente nos pontos de fechamento (Close/OnDestroy/OnApplicationQuit) pra não
        // perder o último ajuste. Sem volatile: tudo aqui roda só na main thread (OnGUI/Update
        // são callbacks do Unity), sem acesso cross-thread.
        private static bool _saveDirty = false;
        ...
        // AUD-03-06: GUIStyle cacheados (mesmo padrão já usado em VoipHUD.cs/InRaidVoipHUD.cs
        // desde a Review 02) — antes, `new GUIStyle(...)` era alocado a cada OnGUI/DrawPlayerRow.
        private GUIStyle? _headerStyle;
        private GUIStyle? _closeBtnStyle;
        private GUIStyle? _subHeaderStyle;
        private GUIStyle? _emptyStyle;
        private GUIStyle? _resetAllStyle;
        private GUIStyle? _doneStyle;
        private GUIStyle? _nameStyle;
        private GUIStyle? _volLabelStyle;
        private GUIStyle? _muteBtnStyle;
        ...
        {
            FlushPendingSave(); // AUD-03-07: garante que nenhum ajuste de volume pendente se perca
        ...
        {
            FlushPendingSave(); // AUD-03-07: melhor esforço contra Alt+F4/fechamento abrupto
        }
        ...
        public static void SetPlayerVolume(string profileId, float volume)
        {
            ...
            ApplyToSpeaker(profileId); // imediato — é o áudio em tempo real, não a persistência
            _saveDirty = true; // AUD-03-07: persistência real fica pro throttle/flush, não aqui
        }

        /// <summary>Ponto único de persistência real. Chamado pelo throttle do Update() e, como
        /// garantia contra perda de dado, em todo caminho de fechamento (Close/OnDestroy/quit).</summary>
        ...
        public void Close()
        {
            ...
            FlushPendingSave(); // AUD-03-07: garante persistência ao fechar o mixer, não só ao sair do jogo
        }
        ...
        void Update()
        {
            if (IsOpen)
            {
                ...
                // AUD-03-07: throttle de persistência — no máximo 1 escrita em disco por segundo
                // enquanto o mixer fica aberto e o volume é ajustado.
        ...
        void OnGUI()
        {
            ...
            GUI.Label(new Rect(modalX + 18f, modalY + 5f, modalW - 80f, 35f), "🎛️ SFT · IN-RAID PLAYER VOLUME MIXER", _headerStyle);
            ...
            if (GUI.Button(new Rect(modalX + modalW - 42f, modalY + 8f, 30f, 28f), "✕", _closeBtnStyle))
            {
                Close();
                return;
            }
            ...
            GUI.Label(new Rect(modalX + 18f, modalY + 50f, modalW - 36f, 20f), "Ajuste o volume de cada jogador para a sua audição local (0% a 200%). Salvo automaticamente.", _subHeaderStyle);
            ...
            if (playersList.Count == 0)
            {
                ...
                GUI.Label(new Rect(0, 50f, listW - 20f, 40f), "Nenhum outro jogador remoto detectado na raid atual.", _emptyStyle);
            }
            else
            {
                float rowY = 5f;
            ...
            if (GUI.Button(new Rect(modalX + 18f, footerY, 160f, 34f), "🔄 Resetar Todos (100%)", _resetAllStyle))
            {
                foreach (var p in playersList)
                {
            ...
            if (GUI.Button(new Rect(modalX + modalW - 138f, footerY, 120f, 34f), "Concluído", _doneStyle))
            {
                Close();
            }
        }
        ...
        private void DrawPlayerRow(PlayerVolumeData player, float x, float y, float w, float h)
        {
            ...
            // Nome do Jogador
            // AUD-03-06: estilo cacheado (alocado 1x), mas a cor depende de isMuted por linha/frame —
            // atualizamos só o campo de cor a cada chamada (escrita simples, não alocação), em vez
            // de recriar o GUIStyle inteiro.
            ...
            _nameStyle.normal.textColor = isMuted ? new Color(0.9f, 0.3f, 0.3f) : Color.white;
            string displayName = !string.IsNullOrEmpty(player.Nickname) ? player.Nickname : player.ProfileId;
            GUI.Label(new Rect(x + 12f, y + 6f, 180f, 22f), displayName, _nameStyle);
            ...
            _volLabelStyle.normal.textColor = isMuted ? new Color(0.9f, 0.3f, 0.3f) : (currentVol > 1.05f ? new Color(0.95f, 0.85f, 0.3f) : new Color(0.4f, 0.9f, 0.5f));

            string volText = isMuted ? "MUTADO (0%)" : (currentVol > 1.05f ? $"{currentVol * 100:F0}% [BOOST]" : $"{currentVol * 100:F0}%");
            GUI.Label(new Rect(x + w - 190f, y + 6f, 100f, 22f), volText, _volLabelStyle);
            ...
            _muteBtnStyle.normal.background = isMuted ? _btnSuccessTex : _btnDangerTex;

            if (GUI.Button(new Rect(x + w - 75f, y + 14f, 65f, 30f), isMuted ? "Unmute" : "Mute", _muteBtnStyle))
            {
