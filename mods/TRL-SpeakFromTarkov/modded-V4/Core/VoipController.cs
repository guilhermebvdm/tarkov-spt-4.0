...
namespace TRL_SpeakFromTarkov.Core
{
    public class VoipController : MonoBehaviour
    {
        ...
        // AUD-03-11: guard one-shot pro poll de início de captura em Update() — substitui o
        // Postfix de EFT.Player.Init (removido, ver GameSessionPatcher.cs). Resetado em
        // ResetVoipCaptureStartedFlag() (chamado por GameWorldDisposePatch no fim da raid).
        private bool _voipCaptureStarted = false;
        ...
        void Update()
        {
            ...
            // AUD-03-11: substitui o Postfix de EFT.Player.Init — dispara 1x por raid quando o
            // jogador local termina de fato a inicialização (MainPlayer só é preenchido em
            // RegisterPlayer/GameWorld.cs:2277 quando IsYourPlayer, depois do Init estar completo).
        ...
        // AUD-03-11: chamado por GameWorldDisposePatch no fim da raid, pra que a próxima raid
        // volte a disparar StartVoipCapture()/SetGameStateChannel(true) no Update().
        public void ResetVoipCaptureStartedFlag() => _voipCaptureStarted = false;
