...
**Problema:** A seção [§1 Estratégia](001-stamina-e-velocidade-02-spec-tech.md#1-estratégia) e o stub de [`StanceStaminaDrainPatch.cs`](001-stamina-e-velocidade-02-spec-tech.md#5-stubs-de-código) ainda descrevem **Harmony postfix em `PlayerPhysicalClass.method_10`** para o modo Drain. Mas a spec funcional foi explicitamente alterada para Opção B (tick manual em hipfire) — ver [001-stamina-e-velocidade-01-spec.md §"Stamina"](001-stamina-e-velocidade-01-spec.md):
...
**Problema:** O stub de [`StanceStaminaRecoveryPatch.cs`](001-stamina-e-velocidade-02-spec-tech.md#5-stubs-de-código) faz:
...
**Problema:** A classe estática [`StanceStaminaState`](001-stamina-e-velocidade-02-spec-tech.md#5-stubs-de-código) tem 3 campos públicos mutáveis (`Mode`, `Intensity`, `IsSuspendedByProne`) mas nenhum `Reset()` ou `Initialize()`. Spec funcional exige:
...
1. Resolver os 5 🔴 bloqueadores na spec técnica (editar [001-stamina-e-velocidade-02-spec-tech.md](001-stamina-e-velocidade-02-spec-tech.md) inline). Para PA-01-01, isso significa reescrever §1 Estratégia, §2 Pontos de patch, §5 Stubs de código (substituir `StanceStaminaDrainPatch` por método tick em `StanceManager`) e §6 Fluxo de dados.
