...
**Reviews anteriores:** [01](001-stamina-e-velocidade-technical-review-01.md) · [02](001-stamina-e-velocidade-technical-review-02.md)
**Data:** 2026-05-08
...
**Resolução:** `Plugin._stanceConfigs` agora é `Dictionary<Stance, StanceConfig>`. `_stanceDefaults` é uma tupla com `Stance.Default/Stance1/Stance2/Stance3`. Métodos `OnStanceChanged(Stance, Stance)` / `ApplyStaminaStance(Stance)` / `GetActiveStaminaStance() → Stance`. Subseção [§1 "Modelo de stance"](001-stamina-e-velocidade-technical-spec.md#1-estratégia) explicita que "Stance 0" no F12 = `Stance.Default` interno.
...
**Resolução:** A property `CurrentStance` em [`StanceManager.cs:22`](../../modded/StanceManager.cs#L22) é modificada para detectar mudança e disparar `OnStanceChanged(prev, value)`. `private set` mantido — os 3 call-sites internos (linhas 94/111/116) não precisam mudar. Documentado em [§1 "Wiring da troca de stance"](001-stamina-e-velocidade-technical-spec.md#1-estratégia) e nos checklists de §4 e §8.
...
**Decisão:** `[x]` **Aceitar sugestão** · ✅ Resolvido em 2026-05-08
**Resolução:** Stub do `BaseLocalGameStopPatch.GetTargetMethod` em [§5](001-stamina-e-velocidade-technical-spec.md#5-stubs-de-código) agora usa `AccessTools.Method(typeof(BaseLocalGame), nameof(BaseLocalGame.Stop), new[] { typeof(string), typeof(ExitStatus), typeof(string), typeof(float) })`. Item adicionado ao checklist [§8](001-stamina-e-velocidade-technical-spec.md#8-checklist-de-implementação): "Logar no `Awake` se algum `MethodInfo` resolver para null".
...
**Decisão:** `[x]` **Aceitar sugestão** · ✅ Resolvido em 2026-05-08
**Resolução:** `OnRaidEnd` agora chama `ResetState()` ao final, consolidando o cleanup do nosso backlog com o cleanup pré-existente do mod (`CurrentStance`, tac sprint, dirty-flags de offsets). Subseção [§1 "Reuso do `ResetState()` existente"](001-stamina-e-velocidade-technical-spec.md#1-estratégia) explicita a integração.
...
`FieldInfo` e `MethodInfo` são cacheados em `static readonly` no `StanceManager`. Custo: 1 invoke/frame quando há drain ativo. Subseção [§1 "Granularidade do drain"](001-stamina-e-velocidade-technical-spec.md#1-estratégia) atualizada. Item adicionado ao checklist e nota em [§7](001-stamina-e-velocidade-technical-spec.md#7-riscos-e-dependências) sobre logar warning se `AccessTools.Field` retornar null (BSG renomeando esses campos privados em update futuro).
...
**Decisão:** `[x]` **Aceitar sugestão** · ✅ Resolvido em 2026-05-08
**Resolução:** Stub do `StanceStaminaRecoveryPatch` em [§5](001-stamina-e-velocidade-technical-spec.md#5-stubs-de-código) agora usa `if (gw.MainPlayer is HideoutPlayer) return;` direto, sem double-negação.
