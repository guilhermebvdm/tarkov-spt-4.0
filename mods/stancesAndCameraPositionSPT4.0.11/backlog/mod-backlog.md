...
| 018 | Correr agachado e rastejar rápido (crouch-run + high-crawl) | Segurar o sprint **nativo** (sem tecla nova) agachado ou em prone mantém a postura e acelera os passos, com ganho de velocidade gradual, animação de sprint forçada no agachado, piso de postura mínima e sobretaxa de stamina que respeita a skill. Faltam: clamp do teto contra a velocidade de sprint em pé e guarda de ADS/mount (ver checklist da spec técnica) — validação in-game pendente. | [018-rastejar-rapido/](./018-rastejar-rapido/) | 🟡 |
...
| 021 | Toggle para tombamento de mira lateral | Expõe no F12 um checkbox (default DESATIVADO) para preservar o tombamento nativo (`_scopeRotation`) de miras em trilho lateral/canted, hoje sempre descartado pelos patches de rotação de postura. | [021-toggle-tombamento-mira-lateral/](./021-toggle-tombamento-mira-lateral/) | 🟢 |
| 022 | Exposição de collider durante a transição de saída do prone | Investigar se dá pra mitigar a janela de hitbox "em pé" que `Prone2StandStateClass` aplica sempre ao sair do prone, independente da pose final — descoberto no item 018, não urgente. | [022-collider-transicao-prone/](./022-collider-transicao-prone/) | ⚪ |
| 023 | Atraso na checagem de carregador (Magazine Check Delay) | Atrasa a exibição do HUD de munição ao checar o carregador OU a câmara pra sincronizar com a animação de inspeção. Auditado retroativamente (spec → spec técnica → reviews → code-review); 3 achados de doc/logging aplicados. Falta validação in-game formal (5 testes listados na spec técnica §8). | [023-atraso-checagem-carregador/](./023-atraso-checagem-carregador/) | 🟡 |
