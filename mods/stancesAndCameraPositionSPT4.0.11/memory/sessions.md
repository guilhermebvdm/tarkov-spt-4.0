...
- ⚠️⚠️ **CORRIGIDO EM 2026-09-09 (Sessão 17) — este bullet estava ERRADO e já causou retrabalho real.** O fork ativo **NÃO é `modded`** — é **`modded-testchannel`** (criado na Sessão 15, 2026-08-02, como cópia de teste; recebeu desde então `HandsStateGuard`, compat com Climbable Ladders — Sessões 13/14 de 2026-09-05 — e todo o trabalho da Sessão 17). `modded/` ficou **travado em v2.17.0** e está sem essas features. Uma sessão inteira (17) implementou os itens 021 e 018 em `modded/` por confiar neste snapshot desatualizado, e teve que reverter tudo (`git restore`) e refazer em `modded-testchannel/` quando o usuário percebeu. **Antes de implementar qualquer coisa neste mod, confirmar o fork ativo com o usuário — não confiar cegamente neste arquivo.** Versão atual (`modded-testchannel`, pós-Sessão 17): **v2.20.1**. Ver Sessão 15 (origem do fork), Sessões 13/14 de 2026-09-05, e Sessão 17.
- **Histórico de versões pré-fork-testchannel (todas em `modded/`, hoje desatualizado — mantido só como histórico):** 2.11.0 (`22ae6176`, 26/07, envelope de comprimento no pacote de stance do FIKA) → 2.12.1 (`bada9d2a`, 27/07, chamber-check UI via `EftBattleUIScreen`) → 2.13.0/2.14.0 (Sessão 12) → 2.15.0-2.17.0 (Sessão 13, faxina + identidade TRL). ⚠️ Não existiu v2.12.0 (`PickupAimingSafetyPatch` entrou sem release próprio). Narrativa completa nas Sessões 12/14 antigas e `reference_stances_canonical_build`.
...
- **[P-17.1] (aberta 2026-09-09) 🔴 Re-validar prone-run in-game após o fix de detecção de sprint.** Usuário reportou 2x que a velocidade do prone-run não mudava (com a tecla já em movimento E com ela pressionada parado). Causa raiz encontrada: `ProneMoveStateClass.EnableSprint` só existe/roda em movimento, e não existe latch nativo pra prone (ao contrário do `Bool_1` de `RunStateClass`). Fix: nova fonte de sinal em `Player.ToggleSprint()`/`Player.EnableSprint(bool)`. Build `v2.20.1` compilada e entregue ao usuário — falta a confirmação de que funcionou. Ver item 018 do backlog, `018-rastejar-rapido-02-spec-tech.md` §1.8.
- **[P-17.2] (aberta 2026-09-09) 🟡 Mesma classe de bug do prone-run (P-17.1) pode existir no crouch-run** — segurar sprint agachado PARADO e só depois andar pode não ativar o boost de velocidade (o `Bool_1`-latch nativo resolve pose/stamina sozinho nesse caso, mas não passa pelo `CrouchRunEnableSprintPatch.Prefix`, que é quem seta `SprintKeyHeld`). Não reportado como quebrado pelo usuário (só testado em movimento) — documentado, não corrigido. Mesmo padrão de fix disponível se precisar.
- **[P-17.3] (aberta 2026-09-09) 🟡 Item 021 (toggle "Straighten Weapon On Canted Sights") implementado e compilado em `modded-testchannel` v2.20.1 — validação in-game não confirmada nesta sessão** (usuário só testou/confirmou o item 018).
- **[P-17.4] (aberta 2026-09-09) 🟡 Item 018: 2 lacunas conhecidas não implementadas** — (a) clamp do teto de velocidade do crouch/prone-run contra a velocidade real de sprint em pé (hoje só a faixa do slider 1.0–2.0× limita); (b) guarda de ADS/mount (hoje dá pra ativar crouch/prone-run mirando ou montado). Ver checklist §8 de `018-rastejar-rapido-02-spec-tech.md`.
- **[P-17.5] (aberta 2026-09-09) 🟢 Prompt de auditoria pronto para o mod `TRL-SpeakFromTarkov`** (criar `modded-V4` a partir de `modded-V3-audit`, mantendo a V3 estável) — entregue ao usuário nesta sessão, ainda não executado. Foco: DTX/VBR + Spatial Culling Host-Side (ganho de banda real, já no próprio backlog daquele mod) vs. uso de canal real do LiteNetLib (ganho marginal, depende de reimplementar formato de serialização privado do FIKA — ver achados abaixo). **Isso é de outro mod — não tem memória própria atualizada por esta sessão; só o ponteiro aqui.**
- ~~**[P-13.1]**~~ (aberta 2026-08-02) — **superada em 2026-09-09**: era sobre validar a pilha 2.13.0→2.17.0 em `modded/`, fork hoje abandonado (ver correção no topo). Sem efeito prático — `modded-testchannel` já está muitas versões à frente e validado em raid pelo usuário (item 018, Sessão 17).
...
## 2026-09-09 02:28 (GMT-3) — Sessão 17: Item 021 (toggle mira canted) + Item 018 (crouch/prone-run) — correção de fork + bug do prone

**Tema central:** Ciclo completo de backlog (spec → spec técnica → review → code-mod) para os itens 021 e 018, com uma correção de fork no meio (implementação inicial no fork errado) e um bug real de prone-run encontrado e corrigido a partir de teste do usuário.

**Decisões-chave:**
- **Item 021 criado, especificado e implementado:** toggle F12 `Straighten Weapon On Canted Sights` (default `false`) — antes o mod descartava incondicionalmente o tombamento nativo de miras em trilho lateral (`_targetScopeRotation`/`_scopeRotation`, `ProceduralWeaponAnimation.cs:276-278`) em `ApplyComplexRotationPatch`/`ApplySimpleRotationPatch`. Fix: reponta pro campo já suavizado (`_scopeRotation`) e compõe `weapRotation * effectiveScopeRotation * CurrentRotation` em vez de descartar. Nunca extrai `.eulerAngles` do scope (só multiplica como Quaternion) — é isso que evita o gimbal-flip que o comentário legado do próprio código já alertava.
- **Item 018 criado (correr agachado + rastejar rápido), investigado e implementado.** Achado técnico central: `RunStateClass.EnableSprint` (`RunStateClass.cs:401-420`) força `SetPoseLevel(1f)` incondicionalmente ao pedir sprint agachado — é isso que levanta o personagem. `ProneMoveStateClass.EnableSprint` **não** faz isso (só seta uma flag local `Bool_5`), então prone nunca levanta sozinho, mas também nunca acelera de verdade nativamente.
- **⚠️ Correção de fork (custou uma implementação inteira):** os itens 021 e 018 foram implementados primeiro em `modded/`, seguindo o que o topo desta própria memória dizia ("Fork ativo = modded"). O usuário apontou que o fork correto é `modded-testchannel/` — `modded` está travado em v2.17.0, sem `HandsStateGuard`/compat de escadas que `modded-testchannel` já tem desde as Sessões 15/13/14. Revertido `git restore` em `modded/` e reaplicado tudo em `modded-testchannel/`. Ver correção no topo desta memória.
- **Bug real do prone-run encontrado por teste do usuário e corrigido.** Causa raiz: `ProneMoveStateClass.EnableSprint` só existe enquanto `CurrentManagedState` já é essa classe (ou seja, já em movimento); `ProneIdleStateClass` (prone parado) não a sobrescreve; e não existe latch nativo equivalente ao `Bool_1` de `RunStateClass` pra prone. Fix: nova fonte de sinal em `Player.ToggleSprint()`/`Player.EnableSprint(bool)` (`Player.cs:26000-26012`) — os dois métodos por onde TODO input de sprint do jogador passa (confirmado em `Class1728.cs:99-104`, o roteador de `ECommand`), independente de qual sub-estado esteja ativo.
- **Simplificações pedidas pelo usuário após teste in-game:** removida a sobretaxa de stamina do crouch-run (confirmado que `PlayerPhysicalClass.Sprint()`/dreno nativo já se aplica sozinho, porque o Prefix do crouch chama `movementContext.EnableSprint()`); mantida a do prone-run (confirmado no Assembly que o jogo genuinamente não cobra stamina nativa ali); removida a rampa de ganho de velocidade gradual das duas posturas (usuário achou sem sentido no mod, depois de eu tê-la implementado a pedido dele mesmo numa iteração anterior da mesma sessão).
- **Investigação de "canal de rede" (LiteNetLib) motivada por uma dúvida do usuário sobre a arquitetura "Canal 3 TRL".** Confirmado no código-fonte real do FIKA (`references/fika-plugin/`): `IFikaNetworkManager.SendData` não tem parâmetro de canal; internamente cai sempre no canal 0 padrão do LiteNetLib. O FIKA sobe `ChannelsCount = 2` (`FikaClient.cs:145`/`FikaServer.cs:160`) mas **nunca usa o canal 1** — está livre. `NetPeer.Send(data, channelNumber, deliveryMethod)` é público e aceitaria canal explícito; `FikaClient.ServerConnection` (o `NetPeer` cru) também é público. Usar o canal 1 de verdade exigiria reimplementar o formato de bytes privado que o `FikaClient`/`FikaServer` gravam (`_dataWriter`/`_packetProcessor` são privados) — dependência de internals não documentados do FIKA, frágil a updates.

**Lições / hipóteses descartadas:**
- **Hipótese descartada:** "o snapshot do topo desta memória (`Fork ativo = modded`) está atualizado" — estava desatualizado desde a Sessão 15 (2026-08-02) e causou retrabalho real nesta sessão. **Lição institucional:** antes de implementar em QUALQUER mod com múltiplos forks (`modded`, `modded-beta`, `modded-testchannel`, `modded-V2/V3/...`), confirmar com o usuário qual é o ativo, mesmo que a memória pareça dizer o contrário — comparar versão do `.csproj`/`BepInPlugin` entre forks candidatos é um jeito rápido de flagrar divergência real (2.17.0 vs. 2.19.16 já teria denunciado o problema).
- **Hipótese descartada:** "`ProneMoveStateClass.EnableSprint` é um sinal confiável pra saber se o jogador quer correr em prone" — falhou porque esse método só roda em movimento, sem latch nativo pro caso parado. Substituído por patchear `Player.ToggleSprint()`/`Player.EnableSprint(bool)`, um nível acima da hierarquia de sub-estados de movimento.
- **Hipótese descartada:** "o 'Canal 3 TRL' isola tráfego de rede de verdade" — reconfirma o que a própria Sessão 12 (2026-07-29) já tinha decidido (canal exclusivo por mod foi descartado a favor de assinatura compartilhada) — não é um canal LiteNetLib real, é uma convenção de magic header entre pacotes que continuam todos no canal 0.

**Atividade cronológica:**
1. Investigação da feature "mira inclinada" em `modded-testchannel` (a pedido do usuário) → confirmado no Assembly que o mod descarta o tombamento nativo → item 021 criado e levado ao ciclo completo (spec → spec técnica → review → code-mod).
2. Item 018 criado a partir de uma ideia antiga do backlog (só prone) + pedido do usuário de expandir pra agachado, com investigação técnica prévia (à mão, antes do ciclo formal) confirmando que sprint agachado força stand-up e prone não.
3. Ciclo completo do item 018 (spec, spec técnica com 2 rodadas de review técnica, code-mod) — implementado primeiro em `modded/` (erro de fork).
4. Usuário aponta o fork errado → revert + reimplementação em `modded-testchannel/`.
5. Usuário pede 3 refinamentos em sequência (ganho gradual de velocidade, animação de sprint forçada no crouch, piso de postura mínima) — cada um investigado no Assembly e implementado, com 1 review técnica adicional corrigindo o uso de `force: true` (seria um "pop" visual) para `force: false`.
6. Usuário testa in-game: crouch-run funcionou; prone-run não mudou a velocidade. Também pede pra reavaliar a sobretaxa de stamina e remover a rampa.
7. Causa raiz do bug do prone investigada e corrigida; sobretaxa/rampa ajustadas conforme pedido; recompilado (v2.20.1).
8. Discussão paralela sobre a arquitetura de rede "Canal 3" deste mod e do `TRL-SpeakFromTarkov` (investigação read-only, sem mudança de código) — terminou num prompt de auditoria preparado para uma sessão futura no outro mod.

**Pendências abertas nesta sessão:**
- [P-17.1] (aberta 2026-09-09) 🔴 Re-validar prone-run in-game após o fix.
- [P-17.2] (aberta 2026-09-09) 🟡 Mesmo tipo de bug pode existir no crouch-run (não confirmado, não corrigido).
- [P-17.3] (aberta 2026-09-09) 🟡 Item 021 sem validação in-game confirmada.
- [P-17.4] (aberta 2026-09-09) 🟡 Item 018: clamp de velocidade e guarda de ADS/mount ainda faltam.
- [P-17.5] (aberta 2026-09-09) 🟢 Prompt de auditoria do `TRL-SpeakFromTarkov` (modded-V4) entregue, não executado.

**Cross-refs:**
- Corrige o snapshot desatualizado desde a Sessão 15 (2026-08-02).
- Reconfirma a decisão de rede da Sessão 12 (2026-07-29) sobre o "Canal 3 TRL".
- Trabalho paralelo no mesmo dia (fora deste mod): investigação read-only + prompt de auditoria para `TRL-SpeakFromTarkov` — sem gravação na memória daquele mod nesta sessão (ver [P-17.5]).
