...
## Mudanças nossas (fork)

> Esta versão acrescenta uma feature de **gestão de stamina e velocidade por stance** sobre o mod original. Backlog: `001-stamina-e-velocidade`.

### Stamina e Velocidade por Stance

Cada stance (`Stance 0` = pronto-de-tiro vanilla, `Stance 1` = pronto baixo, `Stance 2` = coringa, `Stance 3` = pronto alto) ganhou 5 propriedades novas no F12:

- **`Stance N Stamina Mode`** — `None` / `Drain` / `Recovery` (mutuamente exclusivos por stance).
  - `Drain` consome stamina das mãos enquanto a stance está ativa **em hipfire** (em ADS o EFT vanilla cuida).
  - `Recovery` acelera a regeneração de stamina das mãos **em hipfire**.
- **`Stance N Stamina Intensity`** — multiplicador 0.0–2.0 do efeito.
- **`Stance N Modifies Movement Speed`** + **`Stance N Movement Speed Multiplier`** — redutor 50%–100% de velocidade (apenas redução; >100% não é suportado pelo sistema do EFT).
- **`Stance N Apply When Prone`** — toggle se a stance aplica mesmo deitado.

**Defaults** (calibrados pra dar curva de gameplay coerente):

| Stance | Mode | Intensity | Speed % |
|---|---|---|---|
| 0 (vanilla) | Drain | 0.50 | 90% |
| 1 (pronto baixo) | Recovery | 2.00 | 100% |
| 2 (coringa) | None | 1.00 | 100% |
| 3 (pronto alto) | Recovery | 1.50 | 95% |

### ⚠️ Compatibilidade

- **Reserva técnica:** este mod registra um speed limit usando `(Player.ESpeedLimit)9001` — int fora dos valores oficiais da enum. Outros mods que usem este mesmo cast colidem.
- **Mods de stamina externos** (ex.: SPT-BetterArmStamina): nosso postfix de Recovery roda com `[HarmonyPriority(Priority.Low)]` (depois dos demais). Os efeitos podem se compor multiplicativamente.
- **Mods de velocidade**: outros redutores via `MovementContext.AddStateSpeedLimit` com causes oficiais (`Weight`, `Swamp`, etc.) não colidem — o sistema pega o `min`.
...
- ~~Ending tactical sprint causes weapon to warp towards low sprint position~~ Fixed with a configurable delay
