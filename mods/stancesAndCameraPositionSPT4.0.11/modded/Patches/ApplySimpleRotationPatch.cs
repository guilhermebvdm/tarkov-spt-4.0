...
namespace CameraRotationMod.Patches
{
    public class ApplySimpleRotationPatch : ModulePatch
    {
        ...
        private static Vector3 ClampAngles(Vector3 e, ref Vector3 vel, float max)
        {
            ...
            {
                _clampLogBudget--;
                Plugin.Logger.LogWarning($"[STANCE-CLAMP] ApplySimple: spring overshoot batido no limite seguro ({max}°) — divergência da mola contida (causa do flip de câmera). euler={e}");
            }
            return e;
        }
