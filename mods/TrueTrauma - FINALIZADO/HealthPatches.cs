...
namespace TrueTrauma
{
    ...
    public static class DamageTriggerPatch
    {
        ...
        static void Postfix(Player __instance, DamageInfoStruct damageInfo, EBodyPart bodyPartType)
        {
            ...
            // 1. LÓGICA DE DESMAIO
            if (TrueTraumaPlugin.ConfigBlackoutEnabled.Value)
            {
            ...
            if (TrueTraumaPlugin.ConfigStomachEnabled.Value)
            {
            ...
            if (TrueTraumaPlugin.ConfigLegsEnabled.Value && (bodyPartType == EBodyPart.LeftLeg || bodyPartType == EBodyPart.RightLeg))
            {
            ...
            if (TrueTraumaPlugin.ConfigArmsEnabled.Value && (bodyPartType == EBodyPart.LeftArm || bodyPartType == EBodyPart.RightArm))
            {
                if (HealthUtils.IsPartDestroyed(__instance, bodyPartType))
                {
