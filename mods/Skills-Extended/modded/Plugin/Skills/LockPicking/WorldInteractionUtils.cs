...
public static class WorldInteractionUtils
{
    ...
    {
        if (interactiveObject.DoorState != EDoorState.Locked || !interactiveObject.Operatable || !SkillsExtendedPlugin.Keys.KeyLocale.ContainsKey(interactiveObject.KeyId))
        {
            return false;
        }
    ...
    private sealed class LockInspectInteraction
    {
        ...
        public void TryInspectLock()
        {
            ...
            SkillsExtendedPlugin.Log.LogError($"Missing locale data for door {interactiveObject.Id} and key {interactiveObject.KeyId}");
        }
    }
}
