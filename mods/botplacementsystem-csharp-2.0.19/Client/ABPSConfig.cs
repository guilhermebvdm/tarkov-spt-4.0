...
internal static class AbpsConfig
{
    ...
    private const string BossConfig = "2. Boss Settings";
    ...
    private const string DespawnConfig = "5. Despawn Settings";
    ...
    private static ConfigEntry<double> _connectedPlayerTimeout;
    private static ConfigEntry<int> _connectedPlayerCheckTimer;
    ...
    public static void InitAbpsConfig(ConfigFile config)
    {
        ...
        _connectedPlayerTimeout = config.Bind(
            DebugConfig,
            "Connected Player Timeout",
            3.0,
            new ConfigDescription("Timeout for waiting on connected players for ABPS to use their position data.\nIf this value times out, then bots won't get the players added to their enemy list nor will they spawn correctly distanced.",
                new AcceptableValueRange<double>(0.5, 10.0),
                new ConfigurationManagerAttributes { IsAdvanced = true, Order = _loadOrder-- }));
        Plugin.ConnectedPlayerTimeout = _connectedPlayerTimeout.Value;
        _connectedPlayerTimeout.SettingChanged += ABPS_SettingChanged;
            
        _connectedPlayerCheckTimer = config.Bind(
            DebugConfig,
            "Connected Player Check Timer",
            250,
            new ConfigDescription("How often it checks for connected players on raid load in milliseconds. This will only count up to the timeout value.\nMake this higher if players aren't loading before spawns start.\nYou will only know this if you enable debug logging and read the logs.",
                new AcceptableValueRange<int>(100, 3000),
                new ConfigurationManagerAttributes { IsAdvanced = true, Order = _loadOrder-- }));
        Plugin.ConnectedPlayerCheckTimer = _connectedPlayerCheckTimer.Value;
        _connectedPlayerCheckTimer.SettingChanged += ABPS_SettingChanged;
    }
