        ...
        private static bool PatchPrefix(InputClass2 __instance, ECommand command)
        {
            ...
            if (command == ECommand.WeaponMounting && PluginConfig.OverrideMounting.Value && Plugin.ServerConfig.enable_stances)
            {
                ...
                if (StanceController.IsBracing && !StanceController.IsColliding)
                {
                    ...
                    StanceController.DoWiggleEffects(player, pwa, fc.Weapon, StanceController.IsMounting ? StanceController.CoverWiggleDirection : StanceController.CoverWiggleDirection * -1f, true, 1f, useGearSound: true);
                }
