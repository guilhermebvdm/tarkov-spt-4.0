...
public class PuppetMasterSettings : Singleton<PuppetMasterSettings>
{
	...
	public class PuppetUpdateLimit
	{
		...
		public bool Update(List<PuppetMaster> puppets, PuppetMaster puppetMaster)
		{
			...
			for (int i = 0; i < puppetsPerFrame; i++)
			{
				...
				if ((Object)(object)puppets[num] == (Object)(object)puppetMaster)
				{
					return true;
				}
			}
	...
	private void Update()
	{
		...
		{
			if (puppet.isActive && ((Behaviour)puppet).isActiveAndEnabled)
			{
			...
			if ((puppet.mode == PuppetMaster.Mode.Disabled && !puppet.isActive) || !((Behaviour)puppet).isActiveAndEnabled)
			{
