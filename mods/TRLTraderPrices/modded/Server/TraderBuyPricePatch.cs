...
/// <summary>
///     B-3 Rota B — server backstop. Harmony Prefix on <c>TradeHelper.SellItem</c> (the sell-to-trader
///     path — spt-source Helpers/TradeHelper.cs:251). The vanilla method credits the player with
///     <c>sellRequest.Price</c> exactly AS SENT BY THE CLIENT (PaymentService.GiveProfileMoney,
///     PaymentService.cs:193, round-trips it through the TRADER's own currency — no cross-currency
///     conversion); the server never recomputes a buyback price itself.
///     <para>
///     If EVERY sold item has a matching (traderId, tpl) entry in
///     <see cref="TRLTraderPricesMod.BuyOverrides"/>, denominated in the trader's own currency, this
...
///     (<c>modded/Client/Patches/TraderBuyPricePatch.cs</c>, <c>TraderClass.GetUserItemPrice</c>)
...
///     <see cref="TRLTraderPricesMod.BuyOverrides"/>). The client sends ONE aggregate price for the
...
///     the original method removes them (TradeHelper.cs:291) — reading after would find nothing.
///     Harmony is required here: <c>SellItem</c> is not virtual, so DI's <c>typeOverride</c> registration
///     cannot intercept it (same reasoning as OutfitPersistenceFix's Harmony patch).
/// </summary>
[HarmonyPatch(typeof(TradeHelper), nameof(TradeHelper.SellItem))]
internal static class TraderBuyPricePatch
{
    [HarmonyPrefix]
    private static void Prefix(PmcData profileWithItemsToSell, ProcessSellTradeRequestData sellRequest)
    {
        try
        {
            var overrides = TRLTraderPricesMod.BuyOverrides;
            ...
            var trader = TRLTraderPricesMod.Db?.GetTraders()?.GetValueOrDefault(sellRequest.TransactionId);
            ...
            TRLTraderPricesMod.Log?.Debug(
                $"[TRLTraderPrices] buy price backstop: trader {sellRequest.TransactionId} -> {total} ({traderCurrency}) for {sellRequest.Items.Count} item(s).");
        }
        catch (Exception ex)
        {
            TRLTraderPricesMod.Log?.Error("[TRLTraderPrices] buy price backstop failed: " + ex);
        }
    }
}
