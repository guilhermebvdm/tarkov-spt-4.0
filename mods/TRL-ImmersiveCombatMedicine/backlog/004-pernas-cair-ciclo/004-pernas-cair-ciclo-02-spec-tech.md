...
6. **Levantar lento (LIBERAÇÃO — P5):** ao expirar o bloqueio, fase LIBERAÇÃO estável (sem timers). No primeiro get-up: `reentryFlag` + `SetPoseLevel(0f, force: true)` p/ a saída de prone terminar AGACHADA; detecção da saída por polling `IsInPronePose == false` → voz leve + rampa por frame `SetPoseLevel(Mathf.MoveTowards(PoseLevel, PoseMemo, dt/SlowRiseSeconds))` até `PoseMemo` ([Player.cs:23912](../../../../references/eft-decompiled/Assembly-CSharp/EFT/Player.cs#L23912); a suavização vanilla converge rápido demais p/ servir de knob — [MovementContext.cs:2208-2245](../../../../references/eft-decompiled/Assembly-CSharp/EFT/MovementContext.cs#L2208)). `SlowRiseSeconds` = const interna `1.5f` (decisão 13 cobre só os 3 timers — premissa nova, item 011). **"De pé efetivo" (início da JANELA) = fim da rampa** (pose alvo atingida); sem rampa (estabelecimento, analgésico destravando levantar em curso) = jogador já de pé (premissa técnica — item 011). Meio-levantar visual da tentativa frustrada (decisão 6): **fora do default** — a Rodada 2 do P5 rebaixou o veredito a "indeterminado sem protótipo runtime"; o 004 entrega só voz (fallback aceito da funcional), experimento fica registrado p/ item 011.
7. **Cap N2 da JANELA — reuso por EXTRAÇÃO de helper:** `ApplyCap`/`RemoveCapGuarded` do 003 ([TraumaLegsConsumer.cs:116-152](../../modded/Patches/Trauma/TraumaLegsConsumer.cs)) migram para um estático compartilhado `TraumaSpeedCap` (mesma causa `(ESpeedLimit)1000`, mesmo Remove+Add — `AddStateSpeedLimit` é no-op com causa existente, [MovementContext.cs:1672-1679](../../../../references/eft-decompiled/Assembly-CSharp/EFT/MovementContext.cs#L1672); recompute único :2553-2558), com bookkeeping por consumidor. Sem escrita dupla: a linha de perna é exclusiva por jogador (003 mapeia N1/N2/Crouch+N2; 004 mapeia SÓ FallCycle) e o cap da janela **independe do toggle 003** (helper não consulta `TraumaLegsConsumer.IsActive`). Interim removido: `IsN2Tier` do 003 deixa de incluir `LegsFallCycle` ([TraumaLegsConsumer.cs:59-64](../../modded/Patches/Trauma/TraumaLegsConsumer.cs)) e o `OnTransition` do 003 ignora a linha. Gate de sprint estendido: `CanSprintPatch` também força `false` quando `GetLine == LegsFallCycle` + consumidor 004 ativo ([SpeedLimitPatches.cs:15-31](../../modded/Patches/Trauma/SpeedLimitPatches.cs)) — cumpre o contrato "cap N2 bloqueia sprint" na JANELA (prone/bloqueio não sprintam por natureza).
...
| Cap N2 | `TraumaSpeedCap` (extraído do 003 — `AddStateSpeedLimit`/`RemoveStateSpeedLimit` [MovementContext.cs:1672/:1790](../../../../references/eft-decompiled/Assembly-CSharp/EFT/MovementContext.cs#L1672), `MaxSpeed` :910, `UpdateSpeedLimitByHealth` [Player.cs:29068](../../../../references/eft-decompiled/Assembly-CSharp/EFT/Player.cs#L29068)) | Mancar N2 na JANELA, independente do toggle 003 |
...
| `modded/Patches/Trauma/TraumaSpeedCap.cs` | CRIAR | Helper extraído do 003: causa `(ESpeedLimit)1000`, ApplyCap (Remove+Add, log calibração), RemoveCapGuarded (downed-safe), `LineTargetPercent`; consumido por 003 (N1/N2) e 004 (janela). |
| `modded/Patches/Trauma/TraumaPose.cs` | MODIFICAR | (1) `TryInvoluntaryFall` (prone force + readback + fallback agachado + PronePending re-try); (2) fila de adiados ganha `Internal` (re-quedas sem refund) e dispatch por kind no pump; (3) absorção D2 no topo de `TryInvoluntaryCrouch`/`BotCrouchDip` (consulta `IsCycleEngaged` → refund + log ABSORB). |
| `modded/Patches/Trauma/TraumaLegsConsumer.cs` | MODIFICAR | Interim REMOVIDO: `IsN2Tier` sem `LegsFallCycle`; `OnTransition` ignora a linha Cair (004 assume); ApplyCap/RemoveCapGuarded delegam ao `TraumaSpeedCap` (comportamento idêntico). |
...
| `modded/Patches/Trauma/InputPatches.cs` | MODIFICAR | `CantStandUpPatch` ganha o branch condicional do ciclo (BLOQUEIO; prone OU h>PoseLevel no fallback; IsYourPlayer; reentry flag); branch de blackout intacto. Patch NOVO `FallAttemptCommandPatch` (TranslateCommand, detecção-only). |
...
| `modded/TRL-ImmersiveCombatMedicine.csproj` | MODIFICAR | Referência `DrakiaXYZ-BigBrain.dll` (`Private=false`; resolvida pelo compile-mod de D:\SPT\BepInEx\plugins). |
...
namespace TRLImmersiveCombatMedicine.Trauma
{
    ...
    {
        private enum FallPhase { None, Blocked, Released, Rising, Window, Paused }
        ...
        private float _riseTarget;              // rampa do levantar lento → PoseMemo (ref: Player.cs:23912)
        ...
        private void OnTransition(TraumaTransition t)
        {
            ...
            {
                // Entrada/estabelecimento. Establishing (spawn ferido/religar/adoção) = JANELA sem one-shot/toast;
                // já prone nesse instante → BLOQUEIO (item 5 da funcional). Entrada com one-shot SUPRIMIDO pelo
                // cooldown do motor (thrash de analgésico) chega SÓ como transição → engaja em JANELA (corner).
                // Troca de linha DENTRO do ciclo não passa aqui (From==To não publica — TraumaEngine.cs:548).
                if (_phase == FallPhase.None) Engage(p, t.Establishing);
            }
        ...
        private void OnOneShot(Player p, TraumaOneShotKind kind, TraumaLine line)
        {
            ...
            // Derrubar de entrada: TraumaPose executa | adia (D7) | fallback agachado; EXECUTED → EnterBlocked + voz forte
        ...
        {
            // TraumaSpeedCap.Apply(_local, percentN2efetivo) — mesma causa 1000, Remove+Add, log de calibração
            // (ref: MovementContext.cs:1672/:1790/:910; padrão TraumaLegsConsumer.ApplyCap do 003). _capApplied = true.
        }

        private void RemoveWindowCap()
        {
            // if (_capApplied) { TraumaSpeedCap.RemoveGuarded(_local); _capApplied = false; } — downed-safe (003 CR)
        }
        ...
        private void TickHumanCycle()
        {
            ...
            switch (_phase)
            {
                ...
                case FallPhase.Released:
                    if (!mc.IsInPronePose)
                    {
                        // Saída de prone detectada (polling — P5): termina AGACHADO e rampa lenta até PoseMemo
                        StandReentryFlag = true;
                        mc.SetPoseLevel(0f, true);          // get-up termina agachado — ref: MovementContext.cs:2139
                        _riseTarget = 0f;
                        _phase = FallPhase.Rising;
                ...
                case FallPhase.Rising:
                    // Levantar lento: rampa do TARGET por frame (P5 — suavização vanilla converge rápido demais)
                    _riseTarget = Mathf.MoveTowards(_riseTarget, p.PoseMemo, Time.deltaTime / SlowRiseSeconds); // ref: Player.cs:23912
                    mc.SetPoseLevel(_riseTarget);
                    if (_riseTarget >= p.PoseMemo - 0.01f)
                    {
                        StandReentryFlag = false;
                        EnterWindow("rose"); // "de pé efetivo" = fim da rampa (premissa técnica — item 011)
                    }
                    break;
...
//   (GetLine == RequiredLine; mudou → CANCELA com refund SÓ se !Internal). PronePending: entradas do fallback
//   re-tentam prone a cada pump enquanto o ciclo persistir (guard D7 + CanProne re-avaliados).
...
namespace TRLImmersiveCombatMedicine.Trauma
{
    ...
    {
        private sealed class Hold { internal Player Player; internal float ReleaseAt; internal bool Released; }
        ...
        {
            // to != LegsFallCycle (cura/analgésico/None) → release imediato SEM re-hold ("a IA levanta sem re-derrubada")
        }
        ...
        {
            // 1. ReleaseAt vencido → Released=true (camada IsActive=false → árbitro devolve — D14)
        ...
        internal static void ReleaseAll(string reason) { /* toggle-off: NextPosibleGetUp=0 via camada Stop(); _holds.Clear() */ }
    ...
    internal class TraumaDownedLayer : CustomLayer
    {
        ...
        {
            BotOwner.BotLay.NextPosibleGetUp = 0f;
            BotOwner.BotLay.GetUp(false);                                  // devolução — próxima camada elegível assume no tick (D14)
        }
    }
...
namespace TRLImmersiveCombatMedicine.Trauma
{
    ...
    internal static class TraumaVoice
    {
        ...
        /// <summary>LEVE (liberação): OnBeingHurt demand:true (humano local tem OnDemandOnly=true — Player.cs:28668).</summary>
...
[TraumaFallCycleConsumer]
        ...
        │      readback falhou → fallback AGACHADO + PronePending (re-try no pump); D7 → Defer (cancel = refund)
        ...
        ├─ JANELA 3s: cap N2 via TraumaSpeedCap (causa 1000, min-composição) + CanSprint=false;
...
[TraumaBotFall: hold X] — camada BigBrain prio 90 (IsActive=IsHeld) → Start(): BotLay.IsLay=true +
        ...
        │ X expira → IsActive=false → Stop(): NextPosibleGetUp=0 + GetUp(false) → árbitro devolve (D14)
...
1. **"De pé efetivo" = fim da rampa do levantar lento** (não a saída do prone): a JANELA começa com o jogador em pose alvo — leitura mais fiel de "saída do prone concluída" e evita janela consumida pela própria animação. Premissa técnica p/ item 011. Confirmar.
...
3. **Extração do `TraumaSpeedCap`:** refactor toca o 003 entregue (delegação sem mudança de comportamento). Alternativa sem refactor — duplicar Apply/Remove no 004 — foi descartada (dois escritores da mesma causa 1000 = risco de undo cruzado). Confirmar o refactor.
...
- [ ] `TraumaSpeedCap.cs`: extrair Apply/RemoveGuarded/LineTargetPercent do 003 (comportamento idêntico, logs mantidos); `TraumaLegsConsumer` delega.
- [ ] `TraumaPose.cs`: `TryInvoluntaryFall` (prone+readback+fallback+PronePending) + fila com `Internal`/callback + dispatch por kind no pump + `AbsorbIfCycleEngaged` no topo dos caminhos de agachar + `CancelFallsFor`.
- [ ] `TraumaVoice.cs`: forte/leve tipadas + anti-spam 2s.
- [ ] `TraumaFallCycleConsumer.cs`: FSM (deadlines absolutos; establishing=JANELA/prone→BLOQUEIO; pausa blackout/DOWNED; wake→BLOQUEIO; END em line-exit ≤1 frame) + cap da janela + sweeps/world-swap + edges do toggle.
- [ ] `TraumaBotFall.cs`: camada+logic+manager (hold X, re-hold interno, release por cura, ClearAll/ReleaseAll, sweep CR-01-02) + `RegisterLayer` gateado + interop SAIN.
- [ ] `InputPatches.cs`: branch do ciclo no `CantStandUpPatch` (reentry flag; prone qualquer h; fallback só subida; IsYourPlayer) + `FallAttemptCommandPatch` detecção-only.
- [ ] `SpeedLimitPatches.cs`: OR do FallCycle no `CanSprintPatch`; re-log ciente do cap da janela.
- [ ] `TraumaLegsConsumer.cs`: interim REMOVIDO (`IsN2Tier` sem FallCycle; `OnTransition` ignora a linha Cair).
- [ ] Plugin: binds seção 8 + rename `Fall Cycle` (ON) + órfã deletada em `MigrateOrphanedConfigKeys` + `AddComponent` + `RegisterLayer`; csproj com BigBrain (`Private=false`; conferir mapa do compile-mod.sh).
- [ ] `PROPRIEDADES.md` (seção 8 + tabela Renomeadas) + `/update-mod-graph` no commit da entrega.
- [ ] Smoke test (greps por AC): AC1 ciclo completo (fall EXECUTED → Blocked → attempt BLOCKED 1 som/2s → Released → Rising → Window → re-queda interna; timers F12 mudados valem na fase SEGUINTE); AC2 analgésico no bloqueio → END ≤1s + levanta livre; expiração → cai ≤1s; AC3 cura 1 fratura → END + linha nova via 003; AC4 D2 (estômago zerado com ciclo ativo → `crouch ABSORB (fall-cycle)`, sem execução) + desmaio pausa/wake→Blocked; AC5 bot: HOLD ≥X sem atirar/girar, RE-HOLD ao levantar, cura → release limpo, headless (log); AC6 interim: linha Cair sem cap N2 permanente fora da JANELA (log 003 sem FallCycle); AC7 Fika: peer vê queda/prone/levantar e OUVE forte/leve + tentativa (voz nativa); espelho sem efeito próprio; AC8 reset entre raids + spawn ferido → JANELA sem toast/one-shot.
