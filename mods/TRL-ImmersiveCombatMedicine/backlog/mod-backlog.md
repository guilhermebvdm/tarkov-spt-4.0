...
| 011 | Matriz de comportamento total (não-técnica) + plano de teste | Documento no espírito da planilha original do usuário, consolidando TODOS os cenários previstos (matriz × decisões × defaults × interims) em linguagem não-técnica — fonte da verdade dos critérios de aceite. Alimenta: plano de teste estruturado + code-review dedicado de validação de ACs (matriz ↔ código). Escrito ao FIM do overhaul (após 008), antes do 009. | [011-matriz-comportamento/](./011-matriz-comportamento/) | ⚪ |
