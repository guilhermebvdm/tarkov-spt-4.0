...
6. **Lockout de re-ADS (decisão 17 — P9):** prefix em `Player.FirearmController.SetAim(bool)` que retorna `false` quando `value==true` + lockout ativo + humano local — **bloqueia TODAS as rotas de re-entrada** porque todas convergem no funil: input/ToggleAim (:13701), troca de scope `SetAim(int)` (:13705-13709), re-aim automático de fast-slot (:10670-10673), restauração pós-overlap (:13067-13070). Lockout guardado por **ProfileId do jogador** (persiste à troca de arma — é do jogador, não da arma); `lockoutUntil = Time.time + cfg` setado no cancel. **Invisível ao peer:** o prefix impede a mudança de `IsAiming` e o flag `AimingInterruptedByOverlap` (limpo pelo nosso `SetAim(false)`, :13729; só re-setável com `IsAiming==true`, :13062) permanece false → o FikaClientFirearmController não emite pacote (correção da verificação do P9). **Voz de dor da tentativa — mecanismo prioritário do P5, canal UNIFICADO com o 004 (decisão PA-02-03):** o 004 entregou `TraumaVoice` ([TraumaVoice.cs](../../modded/Patches/Trauma/TraumaVoice.cs)) com `PlayStrong` no MESMO `OnAgony` importance:100 e doc "Reusável pelo 005" (:7-9) — mas o reuso ingênuo de `PlayStrong` VIOLARIA a PA-01-02: o anti-spam fixo de 2 s é consumido ANTES do Play e o retorno é IGNORADO (TraumaVoice.cs:33-39,:21), e cooldown fixo 2 s ≠ janela de lockout 1,0–1,5 s. Decisão: **estender `TraumaVoice` com `TryPlayStrong(Player) : bool` accept-gated** — retorna `p.Speaker?.Play(...) != null`, SEM tocar o anti-spam interno (o throttle do 005 vive no consumidor: canal separado por consumidor; `PlayStrong`/`PlayLight`/`Allowed`/`Clear` do 004 INTOCADOS → zero mudança de comportamento do 004) — centralizando trigger/tags num ponto só e **alinhando as tags ao 004** (`ETagStatus.Combat | ETagStatus.Dying` — TraumaVoice.cs:21; o `Combat` puro da versão anterior desta spec divergia sem razão). O Play fura o Busy do Speaker (PhraseSpeakerClass.cs:206-227; `Player.Say` com demand NÃO fura, :28799-28829), audível aos peers via PhrasePacket (fika FikaPlayer.cs:1093-1103). **Dupla fonte de OnAgony no mesmo Speaker:** o 004 emite em queda executada (TraumaFallCycleConsumer.cs:124) e tentativa de levantar negada (InputPatches.cs:104) — a chamada do 005 pode ser engolida pelo Busy≥100 DELE (o retry abaixo cobre) e AMBOS alimentam o Blocker de grupo; premissa registrada p/ o item 011 (reconciliação dos canais de voz — rastro P-3.4). **Throttle: 1 voz por janela de lockout** (flag `voicePlayed` zerado a cada cancel — cobre modo hold sem spam, premissa P-005-A); **o flag só é consumido se `TryPlayStrong` retornar true** — dois gates ENGOLEM a chamada mesmo com importance=100: Busy com outra frase importance ≥100 em curso (`100 <= Int_0` → skip, PhraseSpeakerClass.cs:206-210) e Blocker de GRUPO de fala (`SpeakerManager.FreeToSpeak` — SpeakerManager.cs:106-131 + GClass2291:50-59, sem bypass por demand/importance; squads coop alimentam o Blocker, inclusive o OnAgony do item 004); engolido → **piso de re-tentativa de voz** `_nextVoiceTryAt = Time.time + 0.3f` (tentativas antes do piso não chamam Play nem logam — PA-02-06: a P-005-A não está provada e, se hold re-disparar `SetAim(true)` POR FRAME sob Blocker, seriam ~90-135 `Speaker.Play`+logs em 1,5 s; o piso limita o pior caso a ~5 Plays/janela) e log `voice=skipped(busy|blocked)` no MÁXIMO 1×/janela (flag `_lockoutVoiceSkipLogged`, separado do `voicePlayed`); a próxima tentativa APÓS o piso re-tenta (PA-01-02 — re-tentativa em cadência humana segue tocando na primeira janela livre) + **guard de incapacidade** (sem voz durante inconsciência/downed — espelho DECLARADO do predicado canônico do 004, TraumaFallCycleConsumer.cs:235-236: `TraumaState.BlackoutTimers.ContainsKey(p.ProfileId) || TraumaState.IsFainted || p.HealthController == null || !p.HealthController.IsAlive` → sem voz, mas o BLOQUEIO do re-ADS continua — PA-02-05; cobre a janela de frames do wake em que `BlackoutTimers` já saiu e `IsFainted` ainda não limpou, e o downed do Fika sem entrada em `BlackoutTimers`; o SilenceVoicePatch legado só cobre `Player.Say`, não `Speaker.Play` direto). Colateral cosmético aceito: `ToggleAim` executa `RemoveLeftHandItem`/`SetCompassState(false)` ANTES do SetAim bloqueado — bússola/mão esquerda podem recolher na tentativa (raro; reavaliar no playtest — corner da funcional).
7. **Aposentadoria do legado de braços (D10, comportamento 7):** sai a fadiga de mira 1 s do `MainLoopPatch` ([MovementPatches.cs:119-139](../../modded/Patches/Trauma/MovementPatches.cs) — polling `ProceduralWeaponAnimation.IsAiming` + `SetAim(false)` + voz "TryAim"), sai a voz "Arm" em hit de braço zerado do `HealthPatches` (:126-132 — bloco `ConfigArmsEnabled`; re-âncora PA-02-04: o guard de estômago do 004 ocupou :110-121, INTACTO — deletar :113-119 removeria código do 004 e deixaria a voz viva; paridade com a aposentadoria do 003, que removeu também a voz de hit de perna; o feedback de entrada agora é o toast de 1ª ocorrência + tremor visível) e saem `AimingFatigueTimers` + entrada no `ResetAll` do `TraumaState` (:22,:43). Atualizar também o comentário de fronteira `HealthPatches.cs:123-125` ("estômago e braços seguem legados até os itens 007/006/005") — braços SAI da lista na entrega do 005 (paridade com a nota que o 003 deixou no mesmo arquivo). `Sistema de Braços` (config antigo) permanece bindado porém **INERTE** (tooltip atualizado; migração/remoção no item 010 — mesmo padrão do `Sistema de Pernas`). A migração mojibake existente do `MigrateOrphanedConfigKeys` (Plugin:256-288 — re-âncora PA-02-08) permanece — escreve numa key inerte, inofensivo.
8. **Rename-at-delivery do toggle (padrão 003):** `"Arms Effects (item 005)"` (placeholder — Plugin:124-125) → `"Arms Effects"`, nasce **ON** (master governa); a key órfã do placeholder é DELETADA em `MigrateOrphanedConfigKeys` SEM copiar o valor (o `false` de placeholder não é escolha do usuário — padrão do 003 em Plugin:290-312; template literal mais novo a replicar: bloco `"Fall Cycle (item 004)"` do 004, Plugin:314-334 — re-âncoras PA-02-08; lição CR-03-01).
...
**APIs de aplicação (sem patch — chamadas diretas no dono humano):** `Player.ActiveHealthController` get ([Player.cs:25291](../../../../references/eft-decompiled/Assembly-CSharp/EFT/Player.cs#L25291)) · `AddEffect<Tremor>` via reflection (AHC:3514-3538; prova runtime P2) · `ActiveHealthController.GClass3008` nested PÚBLICO com `ForceResidue()/ForceRemove()/Existing/Active` (AHC:29,219-233,550-570,637-662) · `IHealthController.EffectResidualEvent/EffectRemovedEvent` (watchdog — mesmas assinaturas do motor) · `IHealthController.IsBodyPartDestroyed/IsBodyPartBroken` (escolha da âncora de braço) · `FindActiveEffect<GInterface361>()` (query sem reflection) · `Player.HandsChangedEvent` (:25544) · `Player.FirearmController.OnAimingChanged` (:20037) · `IFirearmHandsController.SetAim(bool)` (uso existente no mod — MovementPatches.cs:47) · `ProceduralWeaponAnimation.IsAiming` (uso existente — MovementPatches.cs:123) · `Breath.TremorOn` (campo público — BreathEffector.cs:74) · `Speaker.Play(EPhraseTrigger, ETagStatus, bool, int?)` — retorna `TagBank` null quando engolido (protótipo P5 compilado); consumido via `TraumaVoice.TryPlayStrong` (extensão accept-gated do utilitário do 004 — decisão PA-02-03) · `FikaBackendUtils.IsHeadless` (gate headless — fika FikaBackendUtils.cs:49; P9 rec. (b)) · `AccessTools.Field(typeof(Player.FirearmController), "_player")` (mesmo acesso do FOVFix — P9).

## 3. Novas propriedades F12 (BepInEx)

Seção nova `9. Trauma 2.0 (Braços)` + 2 edições nas seções 6/2. `PROPRIEDADES.md` atualizado na entrega. **Numeração (PA-02-02):** o 004 ocupou a seção 8 com `8. Trauma 2.0 (Queda)` (Plugin:149-157; PROPRIEDADES.md:99) — a seção de braços entra como **9**; sequência pós-005: 7=Pernas, 8=Queda, 9=Braços (006/007 seguem 10/11).

| Seção | Nome (EN) | Tipo | Padrão | Faixa | Avançado | Tooltip (pt-BR) |
|---|---|---|---|---|---|---|
| `9. Trauma 2.0 (Braços)` | `ADS Cancel Seconds (Zeroed x2)` | float | `4` | 1 a 10 | — | Segundos de mira sustentada com 2 braços ZERADOS até o cancelamento do ADS. Soltar a mira reseta o timer. |
| `9. Trauma 2.0 (Braços)` | `ADS Cancel Seconds (Fractured x2)` | float | `3` | 1 a 10 | — | Segundos com 2 braços FRATURADOS até o cancelamento (fratura pior que zerado por design — decisão 3). |
| `9. Trauma 2.0 (Braços)` | `ADS Cancel Seconds (Zeroed + Fractured x2)` | float | `2` | 1 a 10 | — | Segundos com 2 braços zerados E 2 fraturados. Efetivo = min dos três timers — a linha mais severa nunca fica mais lenta que as outras (warn no log, 1x). |
| `9. Trauma 2.0 (Braços)` | `Re-ADS Lockout Seconds` | float | `1.5` | 1.0 a 1.5 | — | Bloqueio de re-mirar após o cancelamento (persiste à troca de arma). Tentativa durante o bloqueio dispara voz de dor (1 por janela). Faixa fixada pela decisão 17 (1–1,5 s). |
...
| `2. Mecanicas (Trauma)` | `Sistema de Braços` | bool | `true` | — | — | (INERTE desde a v1.6.0 — substituído pelo Trauma 2.0 / Arms Effects. Remoção da key no item 010.) |
...
| `modded/TRLImmersiveCombatMedicinePlugin.cs` | MODIFICAR | Binds §3 (seção 9 — PA-02-02 — + rename do toggle p/ `Arms Effects` default ON + tooltip INERTE no `Sistema de Braços`); delete da key órfã `"Arms Effects (item 005)"` (placeholder — Plugin:124-125) no `MigrateOrphanedConfigKeys` (sem copiar valor — padrão 003, Plugin:290-312; template do 004, Plugin:314-334 — PA-02-08); `AddComponent<TraumaArmsConsumer>()` após o motor; bump `1.6.0` (PA-02-01 — a 1.5.0 foi consumida pelo 004, fd799426; o bump cobre os TRÊS pontos que o 004 bumpou em conjunto: `[BepInPlugin]` Plugin:17, string do log do Awake Plugin:67 e `csproj <Version>` :7). |
...
| `modded/Patches/Trauma/HealthPatches.cs` | MODIFICAR | Remover SÓ o sub-bloco de braços do Postfix (:126-132 — bloco `ConfigArmsEnabled`, voz "Arm" em hit de braço zerado; re-âncora PA-02-04: o guard de estômago do 004 ocupou :110-121 e fica INTACTO) + atualizar o comentário de fronteira :123-125 (braços sai da lista de legados — paridade com a nota do 003). Desmaio e estômago INTACTOS (fronteiras 007/006). |
...
| `modded/Patches/Trauma/TraumaVoice.cs` | MODIFICAR | Novo método `TryPlayStrong(Player) : bool` accept-gated (retorna `Speaker?.Play(...) != null`; mesmo trigger/tags do `PlayStrong` — decisão PA-02-03). `PlayStrong`/`PlayLight`/`Allowed`/`Clear` do 004 INTOCADOS (canal separado por consumidor — comportamento do 004 preservado). |
...
| `PROPRIEDADES.md` | MODIFICAR | Seção 9 nova (a 8 é a Queda do 004 — PROPRIEDADES.md:99; PA-02-02); toggle `Arms Effects` ON; `Sistema de Braços` marcado inerte com nota de migração p/ 010 (gate de entrega). |
...
namespace TRLImmersiveCombatMedicine.Trauma
{
    ...
    public sealed class TraumaArmsConsumer : MonoBehaviour
    {
        ...
        private void ExecuteCancel(Player p)
        {
            ...
            // _lockoutVoicePlayed = false; _nextVoiceTryAt = 0f; _lockoutVoiceSkipLogged = false; // PA-02-06: piso/skip-log zerados por janela
            // _aimAnchor = -1f;
        ...
        internal static bool TryBlockReAds(Player p)
        {
            ...
            {
                // guard de INCAPACIDADE (PA-02-05 — espelho declarado de TraumaFallCycleConsumer.cs:235-236):
                //   TraumaState.BlackoutTimers.ContainsKey(p.ProfileId) || TraumaState.IsFainted
                //   || p.HealthController == null || !p.HealthController.IsAlive → SEM voz (blackout legado, janela
                //   de wake e downed Fika); o bloqueio do re-ADS em si CONTINUA (só a voz é suprimida)
                // bool played = TraumaVoice.TryPlayStrong(p); // decisão PA-02-03: extensão accept-gated do utilitário
                //   do 004 — OnAgony, ETagStatus.Combat | ETagStatus.Dying, demand:true, importance:100 (tags
                //   alinhadas ao 004 — TraumaVoice.cs:21); retorna Play != null; NÃO toca o anti-spam do PlayStrong
                //   ref: P5 (fura Busy de importance<100 — PhraseSpeakerClass.cs:206-227; Say+demand NÃO fura :28799-28829); peers via PhrasePacket
                // inst._lockoutVoicePlayed = played; // PA-01-02: só consome a janela se TOCOU — Play retorna null
                //   engolido por (a) Busy com frase importance>=100 em curso (100 <= Int_0 → skip, :206-210 —
                //   inclusive o OnAgony do PRÓPRIO 004, segundo emissor no mesmo Speaker) e (b) Blocker de GRUPO
                //   (SpeakerManager.FreeToSpeak :106-131 + GClass2291:50-59 — squad coop, sem bypass por demand/
                //   importance); engolido → _nextVoiceTryAt = Time.time + 0.3f (PA-02-06: limita hold-por-frame a
                //   ~5 Plays/janela — P-005-A não provada) e log skipped 1x/janela (_lockoutVoiceSkipLogged);
                //   tentativa após o piso re-tenta (cadência humana continua tocando na próxima janela livre)
                // log: played → "[Trauma2] ads LOCKOUT BLOCK <profileId> remaining=<t:0.##> voice=true"
                //      !played && !_lockoutVoiceSkipLogged → "... voice=skipped(busy|blocked)" + _lockoutVoiceSkipLogged = true
            }
...
// Binds §3 na seção "9. Trauma 2.0 (Braços)" (a 8 é a Queda do 004, Plugin:149-157 — PA-02-02); rename:
...
ConfigArmsEnabled = Config.Bind("2. Mecanicas (Trauma)", "Sistema de Braços", true,
    "(INERTE desde a v1.6.0 — substituído pelo Trauma 2.0 / Arms Effects. Remoção da key no item 010.)");
// MigrateOrphanedConfigKeys: mesmo bloco do "Legs Effects (item 003)" (Plugin:290-312; template literal mais novo:
// "Fall Cycle (item 004)", Plugin:314-334 — PA-02-08) replicado p/ a key "Arms Effects (item 005)" (placeholder —
// Plugin:124-125) — DELETAR sem copiar (o false de placeholder não é escolha do usuário) + Config.Save().
// Awake: gameObject.AddComponent<TraumaArmsConsumer>(); // DEPOIS de TraumaEngine (replay vazio inofensivo — padrão 003)
// [BepInPlugin(..., "1.6.0")] // PA-02-01: a 1.5.0 é a entrega do 004 (fd799426); bump em TRÊS pontos juntos —
//                             // [BepInPlugin] (Plugin:17), log do Awake "Plugin v1.6.0 carregado." (Plugin:67)
//                             // e csproj <Version>1.6.0</Version> (:7) — mesmo conjunto que o 004 bumpou
```

```csharp
// modded/Patches/Trauma/TraumaVoice.cs — ADIÇÃO (decisão PA-02-03; resto do arquivo INTOCADO)
...
[lockout 1–1,5 s por ProfileId (persiste troca de arma)]
        ...
        │   via TraumaVoice.TryPlayStrong (accept-gated, tags do 004 — PA-02-03) — 1 voz/janela (consumida SÓ
        │   se o Play tocou; Busy≥100 — inclusive o OnAgony do 004 — e Blocker de grupo engolem → piso 0,3s de
        │   re-tentativa + log voice=skipped 1x/janela — PA-02-06); sem voz sob incapacidade (predicado do 004,
        │   TraumaFallCycleConsumer.cs:235-236 — PA-02-05); peers via PhrasePacket
        ▼
...
- **Voz — canal unificado com o 004 (decisão PA-02-03, premissa p/ o item 011):** o 005 NÃO reusa `TraumaVoice.PlayStrong` (anti-spam fixo de 2 s consumido ANTES do Play com retorno ignorado — TraumaVoice.cs:33-39,:21 — violaria a PA-01-02); em vez disso ESTENDE o utilitário com `TryPlayStrong(Player):bool` accept-gated (canal separado por consumidor — comportamento do 004 inalterado; tags alinhadas: `Combat | Dying`). O OnAgony do 004 (queda executada/tentativa negada — TraumaFallCycleConsumer.cs:124, InputPatches.cs:104) é um SEGUNDO emissor no mesmo Speaker: pode engolir a chamada do 005 via Busy≥100 (retry com piso cobre — PA-02-06) e ambos alimentam o Blocker de grupo. Premissa registrada p/ o item 011: reconciliar os canais de voz dos consumidores (dupla fonte de OnAgony; throttles independentes que não se enxergam — rastro P-3.4). `importance:100` pode interromper fala em andamento (risco invertido do P5 — aceito); distinção audível do OnAgony por voz é asset-level (validação in-game; fallback OGG do repo é o plano B do P5, herdado). Chamada ENGOLIDA (Busy≥100/Blocker de grupo) não consome a janela do throttle (PA-01-02 — ver §1.6).
- **Config — numeração de seções (PA-02-02):** a seção nova de braços é a **9** — o 004 ocupou a 8 com `8. Trauma 2.0 (Queda)` (Plugin:149-157; PROPRIEDADES.md:99). Sequência pós-005: 7=Pernas, 8=Queda, 9=Braços; 006/007 seguem 10/11.
...
1. **Escopo da aposentadoria legada inclui a voz "Arm" do `HealthPatches` (:126-132 — re-âncora PA-02-04):** a funcional pede a remoção da FADIGA; a voz de hit de braço é legado irmão pendurado no mesmo `ConfigArmsEnabled` (que vira INERTE). Manter a voz com key inerte = código morto; removê-la = paridade exata com o 003 (que removeu a voz de hit de perna junto). Proposta: remover. Confirmar.
...
3. **Cadência do input em modo HOLD (premissa P-005-A, inferência marcada do P9):** o handler de input de raid não aparece no dump (namespace vazio) — não está provado se segurar o botão re-dispara `SetAim(true)` por frame ou 1×. O bloqueio é correto nos dois casos (idempotente) e o throttle de 1 voz/janela cobre o spam de SOM; o pior caso hold-por-frame sob Blocker de grupo (Play+logs a cada frame) é limitado pelo piso de re-tentativa de 0,3 s + log skipped 1×/janela (PA-02-06 — a spec não aposta em nenhum dos dois lados da premissa); smoke test hold E toggle é gate do AC-6. Registrar como premissa p/ o item 011.
...
- [ ] `TraumaArmsConsumer.cs`: registry + SubscribeWithSnapshot + gate `IsHeadless` no `IsActive()` (PA-01-07) + exclusão de bots (log) + mapa linha→efeito + watchdog/re-establish (piso 0,5 s + warn de conflito externo — PA-01-05) + hooks de evento de ADS (subscribe/unsubscribe simétrico em HandsChanged) + timer (âncora max(edge, linha); reinício por mudança de linha; descarte fora de tier) + `ExecuteCancel` + lockout/voz throttled via `TraumaVoice.TryPlayStrong` (decisão PA-02-03) com guard de incapacidade (predicado do 004, TraumaFallCycleConsumer.cs:235-236 — PA-02-05), janela só consumida se o Play tocou (PA-01-02) e piso de re-tentativa 0,3 s + log skipped 1×/janela (PA-02-06; campos zerados no `ExecuteCancel`) + edges do toggle + sweeps world-null/world-swap com `Discard` (PA-01-01).
- [ ] `TraumaVoice.cs`: novo método `TryPlayStrong(Player):bool` accept-gated (tags `Combat | Dying` do 004; SEM tocar `PlayStrong`/anti-spam — canal separado por consumidor, comportamento do 004 preservado — PA-02-03).
- [ ] Plugin: configs §3 (seção 9 — PA-02-02; rename `Arms Effects` ON; tooltip INERTE no `Sistema de Braços`); delete da key órfã `"Arms Effects (item 005)"` no `MigrateOrphanedConfigKeys` (template Plugin:314-334 — PA-02-08); `AddComponent<TraumaArmsConsumer>()`; bump `1.6.0` nos TRÊS pontos ([BepInPlugin] + log do Awake + csproj `<Version>` — PA-02-01).
- [ ] Aposentadoria legada: `MovementPatches` (bloco de fadiga :119-139), `HealthPatches` (voz "Arm" :126-132 + comentário de fronteira :123-125 — re-âncora PA-02-04; se abertura 1 confirmada), `TraumaState` (`AimingFatigueTimers` + ResetAll).
...
| 4 | Mudança de estado via API canônica; side-effects mapeados — AP-04 | ✅ | Tremor via `AddEffect` (state machine + eventos + sync nativos); remoção via `ForceResidue` da instância (mesmo funil de expiração — AHC:550-570); cancel via `SetAim(false)` (funil único — teardown completo + pacote Fika); voz via `Speaker.Play` encapsulado em `TraumaVoice.TryPlayStrong` (pipeline de frases + PhrasePacket; decisão PA-02-03). ÚNICO write direto: `Breath.TremorOn` (campo público consumido por frame) — bypass DOCUMENTADO do gate visual, exigido pela matriz (células com-analgésico→Tremor); side-effect é só o shake procedural (OR com Fracture). |
| 5 | Estado entre raids: raid1→exit→raid2, alt-F4/morte/MIA | ✅ | §5 Update world-null/world-swap: bookkeeping zerado sem tocar objetos destruídos (membros gerenciados); motor reseta separadamente (AC8 do 002); nada estático sobrevive além de `_timerClampWarned`, `_reestablishStormWarned` (warns 1×/sessão intencionais — PA-01-05) e cache de reflection; `TraumaTremor.Owned/OwnedPlayer/OwnedAnchor` são estáticos LIMPOS nos caminhos `Remove`/`Discard` (sweeps §5 — PA-02-07). |
...
| 2026-07-19 | Review técnica rodada 2 aplicada — 8 achados (2 fortes: versão 1.6.0 + seção 9); gate 2x2 fechado |
