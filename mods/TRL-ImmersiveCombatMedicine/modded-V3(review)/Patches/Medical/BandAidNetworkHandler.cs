    ...
    public static class BandAidNetworkHandler
    {
        ...
        private static void OnTraumaFaintReceived(TraumaFaintPacketV2 packet)
        {
            try
            {
                ...
                if (victim != null)
                {
                    if (packet.IsFainted) AggroHelper.NeutralizeAggro(victim);
                    else AggroHelper.RestoreAggro(victim);
                }
            }
            catch (Exception ex)
            {
        ...
        private static void OnTreatmentReportReceived(BandAidTreatmentReportPacketV2 packet)
        {
            try
            {
                ...
                string activePatient = BandAidController.Instance?.ActivePatientProfileId;
