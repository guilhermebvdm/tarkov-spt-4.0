...
namespace TRLImmersiveCombatMedicine.Medical
{
    ...
    public static class MedicHealPatch
    {
        ...
        private static FieldInfo _fiMedsController, _fiQueue, _fiFloat, _fiFirearmsAnimator;
        ...
        private static void EnsureFieldCache(Type owner)
        {
            ...
            _fiFirearmsAnimator = AccessTools.Field(typeof(Player.MedsController), "firearmsAnimator_0") 
                               ?? AccessTools.Field(typeof(Player.MedsController), "_firearmsAnimator");
        }
        ...
        private static void OnPatientEffectRemoved(IEffect effect)
        {
            ...
            try
            {
                ...
                if (_currentObservedMedsControllerClass != null && _method8Cached != null)
                {
                    ...
                    if (TRLImmersiveCombatMedicinePlugin.ConfigDebugMedicLogs.Value)
        ...
        static bool Prefix(object __instance)
        {
            ...
            // Diagnóstico sob debug toggle
            if (TRLImmersiveCombatMedicinePlugin.ConfigDebugMedicLogs.Value)
            {
                float timeSinceRedirect = UnityEngine.Time.time - RedirectStartTime;
                Logger.LogInfo($"🔍 method_5 | owner={operationOwner?.Profile?.Nickname ?? "?"} | doutorOp={isDoctorOperation} | IsRedirecting={IsRedirectingHeal} | BandAidActive={BandAidHealActive} | Patient={CurrentPatient?.Profile?.Nickname ?? "null"} | T+{timeSinceRedirect:F1}s");
            }
            ...
            if (!IsRedirectingHeal || CurrentPatient == null)
            {
                ...
                {
                    if (TRLImmersiveCombatMedicinePlugin.ConfigDebugMedicLogs.Value)
                        Logger.LogInfo("method_5: BLOQUEADO (BandAidHealActive — protegendo _currentObservedMedsControllerClass)");
                    return false;
                }
                return true;
            }
            ...
            try
            {
                ...
                if (patientHc == null)
                {
                    ...
                    if (_fiFirearmsAnimator != null)
                    {
                        var anim = _fiFirearmsAnimator.GetValue(medsController);
                        if (anim != null)
                        {
                ...
                {
                    if (TRLImmersiveCombatMedicinePlugin.ConfigDebugMedicLogs.Value)
                        Logger.LogInfo($"DoMedEffect retornou NULL para bodyPart={bodyPart1} — tentando fallback...");
                ...
                if (result == null)
                {
                    ...
                    if (TRLImmersiveCombatMedicinePlugin.ConfigDebugMedicLogs.Value)
                        Logger.LogInfo("DoMedEffect NULL em todas body parts — mantendo animação sem MedEffect");
                    ...
                    if (_fiFirearmsAnimator != null)
                    {
                        var animator2 = _fiFirearmsAnimator.GetValue(medsController);
                        if (animator2 != null)
                        {
                ...
                if (_fiFirearmsAnimator != null)
                {
                    var animator = _fiFirearmsAnimator.GetValue(medsController);
                    if (animator != null)
                    {
    ...
    public static class AnimCleanupPatch
    {
        ...
        static void Postfix(object __instance)
        {
            ...
            if (TRLImmersiveCombatMedicinePlugin.ConfigDebugMedicLogs.Value)
            {
                ...
                Logger.LogInfo($"=== 🔍 method_9 (CLEANUP) durante redirect | T+{timeSinceRedirect:F1}s ===");
                for (int i = 0; i < Math.Min(stackTrace.FrameCount, 10); i++)
                {
                    var frame = stackTrace.GetFrame(i);
                    var method = frame.GetMethod();
                    if (method != null)
                        Logger.LogInfo($"  [{i}] {method.DeclaringType?.FullName}.{method.Name}");
                }
                Logger.LogInfo($"=== FIM ===");
            }
