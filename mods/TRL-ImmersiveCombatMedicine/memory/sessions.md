...
>
> **Delta 2026-09-12 (Sessão 12):** Auditoria preventiva (sem mudança de código) confirmou que o ICM NÃO tem o bug de resolução de `NetPeer` por `GetPeerById(NetId)` encontrado no TRL-SpeakFromTarkov — todo envio de pacote no ICM é broadcast (host) ou single-destination (client → server), nunca endereça um peer específico por id. Lição preventiva registrada (P-12.1) para o caso de o mod algum dia ganhar relay seletivo por peer.
>
> **Delta 2026-09-12 (Sessão 13):** ICM em **v1.14.5** (`modded-V4`). **Bug crítico corrigido em 2 rodadas:** `EmergencyDrop`/`EmergencyDropSelf` (itens 024/025) podiam travar as mãos do jogador permanentemente (`HandsController` nulo, `NullReferenceException` em loop em `MovementContext.Class1396.method_3`), ou puxar a arma e guardá-la sozinha sem nenhuma ação do jogador. **Fix 01** (guard de reentrância `_weaponEquipPending`) tentou impedir o próprio mod de disparar o drop de novo antes da troca de arma anterior confirmar — **insuficiente**, o usuário reproduziu o bug sem repetição nenhuma. **Fix 02** (definitivo): removeu `TrySetLastEquippedWeapon()` por completo — a causa raiz era a própria criação de um processo assíncrono nativo (`Player.SpawnController`), que colide com QUALQUER operação de mãos (não só a do mod) enquanto está em voo. Comportamento final: o drop de emergência solta as mãos instantaneamente, mas não reequipa a arma sozinho — o jogador aperta a própria tecla de arma. Ver [P-13.3] e `025-emergencydrop-autocirurgia-06-fix-02.md`. Entrega dos itens `023-sequencia-maos-cura-sem-confirmacao` (callbacks nativos de `SetInHands`/`TrySetLastEquippedWeapon` passam a ser realmente usados, não descartados), `024-acelerar-fechamento-kit-emergencydrop` (drop de emergência do kit cirúrgico passa a ser instantâneo via `Player.DestroyController()`, em vez de esperar o `Task.Delay(600)` fixo de `MedsController.Drop`; exclusivo a CMS/Surv12 por decisão do usuário) e `025-emergencydrop-autocirurgia` (mesma tecla estendida pra auto-cirurgia/self-heal, que antes não era interceptada pelo mod — `ClearQueue()`+`CancelApplyingItem()` dispara a punição de carga nativa sem reimplementar, `DestroyController()` pula o fechamento do kit). Build local verde (0 erros/0 warnings), pasta do jogo não tocada.

## Pendências

- ~~[P-13.3]~~ **RESOLVIDA em 2026-09-12** — usuário validou em raid as 2 situações (drop único sem repetição; ciclo rápido repetido várias vezes) e confirmou: "funcionou direitinho em tudo, arma não guarda mais e também posso dropar e usar cirurgia bem rápido várias vezes que não quebra". Fix 02 (`TrySetLastEquippedWeapon()` removido) confirmado como definitivo. Investigação de acompanhamento confirmou que essa API não é chamada em mais nenhum lugar do mod — o único outro ponto que cria um processo de mãos assíncrono é `SetInHands` em `HealRoutine` (item 023), que não tem o mesmo risco (não há uma segunda operação concorrente competindo pelo mesmo item).
- [P-13.2] (aberta 2026-09-12) **Validar in-game o drop de emergência na auto-cirurgia (Item 025, v1.14.3)** — Cenários a testar: (1) CMS/Surv12 em si mesmo, acionar a tecla, confirmar arma pronta no mesmo instante; (2) mesma tecla com bandagem/tala/torniquete/medkit comum em si mesmo — nada deve acontecer; (3) isolamento: médico com perna zerada examina aliado com braço zerado e o cura (CMS deve curar só o aliado); encerrar a interação e usar CMS em si mesmo deve funcionar normalmente; (4) punição de carga nativa (≥ `ItemRemoveAfterInterruptionTime` perde 1 carga, sem notificação especial do mod); (5) confirmar que o jogador não fica travado sem poder correr depois do drop (`UsingMeds`, ver PA-01-01 da review técnica); (6) coop/Fika — sem trava de mãos. 🟡 Validação in-game.
- [P-13.1] (aberta 2026-09-12) **Validar in-game o drop de emergência instantâneo (Item 024, v1.14.2)** — Cenários a testar: (1) Usar CMS/Surv12, acionar a tecla de drop de emergência, confirmar que a arma atira no mesmo instante (sem o intervalo de antes); (2) Acionar a mesma tecla durante bandagem/tala/torniquete/medkit comum — confirmar que nada acontece; (3) Penalidade de carga: acionar com ≥1s de uso (perde carga, notificação de perda) e com <1s (não perde, notificação genérica); (4) Coop/Fika: curar aliado e acionar o drop de emergência nesse meio-tempo — sem trava de mãos nem log de colisão do lado FIKA; (5) Corner case de morte simultânea ao acionar a tecla. 🟡 Validação in-game.
- [P-12.1] (aberta 2026-09-12) **Lição preventiva — nunca resolver `NetPeer` via `GetPeerById(NetId)`** — se o ICM implementar relay seletivo por peer no futuro (ex.: culling por distância), usar `RegisterPacket<T, NetPeer>` + cache `Dictionary<ProfileId, NetPeer>` (padrão de `mods/TRL-SpeakFromTarkov/modded-V4/Network/SftNetwork.cs`), nunca `GetPeerById(fikaPlayer.NetId)` (espaços de numeração incompatíveis). 🟢 Ideia/prevenção — sem ação necessária hoje.
...
## 2026-09-12 (GMT-3) — Sessão 13: Confirmação de mãos na cura (023) + Descarte instantâneo do EmergencyDrop (024) + auto-cirurgia (025), v1.14.0 → v1.14.3

**Tema central:** Ciclo completo de desenvolvimento de dois itens encadeados sobre a mesma área frágil do código (sequência de mãos da cura/`EmergencyDrop`): item 023 (parar de descartar os callbacks nativos de `SetInHands`/`TrySetLastEquippedWeapon`) e item 024 (tornar o drop de emergência do kit cirúrgico realmente instantâneo). Ambos em `modded-V4`, nunca em `modded-V3(review)` ou anterior.

**Decisões-chave:**
- **Item 023 — callbacks nativos usados de verdade:** `HealRoutine` e `EmergencyDrop` chamavam `SetInHands`/`TrySetLastEquippedWeapon` descartando o `callback` (lambda vazio ou `null`). Passaram a usar delegates de tipo **explícito** (`Callback<IHandsController>`/`Callback`, não lambda implícito) — achado de code review: `Result<T>` é **struct** (`!= null` não compila) e um lambda implícito deixava a inferência de tipo ambígua entre sobrecargas de `SetInHands`. `IResult` (usado por `TrySetLastEquippedWeapon`) é interface, então `!= null` é válido lá.
- **Item 024 — causa raiz real do "kit não fecha rápido":** não é uma animação acelerável — é um `Task.Delay(600)` **fixo e incondicional** dentro de `Player.MedsController.Drop`/`method_2` (`Player.cs:19967-19977`), que ignora qualquer parâmetro de velocidade. A via que funciona é `Player.DestroyController()` (`Player.cs:31676-31688`): descarta `HandsController` de forma **síncrona**, sem passar por `Drop()`. Chamado antes de `TrySetLastEquippedWeapon`, o pipeline (`Process<>.Execute`, `Player.cs:22514-22518`) detecta `HandsController == null` e pula o delay inteiro.
- **Item 024 — escopo restrito a cirurgia (decisão do usuário, pós-review):** o `Task.Delay(600)` é universal pra qualquer item médico, mas só CMS/Surv12 têm uma animação visível de "fechar o kit" incômoda o bastante — os demais (bandagem, tala, torniquete, medkit comum) já são confortáveis com vanilla. `EmergencyDrop` passou a só produzir efeito com `stats.IsSurgery == true` (novo campo `_currentHealIsSurgery`, gate em `Update()` + re-checagem defensiva no método). Isso resolveu de quebra um achado de review (auditoria incompleta de `Destroy()`, que é virtual com 8 overrides — só `MedsController.Destroy()` é alcançável neste call site, e o escopo-cirurgia reforça essa garantia).
- **Item 024 — penalidade de carga alinhada ao cancelamento normal:** `EmergencyDrop` não aplicava NENHUM consumo (confirmado por leitura completa antes de implementar, descartando risco de duplo-custo). Passou a aplicar a mesma regra de `CancelHealInProgress` (≥1s de uso → perde 1 carga), com notificação diferenciada (`TreatmentCancelledWithItemLoss` vs `ItemDropped`).
- **Fix 01 (crítico) — `TrySetLastEquippedWeapon` é assíncrono por dentro, causando travamento de mãos:** usuário reportou em raid real que repetir o ciclo de drop de emergência rápido demais (3x seguidas) travava as mãos do personagem permanentemente (`NullReferenceException` em loop, `MovementContext.Class1396.method_3` desreferenciando `HandsController` nulo sem checagem). Causa raiz: `TrySetLastEquippedWeapon` → `Process<>.CreateController()` → `Player.SpawnController(...)` (`Player.cs:22540-22574`) é ASSÍNCRONO — `Player.ProcessStatus` fica `Scheduled` até o callback confirmar (`ExecuteNext()`, `Player.cs:22567`). Disparar um novo `EmergencyDrop(Self)` nessa janela colide com a operação anterior, podendo deixar `HandsController` preso em `null` pra sempre. Corrigido com guard de reentrância `_weaponEquipPending` (seta antes da chamada, limpa no callback, timeout de 5s de segurança). Afeta igualmente os itens 024 e 025 (mesmo call site).
- **Item 025 — mesma tecla estendida pra auto-cirurgia (self-heal):** usuário testou o drop de emergência durante cirurgia EM SI MESMO (não num aliado) e nada aconteceu — descoberto que todo o sistema `BandAidController`/`HealRoutine`/`EmergencyDrop` só existe pra cura de ALIADO; self-heal passa 100% pelo fluxo nativo, nunca interceptado. Investigação (subagent dedicado + verificação manual linha-a-linha) confirmou que a punição nativa de perda de carga por cancelamento tardio roda de forma SÍNCRONA dentro de `ActiveHealthController.CancelApplyingItem()` (`ActiveHealthController.cs:1944-1949`), completamente desacoplada do fechamento do kit — logo, chamar essa API antes de `DestroyController()` não duplica nem pula a punição. Também descoberto que o cancelamento nativo do PRÓPRIO jogo (`Player.MedsController.Remove()`) tem seu próprio `Task.Delay(600)`, diferente do de `Drop()` — replicado sem esse delay via `ClearQueue()` + `CancelApplyingItem()` (mesma dupla que `ObservedMedsControllerClass.Remove()` nativo já faz). Confirmado que o isolamento aliado×self (exemplo do usuário: médico com perna zerada curando aliado com braço zerado) já é garantido por um guard G5 pré-existente em `MedicHealPatch.cs:340-347` — nenhuma mudança necessária ali.

**Lições / hipóteses descartadas:**
- *Acelerar via `RemoveLeftHandItem(speed)`:* descartada — esse mecanismo é gated por `GClass2725.Bool_0`, só engajado por acessórios de mão esquerda via `ToggleHand`, nunca pelo `MedsController`. Não tinha efeito no cenário.
- *`AbstractProcess_0`/`ProcessStatus` como risco do `DestroyController()` abrupto:* investigado a fundo — `Player_0.AbstractProcess_0 = this` só é atribuído dentro de `Begin()` (`Player.cs:22474`), chamado pelo MESMO delegate `execute` em ambos os caminhos de `Process<>.Execute()` (instantâneo e assíncrono). Risco rebaixado de "não confirmado" pra "confirmado por leitura direta, sem necessidade de teste em raid pra esse ponto específico".
- *Uma versão do drop de emergência pra qualquer item médico:* descartada pelo usuário — restringir a cirurgia reduz a superfície de mudança sem perder o objetivo (a real "emergência" só existe quando a animação de fechar é longa).

**Atividade cronológica:**
1. Item 023: `/create-spec` → `/review-spec` → `/create-technical-spec` → `/review-technical-spec` (2 achados) → `/code-mod` → `/code-review` 01 (1 achado, `Result<T>` struct) → `/apply-code-review`. v1.14.1, build verde.
2. Item 024: investigação estática iterativa (3 teorias descartadas antes da correta) → `/add-backlog-item` → `/create-spec` → `/review-spec` → `/create-technical-spec` (achado: `Task.Delay(600)` fixo + `DestroyController()`) → `/review-technical-spec` 01 (3 achados: auditoria AP-03 incompleta, notificação de penalidade ausente, `_healStartTime` não resetado — todos resolvidos) → decisão de escopo (exclusivo a cirurgia) → `/code-mod` → `/code-review` 01 (1 achado 🟠: `ConsumeSafe` rodava depois de `ThrowItem`, operando num item já fora do inventário) → `/apply-code-review` (ordem corrigida: consumir antes de dropar).
3. Item 025: usuário reportou tecla sem efeito em self-heal → `/add-backlog-item` → `/create-spec` → `/review-spec` (isolamento aliado×self esclarecido com exemplo concreto) → `/create-technical-spec` (subagent dedicado investigou onde a punição de carga nativa realmente roda) → `/review-technical-spec` 01 (3 achados 🟢, todos de clareza documental) → `/code-mod`.
4. Compilação Release em `modded-V4` pros três itens: 0 Erros, 0 Warnings. Versão final `1.14.3`.

**Pendências abertas nesta sessão:**
- [P-13.2] (aberta 2026-09-12) 🟡 Validar in-game o drop de emergência na auto-cirurgia (Item 025, v1.14.3) — ver topo.
- [P-13.1] (aberta 2026-09-12) 🟡 Validar in-game o drop de emergência instantâneo (Item 024, v1.14.3) — ver topo.

**Cross-refs:**
- Itens de backlog: `mods/TRL-ImmersiveCombatMedicine/backlog/023-sequencia-maos-cura-sem-confirmacao/`, `mods/TRL-ImmersiveCombatMedicine/backlog/024-acelerar-fechamento-kit-emergencydrop/`, `mods/TRL-ImmersiveCombatMedicine/backlog/025-emergencydrop-autocirurgia/` (fix crítico documentado em `025-emergencydrop-autocirurgia-06-fix-01.md`, aplicável também ao 024).
- **Lição chave pra qualquer item futuro que chame `TrySetLastEquippedWeapon`/`SetInHands`/qualquer `Player.Process<>`:** essas chamadas são assíncronas por dentro mesmo quando "instantâneas" do ponto de vista do C# síncrono (o `HandsController == null` só pula o DELAY de `Drop()`, não torna `SpawnController` síncrono). Uma trava de reentrância NO NOSSO CÓDIGO (tentativa do Fix 01) **não é suficiente** — o `Process<>` nativo pode colidir com QUALQUER operação de mãos, inclusive o próprio jogador reusando um item por conta própria, não só uma re-chamada do mod. Se o objetivo é só "soltar as mãos instantaneamente" (não necessariamente reequipar algo), a solução mais robusta é **não criar o `Process` de troca em primeiro lugar** (ex.: não chamar `TrySetLastEquippedWeapon`) — `DestroyController()` sozinho não mexe em `ProcessStatus`/`AbstractProcess_0`, então não deixa nada pendente pra colidir depois.
- [[feedback_icm_animacao_fragil]] — mais uma vez confirmado: mudanças pontuais + build local antes de qualquer cópia pra pasta do jogo.

---

## 2026-09-12 02:08 (GMT-3) — Sessão 12: Auditoria preventiva — bug de NetPeer/NetId (TRL-SpeakFromTarkov) não existe no ICM

**Tema central:** Confirmar se o ICM tem, ou pode reintroduzir, o bug de resolução de peer do `TRL-SpeakFromTarkov` (item 014): misturar `FikaPlayer.NetId` (id de jogo do FIKA, host sempre 1) com o índice interno de `IFikaNetworkManager.GetPeerById` (ordem de conexão de transporte do LiteNetLib) — dois espaços de numeração sem relação entre si.

**Decisões-chave:**
- **Bug NÃO existe hoje no ICM** — confirmado por leitura direta, não só por inferência. `Patches/Medical/BandAidNetworkHandler.cs` e `LegacyPackets.cs` não têm nenhum `GetPeerById`, `NetId` ou `RegisterPacket<T, NetPeer>`. Todo pacote passa por um único helper `SendPacket<T>` (`BandAidNetworkHandler.cs:199-225`): do lado host, sempre broadcast (`Singleton<FikaServer>.Instance.SendData(ref packet, DeliveryMethod.ReliableUnordered, true)`, flag `broadcast:true`); do lado client, sempre envia pro server (única conexão possível). Os 6 pontos de "relay" do arquivo (linhas 268, 366, 802, 863, 959, 1109) reusam o MESMO helper — nenhum tenta endereçar um peer específico.
- **Sem plano no backlog que tornaria isso relevante em breve** — grep em `mods/TRL-ImmersiveCombatMedicine/backlog/**/*.md`, `ROADMAP.md` e `inventario-funcionalidades.md` por "culling"/"distância de rede"/"selective"/"broadcast" não achou nenhuma proposta de otimização de rede por proximidade ou relay seletivo por jogador. Os únicos hits de "distância" são sobre a distância de INTERAÇÃO (médico↔paciente em metros), tema não relacionado.

**Lições / hipóteses descartadas:**
- Nenhuma hipótese refutada nesta sessão — investigação read-only confirmou a hipótese inicial (mod não tem o bug). A lição em si é preventiva, registrada em [P-12.1] no topo: **se o ICM algum dia implementar relay seletivo por peer, o padrão correto é `RegisterPacket<T, NetPeer>` (`IFikaNetworkManager.cs:137`) + cache `Dictionary<string /*ProfileId*/, NetPeer>` populado a cada pacote recebido + limpeza no fim da raid** — nunca `GetPeerById(fikaPlayer.NetId)`. Implementação de referência: `mods/TRL-SpeakFromTarkov/modded-V4/Network/SftNetwork.cs` (`_profileIdToPeer`, população em `OnReceiveVoipDataV2`, uso em `RelayVoiceToNearbyPeers`, limpeza em `StopSession()`).

**Atividade cronológica:**
1. Grep por `GetPeerById`/`NetId`/`NetPeer`/`RegisterPacket<` em `Patches/Medical/*.cs` — zero ocorrências do padrão problemático.
2. Leitura de `SendPacket<T>` (`BandAidNetworkHandler.cs:199-225`) — confirmado broadcast-only do lado host, single-destination do lado client.
3. Grep dos 6 call-sites de "relay" no mesmo arquivo — todos reusam `SendPacket<T>`, nenhum endereça peer específico.
4. Grep no backlog/roadmap/inventário do mod por termos de culling/distância de rede — nenhum plano futuro relevante encontrado.

**Pendências abertas nesta sessão:**
- [P-12.1] (aberta 2026-09-12) Ver topo. Categoria: 🟢 ideia/prevenção.

**Cross-refs:**
- Lição espelha a correção já aplicada em `mods/TRL-SpeakFromTarkov/memory/sessions.md` (item de backlog 014) — mesmo mecanismo de bug, registrado lá com a correção real aplicada; aqui é só profilaxia, porque o ICM nunca teve o sintoma.
