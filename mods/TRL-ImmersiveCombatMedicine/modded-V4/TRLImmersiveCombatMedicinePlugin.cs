...
namespace TRLImmersiveCombatMedicine
{
    ...
    [BepInPlugin("com.trl.immersivecombatmedicine", "TRL-ImmersiveCombatMedicine", "1.15.0")]
    public class TRLImmersiveCombatMedicinePlugin : BaseUnityPlugin
    {
        ...
        // --- Debug Configs (F12) ---
        public static ConfigEntry<bool> ConfigDebugMedicLogs;
        public static ConfigEntry<bool> ConfigDebugPhysicsLogs;
        ...
        private void Awake()
        {
            ...
            // Configs Debug (F12)
            ConfigDebugMedicLogs = Config.Bind("12. Debug (Dev)", "Debug Medic Logs", false, "Ativa logs detalhados de ações médicas, pacotes de cura e redirecionamentos no console.");
            ConfigDebugPhysicsLogs = Config.Bind("12. Debug (Dev)", "Debug Physics Logs", false, "Ativa logs detalhados de física, recálculo de limites de velocidade e sprint no console.");
            ...
            try
            {
                ...
                {
                    BandAidNetworkHandler.InitFikaEvents();
        ...
        private void Update()
        {
            if (IsFikaInstalled)
            {
                BandAidNetworkHandler.EnsurePacketsRegistered();
            }
            ...
            if (TraumaState.BlackoutTimers.TryGetValue(localId, out float wakeDeadline))
            {
                ...
                if (Time.time < wakeDeadline)
                {
                    if (!TraumaState.IsFainted)
                    {
                        ...
                        var fikaPlayer = gameWorld.MainPlayer as global::Fika.Core.Main.Players.FikaPlayer;
                        if (fikaPlayer != null)
                        {
