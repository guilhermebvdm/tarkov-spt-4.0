...
namespace TRLImmersiveCombatMedicine.Medical
{
    ...
    public class TourniquetManager : MonoBehaviour
    {
        ...
        // Buffer reutilizado pelo Update() para copiar as chaves antes de iterar (evita alocar
        // uma List<EBodyPart> nova a cada frame — no máximo ~4 entradas, mas é hot path).
        private static readonly List<EBodyPart> _updateKeysBuffer = new List<EBodyPart>();
        ...
        public void Update()
        {
            ...
            // Cópia para iterar (pode remover durante iteração) — buffer reutilizado, sem alocar por frame
            _updateKeysBuffer.Clear();
            _updateKeysBuffer.AddRange(_activeTourniquets.Keys);

            foreach (var bodyPart in _updateKeysBuffer)
            {
