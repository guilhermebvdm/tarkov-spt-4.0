...
namespace TRLImmersiveCombatMedicine.Medical
{
    public class BandAidController : MonoBehaviour
    {
        ...
        // ref: item 024 — espelha _itemBeingUsed (setado/resetado nos MESMOS pontos, nunca
        // sozinho): gate de escopo do EmergencyDrop, exclusivo a itens de cirurgia (CMS/Surv12).
        private bool _currentHealIsSurgery = false;
        // ref: item 025 / CR-01-01 — cache do resultado de IsSurgery pro item médico que o
        // JOGADOR está segurando em auto-cirurgia (self-heal nativo), por referência de item.
        // Evita recalcular ItemDatabase.GetStats todo frame em Update() enquanto qualquer item
        // médico está nas mãos.
        ...
        private void Update()
        {
            ...
            // Exclusivo a itens de cirurgia, CMS/Surv12: o encerramento vanilla dos demais itens
            // médicos já é curto o bastante pra ser confortável, então a tecla não tem efeito neles.
            // Os dois contextos são mutuamente exclusivos: selfSurgeryInProgress exige
            // !_isHealingInProgress (invariante: deve espelhar MedicHealPatch.BandAidHealActive,
            // guard G5 em MedicHealPatch.cs:342 — qualquer novo caminho de cleanup precisa zerar
            // os dois, ref: CR-01-02), e o guard G5 já bloqueia o self-heal nativo do médico
            // enquanto uma cura de aliado está ativa.)
            var mainPlayer = Singleton<GameWorld>.Instance.MainPlayer;
            // ref: CR-01-01 — cache por referência de item (mesmo espírito de _currentHealIsSurgery
            // pro lado de aliado): evita recalcular ItemDatabase.GetStats todo frame enquanto
            // QUALQUER item médico está nas mãos (bandagem/tala também casam com MedsController).
        ...
        private void EmergencyDrop()
        {
            ...
            // ref: item 024 — mesmo padrão de CancelHealInProgress: captura elapsed e reseta
            // _healStartTime na hora (PA-01-03), evita estado estático estagnado.
            ...
            //      confirmado em Player.cs:19967-19977 e :19801-19807). HandsController fica null —
            //      mãos livres instantaneamente (ver PASSO 3, fix-02: NÃO reequipamos arma aqui).
            // Re-checagem defensiva: mesmo que o gate do Update() já filtre por cirurgia, este
            // passo não deve ter efeito nenhum se chamado fora desse contexto (AP-03).
            if (isSurgeryItem)
            {
                try
                {
            ...
            // === PASSO 3: MÃOS FICAM LIVRES (sem reequipar arma automaticamente) ===
            // ref: fix-02 — TrySetLastEquippedWeapon foi REMOVIDO deste ponto (existiu nos itens
            // 023/024, guardado por _weaponEquipPending no fix-01). Causa raiz real do travamento
            // de mãos (não era duplicidade de chamada nossa): TrySetLastEquippedWeapon cria um
            // Process<> de verdade, que deixa Player.ProcessStatus = Scheduled até o callback
            // assíncrono do SpawnController confirmar (Player.cs:22540-22574/22567). Se o jogador
            // usar o item médico DE NOVO nessa janela, o novo Process (nativo, fora do nosso
            // controle) vê ProcessStatus == Scheduled e se ENFILEIRA (Player.AbstractProcess_0,
            // Player.cs:22462-22475) em vez de rodar na hora; quando o nosso Process da arma
            // finalmente confirma, ExecuteNext() (Player.cs:22693-22710) dispara esse processo
            // enfileirado automaticamente — e como as mãos agora têm a ARMA (não o item médico),
            // isso desmonta a arma que acabou de ser montada pra montar o item médico de novo
            // ("arma puxada e guardada sozinha" reportado em raid). _weaponEquipPending (fix-01)
            // só protegia contra O NOSSO PRÓPRIO código disparar de novo — não protegia contra o
            // jogador reusar o item nativamente nessa janela, que é o gatilho real. Sem criar esse
            // Process aqui, DestroyController() (acima) deixa HandsController null e NENHUM
            // Process fica pendente do nosso lado — reusar o item na sequência não tem com o que
            // colidir. Troca de comportamento aceita: mãos ficam livres, mas SEM auto-reequipar a
            // arma — o jogador precisa apertar a própria tecla de arma se quiser puxá-la de volta.

            // ref: CR-01-01 — Punição de cancelamento roda ANTES do drop (mesma ordem de
            // CancelHealInProgress, BandAidController.cs:~800), nunca depois: ConsumeSafe pode
            // levar o item a zero carga e chamar DiscardItemNetworked, cujo guard trata
            // `item.CurrentAddress == null` como "já removido, nada a fazer" (MedicalLogic.cs:579)
            // — depois de ThrowItem o item já não pertence mais ao inventário do médico, então
            // rodar o consumo/descarte networked ali seria um no-op silencioso na melhor hipótese
            // e uma operação de rede concorrente sobre um item que acabou de mudar de posse na
            // pior (mesma classe de bug documentada em CR-04/CR-05 neste arquivo).
            // Regra EFT (ItemRemoveAfterInterruptionTime = 1.0s): itens de cirurgia (CMS/Surv12),
            // itens de uso único (tala, bandagem, CALOK) ou consumíveis usados por >= 1.0s
            // perdem 1 uso/carga.
            ...
            if (elapsed >= 1.0f && savedItem != null && savedStats != null)
            {
                ...
                {
                    TRLImmersiveCombatMedicinePlugin.ModLogger.LogInfo($"EmergencyDrop: cancelamento após {elapsed:F1}s (>= 1.0s) — consumindo 1 carga do item desesterilizado ({itemName}).");
            ...
            // ref: CR-01-01 — só dropar se o item ainda tem endereço válido: ConsumeSafe (acima)
            // pode ter descartado o item via DiscardItemNetworked (rede) se a carga chegou a
            // zero — nesse caso o item já não existe mais pra jogar no chão.
            ...
            // ref: PA-01-02 — notificação diferenciada quando a penalidade é aplicada, mesmo
            // padrão de CancelHealInProgress. Substitui a notificação genérica incondicional.
            if (itemConsumed)
            {
                NotificationManagerClass.DisplayMessageNotification(
        ...
        private void EmergencyDropSelf(Player doctor, Player.MedsController meds)
        {
            ...
            // ref: fix-02 — TrySetLastEquippedWeapon REMOVIDO. Causa raiz real do travamento de
            // mãos: cria um Process<> nativo que deixa Player.ProcessStatus = Scheduled até o
            // SpawnController confirmar de forma assíncrona (Player.cs:22540-22574/22567). Se o
            // jogador reusar o item médico nessa janela, o novo Process nativo se ENFILEIRA
            // (Player.AbstractProcess_0, Player.cs:22462-22475) e dispara sozinho quando o nosso
            // Process da arma confirma (ExecuteNext, Player.cs:22693-22710) — desmontando a arma
            // recém-montada pra montar o item de novo (bug reportado em raid, 2x). Sem criar esse
            // Process aqui, DestroyController() acima deixa HandsController null sem nada
            // pendente do nosso lado. Troca de comportamento aceita: mãos ficam livres, mas SEM
            // auto-reequipar arma — o jogador aperta a própria tecla de arma se quiser puxá-la.

            // ref: item 025 — SEM lógica de consumo própria (decisão do usuário): a punição de carga já
            // rodou de forma síncrona dentro de CancelApplyingItem(), acima (confirmado por leitura de
            // ActiveHealthController.cs:1935-1964: só decrementa HpResource, nenhum discard automático).
            // Guard defensivo mantido por consistência (CR-01-01, item 024), não porque essa situação
            // seja esperada aqui.
            ...
            // ref: PA-01-01 — UsingMeds confirmado gate de movimento puro (MovementContext.cs:1193,
            // 1248,1276,1316,3306; PlayerPhysicalClass.cs:268,671), nunca lido pela máquina de estado
            // do efeito — escrever aqui é seguro. Mantido por precaução: o código nativo que
            // normalmente desligaria essa flag pode estar dentro do trecho de Drop()/fechamento que
            // DestroyController() pula de propósito.
            doctor.MovementContext.SetPhysicalCondition(EPhysicalCondition.UsingMeds, false);
            // ref: PA-01-02 — no-op confirmado neste contexto (HealingLegs nunca é ligado fora de
            // HealRoutine, ver doc-comment de ReleaseSurgeryImmobilize); mantido só por consistência
            // de código com EmergencyDrop, custo desprezível.
        ...
        private IEnumerator HealRoutine(Player doctor, Player patient, Item itemUsed, ItemStats stats)
        {
            ...
            _currentHealIsSurgery = stats.IsSurgery; // ref: item 024 — gate de escopo do EmergencyDrop
            ...
            TRLImmersiveCombatMedicinePlugin.ModLogger.LogInfo($"HealRoutine: REDIRECT ATIVADO | item={itemUsed.ShortName.Localized()} | UseTime={stats.UseTime}s | patient={patient.Profile.Nickname}");

            // Colocar o item médico nas mãos (aciona a animação visual)
            // ref: item 023 — Assembly-CSharp/EFT/Player.cs:31845 SetInHands(Item, Callback<IHandsController>)
            //      delega pra TryProceed (Player.cs:32003), cujo callback informa que a operação
            //      resolveu — antes descartado com `(result) => { }`.
            // ref: CR-01-01 — result.Error confirmado como string por experimento de compilação
            // (item 023, code review 01). Falha real agora é logada, não só o timeout do UseTime.
            // NOTA: `Result<T>` é STRUCT (tipo valor) — confirmado por experimento de compilação
            // nesta sessão (`result != null` não compila, CS0019). Por isso não checa null, só
            // `result.Error`. Tipo do callback é explícito (Callback<IHandsController>) em vez de
            // lambda implícito — com múltiplas sobrecargas de SetInHands aceitando Callback<T>
            // para T diferentes, um lambda implícito que não referencia membros de T pode deixar
            // a inferência de tipo ambígua; explícito remove essa ambiguidade de vez.
            bool setInHandsConfirmed = false;
            try
            {
                Callback<IHandsController> setInHandsCallback = delegate(Result<IHandsController> result)
                {
                ...
                doctor.SetInHands(itemUsed, setInHandsCallback);
            }
            catch (Exception ex)
            {
            ...
            // ref: item 023 — antes esse resultado era descartado sem log nenhum. Se o callback
            // nativo não confirmou nem depois do UseTime inteiro, é sinal de algo fora do comum —
            // loga e segue igual (ForceFinishAnimation ainda roda, devolvendo a arma de qualquer jeito).
            ...
            TRLImmersiveCombatMedicinePlugin.ModLogger.LogInfo($"HealRoutine: UseTime TERMINOU | IsRedirecting={MedicHealPatch.IsRedirectingHeal} | T+{stats.UseTime}s");
        ...
        private void ResetAllState()
        {
            ...
            _pendingHealStats = null;
            _pendingHealPatient = null;
            // ref: CR-01-01 — cache do gate de auto-cirurgia não deve segurar referência de
            // Item de uma raid anterior.
            _lastSelfMedsItemChecked = null;
            _lastSelfMedsIsSurgery = false;
