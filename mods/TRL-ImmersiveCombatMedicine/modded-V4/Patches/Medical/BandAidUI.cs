...
namespace TRLImmersiveCombatMedicine.Medical
{
    public class BandAidUI : MonoBehaviour
    {
        ...
        private static void CacheTypes()
        {
            ...
            if (_findEffectMethod != null)
            {
                _genericFindMethods[_heavyBleedType] = _findEffectMethod.MakeGenericMethod(_heavyBleedType);
                _genericFindMethods[_lightBleedType] = _findEffectMethod.MakeGenericMethod(_lightBleedType);
                _genericFindMethods[_fractureType] = _findEffectMethod.MakeGenericMethod(_fractureType);
                _genericFindMethods[_contusionType] = _findEffectMethod.MakeGenericMethod(_contusionType);
                _genericFindMethods[_tremorType] = _findEffectMethod.MakeGenericMethod(_tremorType);
                _genericFindMethods[_painType] = _findEffectMethod.MakeGenericMethod(_painType);
                _genericFindMethods[_intoxicationType] = _findEffectMethod.MakeGenericMethod(_intoxicationType);
            }

            _typesCached = true;
        }

        private static readonly Dictionary<Type, MethodInfo> _genericFindMethods = new Dictionary<Type, MethodInfo>();
        private static readonly Dictionary<EBodyPart, object[]> _bodyPartArgsCache = new Dictionary<EBodyPart, object[]>
        {
            { EBodyPart.Head, new object[] { EBodyPart.Head } },
            { EBodyPart.Chest, new object[] { EBodyPart.Chest } },
            { EBodyPart.Stomach, new object[] { EBodyPart.Stomach } },
            { EBodyPart.LeftArm, new object[] { EBodyPart.LeftArm } },
            { EBodyPart.RightArm, new object[] { EBodyPart.RightArm } },
            { EBodyPart.LeftLeg, new object[] { EBodyPart.LeftLeg } },
            { EBodyPart.RightLeg, new object[] { EBodyPart.RightLeg } },
            { EBodyPart.Common, new object[] { EBodyPart.Common } },
        };
        ...
        private static bool HasEffect(IHealthController hc, EBodyPart bodyPart, Type effectType)
        {
            ...
            if (!_genericFindMethods.TryGetValue(effectType, out var method) || method == null) return false;
            if (!_bodyPartArgsCache.TryGetValue(bodyPart, out var args)) return false;
            try
            {
                return method.Invoke(hc, args) != null;
            }
            catch { }
            return false;
        }
