        ...
        private void EvaluatePlayer(PlayerTraumaRecord rec, bool establishing)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (!establishing)
                {
                    ...
                    else if (region == TraumaRegion.Legs && brokenLegs == 1 && !pk && to != TraumaLine.None)
                    {
                        if (brokenLegs > rec.LastBrokenLegs || mask.HasFlag(TraumaChangeReason.FractureGained) || mask.HasFlag(TraumaChangeReason.PainkillerLost))
                        {
                            TryPublishOneShot(p, TraumaOneShotKind.InvoluntaryProneOnce, to);
                        }
                    }
                }
