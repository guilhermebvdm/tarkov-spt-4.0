...
namespace TRLImmersiveCombatMedicine.Trauma
{
    ...
    public class FikaReviveGetActionsPatch
    {
        ...
        private static void Postfix(GamePlayerOwner owner, ref ActionsReturnClass __result)
        {
            ...
            {
                TraumaState.Logger?.LogError($"FikaReviveGetActionsPatch: {ex.Message} — ações de revive seguem sem o gate do desfibrilador.");
            }
        }
    ...
    public class FikaRevivePlayerPatch
    {
        ...
        private static void Prefix(bool success, object __instance)
        {
            ...
            {
                TraumaState.Logger?.LogError($"FikaRevivePlayerPatch: {ex.Message} — revive segue sem consumo do desfibrilador.");
            }
        }
        ...
        private static void ConsumeDefibrillator(Player player)
        {
            ...
            if (items != null && items.Any())
            {
                ...
                if (TraumaState.Logger != null) TraumaState.Logger.LogInfo("Consumindo Desfibrilador para reviver aliado!");
