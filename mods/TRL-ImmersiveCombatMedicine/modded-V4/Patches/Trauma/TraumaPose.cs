    ...
    internal static class TraumaPose
    {
        ...
        private static string KindWord(TraumaOneShotKind kind, TraumaRegion region) =>
            kind == TraumaOneShotKind.InvoluntaryFall ? "fall"
                : kind == TraumaOneShotKind.InvoluntaryProneOnce ? "prone-once"
        ...
        /// <summary>Queda em Prone ONE-SHOT (1 perna quebrada): sem entrar no ciclo de bloqueio/re-queda do 004.
        /// O jogador vai ao chão (ou fallback agachado se CanProne recusar), grita de dor, mas pode levantar livremente.</summary>
        internal static void TryInvoluntaryProneOnce(Player p, TraumaRegion region, TraumaOneShotKind kind)
        {
            if (p == null || p.MovementContext == null)
            {
                if (!(p is null) && TraumaEngine.TryGetOneShotDeadline(p, kind, out float dNull))
            ...
            if (AbsorbIfCycleEngaged(p, kind, region)) return;
            if (!CanForcePose(p, out string blockedBy))
            {
                Defer(p, region, kind, blockedBy, internalEntry: false, onExecuted: null);
                return;
            }
            ExecuteProneOnce(p, kind, onExecuted: null);
        }

        private static void ExecuteProneOnce(Player p, TraumaOneShotKind kind, Action<Player, bool> onExecuted)
        {
            var mc = p.MovementContext;
            if (mc.IsInPronePose)
            {
                TraumaEngine.ReportOneShotExecuted(p, kind);
                TRLImmersiveCombatMedicinePlugin.ModLogger.LogInfo($"[Trauma2] prone-once EXECUTED (already-prone) {p.ProfileId}");
                TraumaVoice.PlayStrong(p);
                onExecuted?.Invoke(p, true);
                return;
            }
            ...
            if (!mc.IsInPronePose)
            {
                mc.SetPoseLevel(0f); // fallback agachado caso CanProne recuse silencioso
                TraumaEngine.ReportOneShotExecuted(p, kind);
                TRLImmersiveCombatMedicinePlugin.ModLogger.LogInfo($"[Trauma2] prone-once FALLBACK-CROUCH {p.ProfileId}");
                TraumaVoice.PlayStrong(p);
                onExecuted?.Invoke(p, false);
                return;
            }
            TraumaEngine.ReportOneShotExecuted(p, kind);
            TRLImmersiveCombatMedicinePlugin.ModLogger.LogInfo($"[Trauma2] prone-once EXECUTED {p.ProfileId}");
            TraumaVoice.PlayStrong(p);
            onExecuted?.Invoke(p, true);
        }

        internal static void BotProneDip(Player botPlayer)
        {
            if (botPlayer == null || botPlayer.AIData?.BotOwner == null || botPlayer.MovementContext == null) return;
            if (AbsorbIfCycleEngaged(botPlayer, TraumaOneShotKind.InvoluntaryProneOnce, TraumaRegion.Legs)) return;
            var bo = botPlayer.AIData.BotOwner;
            if (bo.BotLay != null)
            {
                bo.BotLay.IsLay = true;
            }
            else
            {
                botPlayer.MovementContext.IsInPronePose = true;
            }
            TraumaVoice.PlayStrong(botPlayer);
            TraumaEngine.ReportOneShotExecuted(botPlayer, TraumaOneShotKind.InvoluntaryProneOnce);
        }
        ...
        internal static void PumpDeferred()
        {
            ...
            for (int i = _deferred.Count - 1; i >= 0; i--)
            {
                ...
                if (e.Kind == TraumaOneShotKind.InvoluntaryProneOnce)
                {
                    if (!CanForcePose(p, out _)) continue;   // segue adiado (D7)
                    _deferred.RemoveAt(i);
                    ExecuteProneOnce(p, e.Kind, e.OnExecuted);
                    continue;
                }
