...
namespace TRLImmersiveCombatMedicine.Trauma
{
    ...
    public sealed class TraumaLegsConsumer : MonoBehaviour
    {
        ...
        private void OnOneShotCore(Player p, TraumaOneShotKind kind, TraumaLine line)
        {
            if (!IsActive()) return;
            if (p is null) return;
            if (kind == TraumaOneShotKind.InvoluntaryCrouch)
            {
                if (p.IsAI)
                {
            ...
            else if (kind == TraumaOneShotKind.InvoluntaryProneOnce)
            {
                if (p.IsAI)
                {
                    TraumaPose.BotProneDip(p);
                    return;
                }
                TraumaPose.TryInvoluntaryProneOnce(p, TraumaRegion.Legs, kind);
            }
        }
        ...
        private void OnToggleOff()
        {
            ...
            TraumaPose.CancelKind(TraumaOneShotKind.InvoluntaryProneOnce, TraumaRegion.Legs, "toggle-off");
