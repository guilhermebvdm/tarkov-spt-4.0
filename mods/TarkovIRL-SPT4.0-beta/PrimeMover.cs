...
public class PrimeMover : BaseUnityPlugin
{
  ...
  private void LoadConfigValues()
  {
    ...
    PrimeMover.MinimumWeaponSway = this.ConstructFloatConfig(0.3f, "c - Sway Values", "Minimum Weapon Sway", "Guarantees a minimum sway effect even for very light weapons.", 0.0f, 2.0f);
    ...
    PrimeMover.NewSwayPositionMulti = this.ConstructFloatConfig(0.75f, "c - Sway multipliers", "Sway position multiplier", "", 0.0f, 10f);
    ...
    PrimeMover.NewSwayRotFinalClamp = this.ConstructFloatConfig(0.06f, "c - Sway multipliers", "Sway input smoothing clamp", "Touch at your own risk kek", 0.0f, 0.1f);
    ...
    PrimeMover.EfficiencyOverweightIpmact = this.ConstructFloatConfig(250f, "e - Efficiency multipliers", "Efficiency overweight coef", "Controls how much being overweight affects your Efficiency.", 0.0f, 200f);
    ...
    PrimeMover.DeadzoneCustomWeight = this.ConstructFloatConfig(4.0f, "j - Deadzone Values", "Deadzone Fixed Weight", "Fixed weight value used strictly for calculating Deadzone. (Overrides dynamic weapon weight)", 0.0f, 20f);
  ...
  private void FixedUpdate()
  {
    ...
    {
      // Movidos para LateUpdate para melhor sincronia com os frames visuais
    }
  }
  ...
  private void LateUpdate()
  {
    ...
    // Limitador de segurança: se o jogo der uma travada (lag spike) e o tempo entre frames for alto,
    // o limitador em 0.02s impede que os multiplicadores estrapolem 1.0 e causem a rotação bizarra.
    float dt = Mathf.Min(UnityEngine.Time.unscaledDeltaTime, 0.02f);
    PlayerMotionController.UpdateMovementMeasurementsInFDT(dt);
    WeaponSelectionController.UpdateAnimationPump(dt);
    NewDeadzoneController.Update(dt);
    NewSwayController.UpdateLerp(dt);
    HeadRotController.UpdateLerp(dt);
    DirectionalSwayController.UpdateDirectionalSwayLerp(dt);
    FootstepController.UpdateStep(dt);
    SwayController.UpdateLerp(dt);
    ParallaxAdsController.UpdateLerps(dt);
    ParallaxController.Update(dt);
  }
