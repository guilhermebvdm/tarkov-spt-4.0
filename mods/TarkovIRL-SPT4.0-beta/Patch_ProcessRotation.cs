...
public class Patch_ProcessRotation : ModulePatch
{
  ...
  private static bool Prefix(MovementState __instance, float deltaTime)
  {
    ...
    if (!Patch_ProcessRotation._movementContextCache.TryGetValue(__instance, out movementContext))
    {
      movementContext = (MovementContext) Patch_ProcessRotation.movementContextField.GetValue((object) __instance);
      if (movementContext != null)
      {
        Patch_ProcessRotation._movementContextCache.Add(__instance, movementContext);
      }
    }
