...
| 006 | Blindar contra cliente sem o mod / config assimetrica | **Descoberto no review completo (G-03/G-04).** Dois problemas na mesma familia: (a) um participante SEM o mod derruba a fila de eventos de rede do quadro ao receber o pacote de renascimento — o anfitriao retransmite bytes crus antes de decodificar, e o Fika nao protege esse ponto; (b) configuracao diferente entre jogadores pode deixar o caido **sem saida** (observador com o modo ligado esconde o resgate, caido com o modo desligado nao tem tecla). Hoje ambos sao requisito documentado, nao codigo. Saidas possiveis: aperto de mao de capacidade antes de emitir o pacote, e transmitir a marca de "modo ativo" por jogador para gatear o resgate pelo estado de QUEM CAIU. | [006-blindagem-cliente-sem-mod/](./006-blindagem-cliente-sem-mod/) | ⚪ |
...
> **001–004 estão 🟡, não 🟢.** O código está escrito, compila limpo e passou por review adversarial
> (001; o do 002 estava em curso ao fim da sessao), mas **nada foi validado em partida** — compilar não é funcionar (AP-06). Só viram 🟢
> depois do teste in-game descrito no `PROPRIEDADES.md`.
