...
| Alvo | Tipo | Motivo |
|---|---|---|
| [`Fika ClientHealthController.CanBeRevivedByOtherPlayer()`](../../../../references/fika-plugin/Fika.Core/Main/ClientClasses/ClientHealthController.cs#L54) (private) | Postfix → `true` | **O destravamento central.** `CanBeDowned` (declarado em :18, condição em **:22**) exige `!_bledOut && (max==0 \|\| revives<max) && CanBeRevivedByOtherPlayer()`. Só a última condição atrapalha: sem resgate por aliado ela é sem sentido, mas bloqueia jogar sozinho ou ser o último vivo. Patchar **este** método em vez de `CanBeDowned` preserva `_bledOut` — essencial contra laço infinito, porque `BleedOut()` (:125) faz `_bledOut = true; IsAlive = true; Kill()` e conta com `CanBeDowned == false` para a morte passar (AP-07) |
...
| [`ActiveHealthController.Kill(EDamageType)`](../../../../references/eft-decompiled/Assembly-CSharp/EFT.HealthSystem/ActiveHealthController.cs#L3923) | Prefix, `Priority.First` | Roda **antes** do prefixo do Fika. Duas funções: (a) **portão de vidas** — sem vida disponível, marca `_bledOut` e deixa a morte seguir; (b) **guarda o `EDamageType` do quadro atual** para o postfix de `CheckIfDamageShouldInstantKill`, que não tem outra fonte (R-07: `LatestDamageInfo` é `internal` no Fika e `LastDamageInfo` é `protected` no EFT) |
