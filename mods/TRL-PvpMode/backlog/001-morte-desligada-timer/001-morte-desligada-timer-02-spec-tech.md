...
| [`ActiveHealthController.Kill(EDamageType)`](../../../../references/eft-decompiled/Assembly-CSharp/EFT.HealthSystem/ActiveHealthController.cs#L3923) | Prefix, `Priority.First` | **Captura o tipo do dano do quadro** — a única fonte confiável dele. Uma versão intermediária desta spec dispensava este patch e lia `Player.LastDamageInfo`; o code review provou que aquele campo **só é escrito no caminho de dano de combate** (`Player.ApplyDamageInfo`, `Player.ManageAggressor`), enquanto fome/sede/estimulante vão direto ao controlador de vida (`ActiveHealthController.cs:1353` e `:1186`) e chegam por `DestroyBodyPart → Kill(damageType)` (`:3883`). O tipo correto existe **só como argumento de `Kill`** (C-01). O portão de vidas **não** vive aqui: é o `__result = false` do postfix acima |
