...
namespace TRLDynamicSpawn.Patches
{
    ...
    public class MarkerDumper : ModulePatch
    {
        ...
        {
            if (!Settings.enableDebugLogs.Value || __instance.SpawnPointMarkers == null) return;

            foreach (var marker in __instance.SpawnPointMarkers)
            {
                if (marker != null && marker.SpawnPoint.Categories.ContainPlayerCategory())
                    Logger.LogInfo(
    ...
    public class SniperPatch : ModulePatch
    {
        public static BotZone FindFarthestZone(List<BotZone> botZones, Vector3 referencePoint)
        {
            ...
            if (botZones.Count == 1) return botZones[0];

            // Seleção em passagem única O(N) sem alocação de Heap
            BotZone farthestZone = botZones[0];
            float maxDistSq = -1f;
            ...
            for (int i = 0; i < botZones.Count; i++)
            {
                var z = botZones[i];
                if (z == null) continue;
                Vector3 diff = z.CenterOfSpawnPoints - referencePoint;
