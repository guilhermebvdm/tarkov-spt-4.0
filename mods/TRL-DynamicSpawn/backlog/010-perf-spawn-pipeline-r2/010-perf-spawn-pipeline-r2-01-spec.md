...
- [ ] **AC-X2 — nível de cache PMC menor (AUD-01-04, semântica corrigida em CR-01-01).** `AddToTargetBackup` registra um **nível permanente** de perfis por (papel, dificuldade) que o SPT repõe a raid inteira — não "pede N perfis". O nível de USEC/BEAR `normal` passa de 30/30 para **15** via `Initial Profile Preload` (Avançado, 5–30); a chamada para Scav (20) era no-op (o jogo já registra 8 por dificuldade) e foi removida. Trade-off: primeira onda em mapa com cap alto pode esperar um `bot/generate` a mais. O valor certo é calibrado pela medição `profilesInList` + `bot/generate byBackup` da V2.
- [ ] **AC-M5 (complemento, CR-01-05):** linhas operacionais **por onda** (`Map needs bots`, `Map is 100% full`, `Max cap reached early`, `No slots`, invasões de elite/rogue/raider, `Max dynamic cap reached`) continuam sem gate — ≤ 8 por onda, nível Info.
...
- [ ] **Limpeza única da fila cancela uma criação vanilla em voo (boss tardio):** hoje acontece 1×/s; passa a acontecer **1× por raid**, no primeiro aquecimento com humano vivo, após `DelayBeforeFirstWave` (bosses nativos já nasceram). Se ainda gerar NRE na V2, a próxima decisão é remover a limpeza de vez.
...
- [ ] **Fim de raid durante `SpawnGroupBotsCoroutine`:** `StopAllCoroutines` no hook de fim; `IsGeneratingDynamicWave` forçado para `false` antes de qualquer early-return (os `try/finally` são síncronos, mas o reset explícito garante o estado mesmo com o componente já destruído).
