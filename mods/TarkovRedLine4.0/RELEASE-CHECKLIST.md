...
|---|---|---|---|---|
| 2026-07-09 | **2.2.0** ✅ (era 2.1.0) — **canal `config-force`** (config que vai pra TODO MUNDO, sobrescreve customização) | 4.0.0 — mudou só o **default do `config.json`** (que só é escrito em server SEM config.json) → **NÃO precisa redeployar** ⚠️ | 2.4.3 — sem mudança | **só o exe (1)**. A regra `config-force` vive no fallback embutido do launcher; o `SyncRuleResolver` faz merge com as regras do server. |
